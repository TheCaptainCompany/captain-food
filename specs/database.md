...
- **Fed by**: `DeliveryRequested`, `DeliveryAcceptedByPartner`, `DeliveryRejectedByPartner`, `DeliveryStatusUpdated`, `DeliveryAcceptedByRider`, `DeliveryPickedUp`, `DeliveryCompleted`, `DeliveryCancelled`, `DeliveryDispatchFailed`, `DeliveryIssueReported`, `DeliveryIssueResolved`, `DeliveryHandedBackByRider`
...
| `status` | `DeliveryStatus` | `TEXT` | — | Derived from the lifecycle event type / DeliveryStatusUpdated.status (DeliveryDispatchFailed → FAILED, the offer-cap exhaustion) / DeliveryHandedBackByRider.foodLocation (PENDING unless WITH_RIDER, then FAILED). |
| `provider` | `DeliveryProvider` | `TEXT` | nullable | INDEPENDENT (rider accepted) or PARTNER (partner accepted); null while PENDING, and RESET to null by a handback (#639 part C step 3-ii — the job is unassigned again). |
| `rider_id` | `RiderId` | `UUID` | nullable | Set for an independent-rider delivery; null for a partner delivery, and RESET to null by a handback (#639 part C step 3-ii — `TestDeliveryReofferedAfterHandBack` is the fold-reset proof). |
...
| `food_location` | `FoodCustody` | `TEXT` | nullable | Where the food is, set by a handback (DeliveryHandedBackByRider.foodLocation) — the board's pinned card headline and the customer tracking banner's predicate; RESET to null by the next acceptance (rider or partner) so a stale handback marker never survives a re-offer (#639 part C step 3-ii). |
| `handed_back_at` | `timestamptz` | `TIMESTAMPTZ` | nullable | Occurrence time of the latest handback (envelope occurredAt); null until one occurs (#639 part C step 3-ii). |
...
- **Fed by**: `RiderRegistered`, `RiderInfoUpdated`, `RiderStatusChanged`, `RiderRestricted`, `RiderReinstated`
...
| `standing` | `RiderStanding` | `TEXT` | — | The platform's grant (ACTIVE/RESTRICTED) -- see the column comment for why the creating arm (RiderRegistered) never writes it. |
| `created_at` | `timestamptz` | `TIMESTAMPTZ` | — | technical — stamped from event.occurred_at (implicit on every read model) |
| `updated_at` | `timestamptz` | `TIMESTAMPTZ` | — | technical — stamped from event.occurred_at (implicit on every read model) |

### `RiderRestriction` · 🛶 V0 · source aggregate `Rider`

- **Fed by**: `RiderRegistered`, `RiderRestricted`, `RiderReinstated`
- **Rules**: Keyed on `riderId` (grain: the rider, not the restriction fact) -- a repeatable-fact grain (riderId + occurrence) is what the business_metrics.yaml `RiderRestriction` projection needs for `restrictRider`/`reinstateRider`'s own fold; THIS read-side table answers 'what is true of this rider right now', so `RiderRestricted` overwrites the prior attribution rather than appending a row -- the Art. 11 LOG lives in `domain_events`, immutable, never here. No `auth_ref`, no `phone` -- this table answers WHY and WHEN a rider is restricted, never WHO to contact; the identity bridge stays exclusively on `Rider`. Rebuild by resetting the `RiderRestriction` checkpoint (its OWN `ProjectorGroup`, never a prefix under the already-advanced `Rider` checkpoint -- the #424 lesson): its group starts at 0 with no `projection_checkpoint` row, so a table born after the log is backfilled by replay, never a migration-time copy (#639 part C step 4-i round 2 item 7).
...
| --- | --- | --- | --- | --- |
| `rider_id` | `RiderId` | `UUID` | PK |  |
| `standing` | `RiderStanding` | `TEXT` | — | Mirrors Rider.standing (the same two facts write it) -- kept here too so myStanding needs no second table join for the value it renders alongside ground/decidedAt/effectiveAt. |
| `ground` | `RiderRestrictionGround` | `TEXT` | nullable | Null before any restriction (RiderRegistered) and untouched by RiderReinstated (the attribution stays for counsel/admin history until the next RiderRestricted overwrites it). |
| `decided_at` | `timestamptz` | `TIMESTAMPTZ` | nullable |  |
| `effective_at` | `timestamptz` | `TIMESTAMPTZ` | nullable |  |
| `reinstated_at` | `timestamptz` | `TIMESTAMPTZ` | nullable | The occurred_at of the LATEST RiderReinstated fact for this rider (an occurrence timestamp, no payload field) -- history, not a live 'currently reinstated' flag: `standing` is the field that answers whether the rider is restricted right now; this column simply stops being the freshest fact once a later RiderRestricted follows it. |
| `created_at` | `timestamptz` | `TIMESTAMPTZ` | — | technical — stamped from event.occurred_at (implicit on every read model) |
| `updated_at` | `timestamptz` | `TIMESTAMPTZ` | — | technical — stamped from event.occurred_at (implicit on every read model) |

### `RiderRoster` · 🛶 V0 · source aggregate `Rider`

- **Fed by**: `RiderRegistered`, `RiderInfoUpdated`, `RiderStatusChanged`, `RiderRestricted`, `RiderReinstated`
- **Rules**: Keyed on `riderId` (grain: the rider, not the restriction fact) — the same 'what is true of this rider right now' grain as `RiderRestriction`, upserted in place; the Art. 11 LOG lives in `domain_events`, immutable, never here. NO `auth_ref` column, ever — a rule line, not merely an omission, so a future edit that reaches for the identity bridge here is refused at review: this table answers WHO to show an admin, never WHO a session resolves to. A test asserts the column does not exist (`SELECT auth_ref FROM rider_roster` errors). NEVER a folded `heldDeliveryJobId`/`heldStatus` column (ADR §2): the held job is a SEPARATE fold (`View_DeliveryJob`, its own checkpoint over `Order-`/`Payment-`/`DeliveryJob-` streams) that would drift from this table's own `Rider-` replay on a rebuild and answer differently from the `held_by_rider`/`held_by_riders` ports — one custody truth, read fresh at query time, never two. Own checkpoint group `"RiderRoster"` from 0 (never a prefix under the already-advanced `Rider` or `RiderRestriction` checkpoints — the #424 lesson, generalised a second time): a table born after the log is backfilled by replay, never a migration-time copy. Rebuild by resetting the `RiderRoster` checkpoint. No CHECK constraint (`DbFaultPolicy::Skip` semantics, the 4-i `RiderRestriction` reasoning): a stray value fails the ONE row's projection rather than halting the whole group. `standing` is a MECHANICAL `derive:` literal map (`RiderRegistered` → `ACTIVE`, `RiderRestricted` → `RESTRICTED`, `RiderReinstated` → `ACTIVE`) — unlike `Rider.standing`/`RiderRestriction.standing`, which are hand-written `Compute` hooks that preserve the PRIOR value on the creating arm (a checkpoint-reset replay-in-place over an existing row without a TRUNCATE, which those two tables' own rebuild rule forbids). `RiderRoster`'s rebuild is by checkpoint reset AND truncate together (`a_from_zero_replay_yields_the_same_roster_rows`, a full drain from position 0), so chronological event replay alone reproduces the correct end state and the mechanical map is sufficient — a deliberate simplification, not an oversight; a rebuild that resets ONLY this checkpoint without truncating the table would re-grant every restricted rider ACTIVE for the length of the drain, which is why the rule above is TRUNCATE-together, never checkpoint-only.
- **Note**: The admin triage/detail read model (#639 part C step 4-iii-A, ADR-20260904-152807 §1): name, phone, availability and the platform grant (standing/ground/decidedAt/effectiveAt/reinstatedAt) for EVERY rider, one row per rider. NOT `internal` — this table IS the `riders`/`rider` GraphQL `reads` target, ADMIN-only at the door. The held delivery job is deliberately NOT a column here (ADR §2, "one custody truth, read at query time — never a folded column"): the DETAIL reads `held_by_rider(rider_id)` fresh at query time and the LIST joins the page's rider ids to held statuses in one set-based query, both through `DeliveryReadRepository`, never a second fold of the custody lifecycle `View_DeliveryJob` already derives.

- **Indexes**: `(display_name, rider_id)`, `(standing)`

| Column | Type | SQL | Constraints | Notes |
| --- | --- | --- | --- | --- |
| `rider_id` | `RiderId` | `UUID` | PK |  |
| `display_name` | `text` | `TEXT` | — | Mirrors `Rider.display_name` — NOT NULL (RiderRegistered always initialises it; RiderInfoUpdated overwrites it only when it carries one, the partial-update convention). |
| `phone` | `PhoneNumber` | `TEXT` | — | Mirrors `Rider.phone` — a profile attribute, never a lookup key (no unique constraint, no index — French mobile numbers are recycled, the `Rider.phone` rule's reasoning applies identically here). |
| `status` | `RiderStatus` | `TEXT` | — | Availability/lifecycle status, straight off the payload of whichever event wrote last — mirrors `Rider.status` exactly; the LEGACY `SUSPENDED` value decodes and renders (never as an authorization signal, never as `standing`). |
| `standing` | `RiderStanding` | `TEXT` | — | The platform's grant (ACTIVE/RESTRICTED), mechanically derived — see the table rule above for why this column is `derive:` here while `Rider.standing`/`RiderRestriction.standing` are hand-written. |
| `ground` | `RiderRestrictionGround` | `TEXT` | nullable | Null before any restriction and untouched by RiderReinstated (the attribution stays for admin/counsel history until the next RiderRestricted overwrites it) — mirrors `RiderRestriction.ground`. |
| `decided_at` | `timestamptz` | `TIMESTAMPTZ` | nullable | Mirrors `RiderRestriction.decided_at` — a timestamptz VALUE column parsed from the RFC3339 payload string, hence a hand-written `Compute` hook (the generator's own `type_derived`-timestamptz-excludes-ScalarLatest rule), not `derive:`. |
| `effective_at` | `timestamptz` | `TIMESTAMPTZ` | nullable | Mirrors `RiderRestriction.effective_at` — same parsed-string reasoning as `decided_at`. |
| `reinstated_at` | `timestamptz` | `TIMESTAMPTZ` | nullable | The occurred_at of the LATEST RiderReinstated fact (an occurrence timestamp, no payload field) — mirrors `RiderRestriction.reinstated_at` exactly; history, not a live flag. |
| `created_at` | `timestamptz` | `TIMESTAMPTZ` | — | technical — stamped from event.occurred_at (implicit on every read model) |
| `updated_at` | `timestamptz` | `TIMESTAMPTZ` | — | technical — stamped from event.occurred_at (implicit on every read model) |

### `Member` · 🛶 V0 · 🔒 internal · source aggregate `RestaurantMembership`

- **Consumed by**: command handlers / auth resolution (no GraphQL query).
- **Fed by**: `RestaurantAccessGranted`
- **Rules**: Rebuild = checkpoint reset, never TRUNCATE (the `Rider`/`RiderRestriction` precedent, PROP-20260831-180622 §6.4): upsert keyed on `member_id` with ONE creating arm (`RestaurantAccessGranted`), so a from-zero replay rewrites every row in place and no member is denied mid-rebuild. TRUNCATE + reset fails every member closed for the length of the drain. This rule holds only while `Member` carries no grant-shaped column (dba) -- the day one is added, the rebuild discipline must be re-examined exactly as `Rider.standing` forced `RiderRoster` to split off with its OWN opposite recipe. `auth_subject` is UNIQUE, not merely indexed, and the difference is a security property (the `Rider.auth_ref` precedent): the seam's lookup is `fetch_optional`, which on multiplicity returns an ARBITRARY row -- the constraint converts a silent breach into a visible denial. It does NOT create the invariant: the write-side reservation that does is `database/tables/reservations.yaml#/auth_subject_reservations`, keyed `(principal_kind, auth_subject)` -- `grant_restaurant_access` reserves BEFORE the append, so a duplicate is refused up front and this constraint is the second line, never the first. RestaurantAccessRevoked touches NOTHING here (ADR-20260905-101349 §5): the binding a person resolves to outlives any one grant they lose -- revoking access is a `ScopeMembership` fact, never a `Member` fact. No CHECK constraint (`DbFaultPolicy::Skip` semantics, the `RiderRoster` precedent): a stray value fails the ONE row's projection rather than halting the whole group. ERASURE, named here so the #194 sweep cannot miss an app-projected table the generated dispatch skips (the `Rider`/`ScopeMembership` precedent): `auth_subject` and `member_id` are a natural person's data held OUTSIDE the stream, and the deletion engine deletes streams, not projection rows. The `RestaurantMembership` aggregate declares no `deletion:` block today, so there is nothing to build yet -- but a member tombstone fold is OWED the moment one is declared, on the SAME clock as Customer's (ADR-20260905-101349 §11, MEMBER-ACCESS-GRANT-PRECONDITIONS).
...
| Column | Type | SQL | Constraints | Notes |
| --- | --- | --- | --- | --- |
| `member_id` | `MemberId` | `UUID` | PK | Keyed on the DOMAIN person id, never `membershipId` (the grant relationship) -- resolved by the projection worker from the PAYLOAD, not from the stream name: the aggregate that authors this event is `RestaurantMembership-{membershipId}`, a DIFFERENT identity from the row this table keys on (the same cross-stream shape as `ScopeMembership`'s `RestaurantListingClaimed` arm). |
| `auth_subject` | `AuthSubject` | `TEXT` | unique | The login credential this person resolves through (6-ii's seam). UNIQUE, no other index (deliberately no `index: true` beside `unique:` -- the two are separate emitter passes and would emit a redundant second btree, the `Rider.auth_ref` precedent) -- see the table rule. |
| `created_at` | `timestamptz` | `TIMESTAMPTZ` | — | technical — stamped from event.occurred_at (implicit on every read model) |
| `updated_at` | `timestamptz` | `TIMESTAMPTZ` | — | technical — stamped from event.occurred_at (implicit on every read model) |
...
- **Consumed by**: command handlers / auth resolution (no GraphQL query).
- **Fed by**: `OrderPlaced`, `DeliveryAcceptedByRider`, `DeliveryCancelled`, `DeliveryDispatchFailed`, `RestaurantRegistered`, `RestaurantListingClaimed`, `RestaurantAccessGranted`, `RestaurantAccessRevoked`
