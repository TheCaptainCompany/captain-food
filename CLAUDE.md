...
- [specs/database.md](specs/database.md) — event store schema (`domain_events`) + the `View_*` projection (read) tables, each declared by its source aggregate/events, business filters and columns (query→view mapping is the `@reads` binding in [specs/api.yaml](specs/api.yaml), rendered in the generated documentation).
