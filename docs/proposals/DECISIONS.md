...
### 27bis. ⚠️ MET-R — the product owner's own design contradicts two clauses of the ADR. This is a DECISION REVERSAL, filed rather than executed
...
| # | Decision | Recommendation |
|---|---|---|
| **MET-R** | ⬅️ **PRODUCT-OWNER-OWED.** Confirm the reversal. The ADR is `Accepted` and one day old; two of its clauses no longer hold | **Recommended: confirm.** On confirmation the ADR is **superseded by a short new one** (never rewritten — it is a historical record), and the CLAUDE.md business-metrics bullet's mechanism half is updated in the same change. **Nothing implements either design until this row closes** — that is why the amendment was filed rather than executed, per the *"does it contradict a recorded decision"* test in CLAUDE.md |
| **MET-Q7** | The hosted product-analytics SDK row above | **Recommended: CLOSE as "no"** rather than defer. The projection design removes its motivation on the order side (the funnel is a fold over our own log, in our own erasure path) and [PROP-20260811-000946](PROP-20260811-000946-behaviour-event-tracking-in-the-screens-spec.md) removes it on the browse side. What remains is only "buy convenience", against a third-party tenancy holding customer behaviour |
