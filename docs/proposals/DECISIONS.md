...
| **MET-G** | ⚠️ **Not about the fork, and must not be lost inside it**: the projector's per-event failure handling is **log-and-skip**, which is correct for a read model and **wrong for a money-adjacent metric** — a skipped event leaves the count permanently wrong with only an ERROR log | Wants a **projection-lag or parity check**, not a redesign — the shape `alertable:` and [#483](https://github.com/TheCaptainCompany/captain-food/issues/483)'s dead-man's switch exist for. **Adjacent to the `DbFaultPolicy` decision still open from [#474](https://github.com/TheCaptainCompany/captain-food/issues/474)** — cross-referenced, deliberately not duplicated, because the fault policy is the general answer and this is one consumer of it |
| **MET-Q7** | The hosted product-analytics SDK row above | ⬅️ **still PRODUCT-OWNER-OWED. Recommended: CLOSE as "no"** rather than defer. The projection design removes its motivation on the order side (the funnel is a fold over our own log, in our own erasure path) and [PROP-20260811-000946](PROP-20260811-000946-behaviour-event-tracking-in-the-screens-spec.md) removes it on the browse side. What remains is only "buy convenience", against a third-party tenancy holding customer behaviour |
| **MET-W** | ⚠️ **A dependency this surfaced, not a question**: `retention:` as a free duration string (`P90D`) contradicts a **recorded legal position** — *"This table IS the written retention schedule CNIL expects — windows declared **once, in the DSL**, feeding both the sweep and the DPIA"* ([legal brief:82](../legal/BRIEF-20260808-account-erasure-two-path.md)). No duration scalar exists (`Duration`/`Retention`/`interval` = zero hits in `specs/common/scalars.yaml`) | **A declared retention-window catalog, `$ref`'d** — not a `Duration` scalar with a pattern. A pattern catches `P90DD`; it cannot catch a **well-formed window nobody approved**, which is the failure that matters. This is not new work — the legal table exists and already claims to be DSL-declared; this makes the claim true. Owner: arguably [#194](https://github.com/TheCaptainCompany/captain-food/issues/194) rather than either metrics/tracking issue, so [PROP-20260811-000946](PROP-20260811-000946-behaviour-event-tracking-in-the-screens-spec.md) **depends on it** rather than building it |
