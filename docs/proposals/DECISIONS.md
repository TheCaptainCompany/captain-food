...
| **Identity-bridge home** | Where the role↔domain-id bridge lives: JWT claims for all roles vs common-schema bridge tables — must NOT invent a third mechanism beside `Actor.domain_id` | Open — **now URGENT** (final-vision audit A4, 2026-08-09): slice 3 [#415 "Rider identity: View_Rider, register/update/profile surface, onboarding screens (#348 slice 3)"](https://github.com/TheCaptainCompany/captain-food/issues/415) is next in the adopted order and would embed the `View_Rider` `authRef`-bridge option; decide before (or as part of) its approval — **customer brief card** |
...
| **D6 endpoint** (final-vision audit A3, 2026-08-09) | The rider-epic option table calls extending the step DSL (expressing the ReclamationProcess 3-way branch, retiring the wrapper seam and both `emits:`/`sends:` declarations) "the honest endpoint" while recommending the declared `sends:` — under [ADR-20260808-235113](../adr/ADR-20260808-235113-final-vision-first-no-intermediate-steps.md) either `sends:` is DECLARED the final mechanism for hand-written seams, or it is confirmed as staging with the step-DSL branch recorded as the designed final | Open — **customer brief card** |
