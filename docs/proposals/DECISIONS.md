...
| **PROP-185140 §6.4 claim staleness** | How long a scope claim may be trusted before re-derivation — the one real policy question the authorization set leaves open | Open — stays open by explicit decision (ADR-20260808-171056). Widened 2026-08-09 by [#433](https://github.com/TheCaptainCompany/captain-food/issues/433) (claims for ALL roles): customers/riders' only transition is null→set at mint, so the #429 bearer item carries the BLOCKING stamp-before-issue precondition, and #194 erasure must scrub `app_metadata` + revoke refresh tokens (a claim outlives erased data until expiry) |
...
| **Rebrand Captain → Solida** | Class-42 trademark opposition on "Solida" — external, only the customer/opposer resolves it; rename sweep pre-scoped in [#411 "Rebrand Captain → Solida (solida.food): rename sweep, BLOCKED on class-42 trademark confirmation"](https://github.com/TheCaptainCompany/captain-food/issues/411) | Waiting on external — **customer** ([ADR-20260808-212741](../adr/ADR-20260808-212741-solida-studio-strategic-frame.md) §4) |
...
## 25. Platform observability: the technical layer has no contract — PROP-20260810-005727

Surfaced by a capacity question about Postgres disk on OVH, which turned out to have a ~€1.72/month
financial answer and a structural one: `specs/observability.yaml`'s **twelve contracts all bind to
domain workflows**, and no technical resource — disk, connections, certificates, node, GitOps sync,
mailbox depth — is watched by anything. ADR-20260807-114122 dropped the Prometheus stack to afford
the single-node shape and moved alerting to Honeycomb, which receives application OTel only.
**Everything here is a PR against an unapplied desired state** (`deploy/platform/README.md`: *"nothing
applies this tree today"*) — the same work after cutover costs a supervised console session.

Complements §22's **Business-signal observability contracts** row: that one is the business half
(funnel, cohorts, rider utilization), this one is the technical half. Neither subsumes the other.

| Decision | Question | Status / owner |
|---|---|---|
| **D1 — collection mechanism** | OTel Collector Deployment → Honeycomb **plus** the CronJob prober for integrity verdicts (recommended, and it closes [#364](https://github.com/TheCaptainCompany/captain-food/issues/364)'s open collector-placement item) vs prober-only vs kube-prometheus vs an external SaaS. The two recommended mechanisms are complements, not stages — a poller has no trend, and a time series cannot express "the restore drill's checksums matched" | Open — **customer** (blocking concern: the collector's ~300-400 Mi against a node ADR-20260807-114122 already calls snug) |
| **D2 — what constitutes a page** | Every alert path in the tree today ends at a GitHub issue, and a GitHub issue at 20:30 Saturday is not a page. Recommended: issues for everything **plus** SMS via the existing OVH adapter for a named SEV-1 set. At `instances: 1` there is no failover, so the page buys the minutes a manual recovery takes | Open — **customer** |
| **D3 — cause-alerts on non-elastic resources** | §2b practice 8 says alert on SYMPTOMS, not causes; free disk is a cause. Recommended: a narrow named exception with an explicit three-part membership test (cannot be expanded in-incident, hard stop not degradation, cheap action with lead time). Practice 8 **already lists cert expiry**, which is a cause by the same test — so this codifies an exception the doctrine already contains | Open — team first, customer if contested |
| **D4 — where the platform contract lives** | New `platform:` kind inside `specs/observability.yaml` (recommended — one gated home; ADR-20260808-063951 explicitly invites promotion once "a second consumer appears, e.g. capacity math in the validator") vs a separate spec file vs ungated YAML beside `deploy/platform/`. **DSL rule applies**: `specs/**` is never touched by execution loops, so this needs product-owner approval | Open — **customer** (DSL change) |
| **D5 — disk thresholds** | warn 70 / page 85 (recommended) vs the conventional 80/90. 70% of 20 Gi leaves roughly the headroom one rewriting migration needs, and since expansion is a **one-way ratchet** — Cinder never shrinks, and D6 of ADR-20260807-002705 deleted every dump-restore path — *when* the sizing decision gets made is worth more than the alert count. Expansion stays supervised, never automated | Open — team can decide; customer veto |
| **D6 — the emission-site gate** | A validator rule that every declared metric has ≥1 emission site, and every declared probe a scheduled job. `orders_placed_total` — *"the metric that says a stranger paid us"* — has zero emission sites today, so its silence reads as health. Adding forty platform signals to a spec that cannot prove they are emitted multiplies that failure. Runtime registration assertion recorded as the endpoint | Open — team can decide; strongly recommended |
