...
**The decided tracked surface is exactly**: `.claude/skills/decision-lookup/SKILL.md`, `.claude/skills/decision-lookup/scripts/decision-lookup.sh`, `.claude/skills/decision-lookup/scripts/stub-tests.sh`, the `.qmd/` ignore entries in `.gitignore` and `.claudeignore`, and — since the 2026-08-24 narrowing — the single `bash .claude/skills/decision-lookup/scripts/stub-tests.sh` step in `.github/workflows/ci.yml`'s always-run `changes` job together with the `the_stub_suite_runs_in_the_always_run_changes_job` codegen test that pins it — plus this proposal, rows `RETRIEVAL-QMD` and `RETRIEVAL-QMD-CI`, ADR-20260824-205911, the regenerated register index, and the journal entries. **Anything beyond that surface — a Makefile target, a hook, any other CI or workflow step, a validator, agent-contract or mandatory-workflow reference to QMD — requires a new decision row.** The skill is advisory and non-blocking: no gate, hook, validator, or agent contract consumes its output.

**Non-goals — every one of these requires a NEW decision row, none is a fallback:** model downloads of any kind; vector/semantic search; reranking; query expansion; hybrid modes; the QMD MCP server or any server process; GraphRAG (§13); hosted or external services; credentials; hooks; **any CI or workflow change beyond the single pinned stub-suite step**; `.claude/agents/**` or agent-contract edits; `settings.json` changes; decision-register semantic changes; generated-spec changes; production code; incremental index maintenance (young lens: "freshness pressure is how a cache acquires authority" — rebuild-only, §6); any mandatory-workflow rule (the skill is **advisory** in Phase 0 — it creates no obligation to use it).
