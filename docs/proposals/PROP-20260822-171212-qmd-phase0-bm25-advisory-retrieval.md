...
**The decided tracked surface is exactly**: `.claude/skills/decision-lookup/SKILL.md`, `.claude/skills/decision-lookup/scripts/decision-lookup.sh`, `.claude/skills/decision-lookup/scripts/stub-tests.sh`, the `.qmd/` ignore entries in `.gitignore` and `.claudeignore`, and — since the 2026-08-24 narrowing — the single `bash .claude/skills/decision-lookup/scripts/stub-tests.sh` step in `.github/workflows/ci.yml`'s always-run `changes` job together with the `the_stub_suite_runs_in_the_always_run_changes_job` codegen test that pins it — plus this proposal, rows `RETRIEVAL-QMD` and `RETRIEVAL-QMD-CI`, ADR-20260824-205911, the regenerated register index, and the journal entries. **Anything beyond that surface — a Makefile target, a hook, any other CI or workflow step serving this integration, a validator, agent-contract or mandatory-workflow reference to QMD — requires a new decision row.** The skill is advisory and non-blocking: no gate, hook, validator, or agent contract consumes its output.
