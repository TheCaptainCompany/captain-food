...
- **Status**: **Proposed** — decision pending on register row `RETRIEVAL-QMD` (`status: open`). The sandbox-only protocol (§4) is the final form the founder accepted **in principle** on 2026-08-22; per his row-status correction of the same day, **no decision is recorded until it lands on the row in a durable change** — this proposal never implies QMD integration is decided, and a future `decided` outcome speaks only to authorizing the sandbox experiment.
- **Date**: 2026-08-22
- **Decision row**: `RETRIEVAL-QMD` (open; this proposal is its option-space authority — the row is a compact index and link)
