...
- **Status**: **Decided** (founder, 2026-08-22; **narrowed by the founder 2026-08-24**) — a **minimal, advisory, BM25-only QMD integration** is authorized: the `decision-lookup` skill, its wrapper, its hermetic stub suite, and **one CI step that runs that suite**, and nothing else. See the Integration decision record below for the sandbox-spike evidence, the narrow boundary, the non-goals, and what still requires separate approval.
- **Date**: 2026-08-22
- **Decision row**: **`RETRIEVAL-QMD-CI`** is the **chain head** and the controlling row (decided 2026-08-24 by [ADR-20260824-205911](../adr/ADR-20260824-205911-the-decision-lookup-stub-suite-runs-in-ci.md), `reconsiders: RETRIEVAL-QMD`); `RETRIEVAL-QMD` is `superseded` by it, with this proposal still its `decided_by`. This proposal remains the option-space authority for both; the rows are compact indexes and links.
