...
- **Status**: **Decided** (founder, 2026-08-22; **narrowed 2026-08-24**, and **narrowed again 2026-09-01**) — a **minimal, advisory, BM25-only QMD integration** is authorized: the `decision-lookup` skill, its wrapper, its hermetic stub suite, **one CI step that runs that suite**, and — since 2026-09-01 — a corpus that includes the **decision-register row files** `docs/decisions/*.yaml`. Nothing else. See the Integration decision record below for the sandbox-spike evidence, the narrow boundary, the non-goals, and what still requires separate approval.
- **Date**: 2026-08-22
- **Decision row**: **`RETRIEVAL-QMD-ROWS`** is the **chain head** and the controlling row (decided 2026-09-01 by [ADR-20260901-025538](../adr/ADR-20260901-025538-yaml-decision-rows-enter-the-qmd-retrieval-corpus.md), `reconsiders: RETRIEVAL-QMD-CI`). The chain is `RETRIEVAL-QMD` → `RETRIEVAL-QMD-CI` → `RETRIEVAL-QMD-ROWS`; the first two are `superseded`, and both remain readable at their own paths as history. This proposal remains the option-space authority for all three; the rows are compact indexes and links.
...
**Smoke-test result, recorded accurately (evidence, not benchmark)**: scriptless pinned BM25 installed with all lifecycle scripts blocked ("Blocked 6 postinstalls") in 18.1 s; indexed 337 Markdown files in 2.2 s; five test queries at ~0.3 s each returned ranked, scored, path-anchored candidates that surfaced the deciding ADR first on three of five questions and beat flat `rg` lists on the vaguest query. At the time of the spike its default Markdown mask **excluded `docs/decisions/*.yaml`** — **that limitation is retired as of `RETRIEVAL-QMD-ROWS` (2026-09-01)**: rows are indexed, and a row key retrieves its own row at rank 1. **Both QMD and `rg` missed the controlling record on the docs-only-CI citation case** (this caveat stands); and the corpus was contaminated by this proposal's own example transcripts — no performance claim transfers without a clean, contamination-excluded benchmark.

**What is authorized (all of it, nothing more)**: `.claude/skills/decision-lookup/SKILL.md` + `.claude/skills/decision-lookup/scripts/decision-lookup.sh` + `.claude/skills/decision-lookup/scripts/stub-tests.sh`, plus (narrowing of 2026-08-24) **exactly one `.github/workflows/ci.yml` step running that stub suite in the always-run job pinned by the `the_stub_suite_runs_in_the_always_run_gate_job` codegen test, and exactly one codegen test pinning it there** — the locus is named by that PIN TEST, never by a job name, because a job name drifts every time the locus moves (it already did: `changes` → `gate-scripts`, `GATE-STEP-LOCUS` option (a), founder 2026-08-27) and the chain head then carried a false statement for days; local, scriptless, exact-pinned QMD BM25 over the approved committed corpus — **Markdown *and* the `docs/decisions/*.yaml` register rows** since the 2026-09-01 narrowing (`RETRIEVAL-QMD-ROWS`), which retires this fence's earlier "committed-Markdown" wording; at most three candidates with path and short excerpt; a fixed advisory disclaimer on every output; mandatory direct reading of candidates and exact `docs/decisions/<KEY>.yaml` resolution before any decision assertion or founder question; `rg + aliases` fallback whenever QMD is unavailable, stale, or empty; **no result is never evidence of "undecided"**.

**Non-goals (each requires a new decision)**: GraphRAG (the [#643](https://github.com/TheCaptainCompany/captain-food/issues/643) deferral stands untouched), vector search, embeddings, model downloads, reranking, query expansion, hybrid modes, **MCP or any server**, **hosted services**, **credentials**, incremental index maintenance, hooks, **any other CI or workflow change that references or serves this integration** (the clause governs the QMD surface, not unrelated CI work), validator changes, agent-contract changes, `settings.json` changes, decision-register **semantic** changes, generated-spec changes, production code, and any mandatory-workflow rule. **The retrieval surface stays a local CLI over a gitignored, disposable cache** — no server, no daemon, no network path on the lookup path — the skill is advisory and non-blocking, and the CI step tests the **wrapper**: it runs no QMD, installs nothing, and never touches a live `.qmd/` cache. **QMD is not memory and not decision authority**: it retrieves candidates from the committed corpus — Markdown **and** the `docs/decisions/*.yaml` rows since `RETRIEVAL-QMD-ROWS` (2026-09-01); the earlier "committed Markdown" wording is retracted, not merely dated, because it was the premise behind "the index cannot see row files"; a result or absence of a result never decides anything; `rg + aliases`, direct source reading, exact row resolution, and the AskUserQuestion gate are unchanged and remain the authority path.
...
**Separate approval still required**: executing the activation test (the first `--install` run) in any environment; **any further corpus-mask widening** (the 2026-09-01 widening to `docs/decisions/*.yaml` is decided and is the only one); any Phase-1-type capability.

### The 2026-09-01 narrowing — row indexing (`RETRIEVAL-QMD-ROWS`)

Founder, verbatim: *"integrate yaml decision row indexing in retrieval qmd ci"*. The register's
**declaration site** is now in the corpus. Design points that are decisions, not mechanics:

- **The mask exists in THREE places and all three must agree**: the `git archive` pathspec, the
  `find … ! -name '*.md'` sweep, and **qmd's own collection glob** (`pattern:` in the corpus-local
  `.qmd/index.yml`, defaulted by the tool to `**/*.md`; `qmd collection add . '<glob>'` accepts a
  pattern argument and silently ignores it). Widen two of three and the corpus exports the rows,
  keeps them, indexes an empty `.yaml` arm, stamps, and answers **forever without returning a row**.
- **A rebuild-time ingestion canary** closes that: a nonce-bearing `docs/decisions/*.yaml` file is
  planted before `qmd update` and searched for after; zero hits wipes the caches, **never stamps
  the corpus**, and takes a named fallback. A **nonce**, not register content — a guard coupled to a
  real row would red whenever anyone opens or supersedes an unrelated decision.
- **Excluded**: `_legacy.yaml` and `_exempt.yaml` (control files, not rows — and a hit on
  `_legacy.yaml` points at a key with no row file to resolve, the one case mandatory resolution
  cannot discharge) and `docs/decisions/README.md` (by construction, the pathspec is `*.yaml`).
- **Included**: every row, `superseded` and `withdrawn` alike — `superseded_by:` is what makes a hit
  on a retired row resolvable to its head, so the DAG is the point.
- **Rendering**: the **path is the payload**. A row candidate prints as `resolve … at HEAD`, never
  excerpted, because no field subset of a row is safe out of context (`PMW-1` is `status: "decided"`
  beside a `note` recording that its premise is gone). Every lookup prints
  `corpus: <sha> (working tree not indexed)`.
- **`docs/decisions/**` is NOT added to `claude_citation_corpus`** and must not be: rows cite
  superseded rows by construction, so wiring them in would red `make validate` across the register.
- **Failure protocol extended**: **corpus composition** is a fifth protected dimension, **silent
  partial ingestion** a fourth trigger.
...
**The decided tracked surface is exactly**: `.claude/skills/decision-lookup/SKILL.md`, `.claude/skills/decision-lookup/scripts/decision-lookup.sh`, `.claude/skills/decision-lookup/scripts/stub-tests.sh`, the `.qmd/` ignore entries in `.gitignore` and `.claudeignore`, and — since the 2026-08-24 narrowing — the single `bash .claude/skills/decision-lookup/scripts/stub-tests.sh` step in `.github/workflows/ci.yml`'s always-run `gate-scripts` job (moved out of `changes` when `GATE-STEP-LOCUS` was decided to option (a), founder 2026-08-27, ADR-20260827-081500) together with the `the_stub_suite_runs_in_the_always_run_gate_job` codegen test that pins it — plus, since the 2026-09-01 narrowing, the corpus composition itself (the `docs/decisions/*.yaml` pathspec arm, the collection-glob widen, the ingestion canary and its companion ingestion-**completeness** check, whose red against the real pinned tool opened `RETRIEVAL-QMD-INGEST-LOSS` and left the tool fallback-only pending that decision) — and this proposal, rows `RETRIEVAL-QMD`, `RETRIEVAL-QMD-CI`, `RETRIEVAL-QMD-ROWS` and the open `RETRIEVAL-QMD-INGEST-LOSS`, ADR-20260824-205911, ADR-20260901-025538, the regenerated register index, and the journal entries. **Anything beyond that surface — a hook, any other CI or workflow step serving this integration, a validator, agent-contract or mandatory-workflow reference to QMD — requires a new decision row.** The one exception, named rather than implied: the **`make stub-tests`** target, which carries no QMD behaviour at all — it passes `DECISION_LOOKUP_ALLOW_DIRTY=1` to the suite already on the decided surface, exactly as `make hooks-test` does for the sibling guard, and rides along under the superseded `RETRIEVAL-QMD-CI` clause (b), with `docs/decisions/README.md` + ADR-20260821-010543 as its authority. This sentence read "a Makefile target" as an unqualified non-goal while such a target landed in the same PR (review #15); a self-verification block that is default-on and has no interactive entrypoint gets DELETED rather than opted out of, which is why the target is ergonomics for a decided surface and not a widening of it. The skill is advisory and non-blocking: no gate, hook, validator, or agent contract consumes its output.

**Non-goals — every one of these requires a NEW decision row, none is a fallback:** model downloads of any kind; vector/semantic search; reranking; query expansion; hybrid modes; the QMD MCP server or any server process; GraphRAG (§13); hosted or external services; credentials; hooks; **any other CI or workflow change that references or serves this integration** (the clause governs the QMD surface, not unrelated CI work); `.claude/agents/**` or agent-contract edits; `settings.json` changes; decision-register semantic changes; generated-spec changes; production code; incremental index maintenance (young lens: "freshness pressure is how a cache acquires authority" — rebuild-only, §6); any mandatory-workflow rule (the skill is **advisory** in Phase 0 — it creates no obligation to use it).
...
- **6.2 The cache is project-local and never enters Git or Claude context** (decided integration, superseding the sandbox location): `.qmd/` lives at the repository root — `tool/` (pinned package + lockfile + manifests), `corpus/` (`git archive` export of the one resolved HEAD SHA — never a later symbolic-`HEAD` re-resolution — + `.sha` revision stamp carrying that same resolved SHA + the project-local index database qmd 2.8.3 writes at `corpus/.qmd/index.sqlite`), `index/` (qmd's HOME-side config) — **gitignored and claudeignored** (entries landed with the integration commit), derived, disposable, never authoritative, never citable. Before every lookup the wrapper validates the cache: `corpus/.sha` must equal `git rev-parse HEAD` **and** the index database must exist — a matching stamp with a missing index is a broken cache and is rebuilt, never served as an empty result. HEAD is resolved **once** into a SHA used for both the `git archive` source and the stamp (no re-resolve race); the working tree is never indexed; a failed rebuild wipes the caches and yields the `rg + aliases` fallback, never stale output. The stamp is written only after the index database is verified present and non-empty — an update that succeeds without producing the index is a rebuild failure, never a stamped cache. The cache-hit check also runs a bounded openability probe (**immutable read-only** sqlite connect — zero writes, zero locks, zero busy timeout — + `PRAGMA schema_version`; never `quick_check`/`integrity_check` per lookup; a read-write or even plain read-only connect would touch the derived index via WAL recovery or `-shm` creation, i.e. a repair §6.3 forbids), and a failed search wipes the derived caches before its named fallback — both per §6.3's delete-wholesale-never-repair policy, so a corrupt index **that qmd reports as a non-zero exit** is rebuilt rather than degrading every lookup until HEAD changes (corruption surfacing as a successful exit with garbage or empty output keeps the cache and degrades per-HEAD — the output-contract fallback deliberately never wipes, because it is also the schema-pin-mismatch path). Probe **unavailability or failure is never read as corruption** — only the probe's deliberate not-openable verdict (exit 1) may wipe: python3 is preflighted on the lookup path by execution, and on any other probe outcome (missing compile-optional sqlite3 module, import-chain failure, signal death) the best-effort probe accepts the stamped non-empty hit at the pre-probe trust level (the rebuild arm serves exactly that trust level unprobed, and qmd bundles its own SQLite, so host-python module absence says nothing about the index; refusing would disable the advisory tool on such hosts between HEAD changes for zero gained safety). Honest cost of the search-failure wipe: the exit code cannot distinguish a damaged index from qmd rejecting the query itself, so a query-triggered failure also wipes and the next lookup pays a full rebuild — accepted over ever serving a possibly-poisoned cache. `docs/status/**` is excluded from the corpus (the journals narrate this tool's own verification queries and answers — the recorded self-contamination shape; `rg + aliases` still searches them directly). `DECISION_LOOKUP_HOME` overrides the cache location for hermetic tests only.
...
- **`--install` (the activation test)**: the only path that touches the network; pinned `bun add --exact`, scriptless (`trustedDependencies: []` verified **structurally** via stdlib JSON parsing — the key must exist and be exactly an empty list — plus `bunfig.toml` `ignoreScripts = true`), lockfile integrity verified via a **structural version↔integrity binding** on the `@tobilu/qmd` packages entry — bun's JSONC trailing commas stripped, the file json-parsed, and the one entry must itself name the exact pin AND carry the recorded digest as its final element (never two independent presence greps, which different entries could satisfy); python3 is preflighted **by execution** before any network touch, since the binding verdict routes through it. Exits **non-zero** on any failure with the remove-`.qmd/`-before-retry instruction and the reversal-decision rule.
...
`RETRIEVAL-QMD-ROWS` — before any reinstall or any change to package, version, permissions,
dependency shape **or corpus composition** (the fifth protected dimension, added 2026-09-01).
Name the head, never `RETRIEVAL-QMD` or `RETRIEVAL-QMD-CI`: both are `superseded`, and the validator
...
**This sentence has now had to be moved twice** — which is why the head is named here rather than
implied, and why the wrapper's own `activation_fail` message names it too.
