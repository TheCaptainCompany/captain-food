...
- **Concerns** (a proposal cannot move to `Approved` while one is unchecked):
  - [x] **public-graph-limits**: `limit_depth` / `limit_complexity` occurred **nowhere** in `specs/**` or `crates/**` before 6-ii. **Discharged 2026-09-05, round 2 of 6-ii ([PR #899](https://github.com/TheCaptainCompany/captain-food/pull/899))**: a per-role `parse_query` extension, `crates/server/src/graphql/query_limits.rs:1` (registered unconditionally in `crates/server/src/graphql/schema.rs::build_schema_for_scope`), refuses a document past its acting role's ceiling BEFORE any resolver runs — the ceilings are codegen-derived, `crates/server/src/graphql/generated/limits.rs:1` (`tools/codegen-rs/src/emit/web.rs::compute_graphql_role_limits`), per-role max over every GENERATED client document a screen actually binds, times the declared `GRAPHQL_LIMIT_HEADROOM_PERCENT`. Proof: `crates/server/tests/graphql_role_limits.rs:1` (per-role depth boundary pair, a complexity-based per-role differentiation test, the M4 mutant — limits collapsed to PUBLIC-only — planted and reverted); `crates/server/src/graphql/query_limits.rs`'s own unit tests (the field-count rule, introspection exemption, fragment/cycle handling, the headroom-floor safety test); `tools/codegen-rs/src/tests.rs::graphql_role_limits_gate` (the emitted file matches a fresh recomputation; every role clears the floor).
...
| `AccessBasis` (né `AccessEvidence`, renamed 2026-09-05 — only one of the four is evidence we hold, [ADR-20260905-101349](../adr/ADR-20260905-101349-step-6-lands-in-four-slices-the-bridge-and-the-grant-first-the-door-second-and-the-accept-is-two-commands-in-two-lanes.md) §3) | `specs/network/` | The closed discriminator on the ONE grant act: `CAPTAIN_ONBOARDING \| GOOGLE_BUSINESS_PROFILE \| OWNER_DECLARATION \| MEMBER_INVITATION`. | Four sibling commands would be four write paths into the table every read-authorization predicate resolves against, each with its own guard to get wrong. |
| `MemberAuthority` | `specs/network/` | The authority axis: `MANAGER \| OPERATOR` (settled 2026-09-05, [ADR-20260905-101349](../adr/ADR-20260905-101349-step-6-lands-in-four-slices-the-bridge-and-the-grant-first-the-door-second-and-the-accept-is-two-commands-in-two-lanes.md) §3; `ADMINISTRATOR` refused — it collides with the platform ADMIN stem). | **Provenance test** (`evans`): *could a colleague invited on a Tuesday hold this value?* `ADMINISTRATOR` — yes. `OPERATOR` — yes. **`OWNER` — no**, which is exactly why `OWNER` is provenance masquerading as authority. Ownership, when it matters, is `AccessEvidence`. |
...
Three states, two authorities. `MANAGER` sees the controls (`ADMINISTRATOR` was refused — one stem
for two populations beside the platform ADMIN, ADR-20260905-101349 §3, fixed here 6-iv); `OPERATOR`
sees the list without them — **the control is absent, not disabled**.
...
| 6 | **Staff authentication, in FOUR slices** — ruled 2026-09-05 by the team, [ADR-20260905-101349](../adr/ADR-20260905-101349-step-6-lands-in-four-slices-the-bridge-and-the-grant-first-the-door-second-and-the-accept-is-two-commands-in-two-lanes.md) (thirteen lenses): **6-i — MERGED via [PR #897](https://github.com/TheCaptainCompany/captain-food/pull/897)** (squash `e7582283`, two review rounds, follow-ups [#898](https://github.com/TheCaptainCompany/captain-food/issues/898)) — the bridge and the grant: the `RestaurantMembership` aggregate (`GrantRestaurantAccess` / `RevokeRestaurantAccess`, ADMIN-only, human-acted; accepts only `basis: CAPTAIN_ONBOARDING`, the other three `AccessBasis` values declared and refused), the `(MEMBER, authSubject)` reservation in 2a's `auth_subject_reservations` (no new table, no release), the `Member` bridge table (own checkpoint from 0), `ScopeMembership`'s targeted MEMBER grant/revoke pair (never the broad role-wide delete), the `RUN_MEMBER_ACCESS_GRANT` gate (default false, `MEMBER-ACCESS-GRANT-PRECONDITIONS` open) and the spec-only `restaurants_with_a_bound_member` fold. Ships DARK: no query, no UI, no door — inventory until 6-ii. **6-ii — the door — MERGED via [PR #899](https://github.com/TheCaptainCompany/captain-food/pull/899) (squash `6369aeb1`, 2026-09-05) at the three-round ceiling; the `public-graph-limits` Concern is ticked; the door stays DARK, its flip gated by `MEMBER-SIGN-IN-DOOR-PRECONDITIONS`**: PUBLIC `requestMemberSignInLink`/`confirmMemberSignIn` (byte-identical accept response for a member and a stranger; the `Member` bridge never consulted on the request leg), the third role-only stamper `identity.stamp_member_claim` (`{role: MEMBER}`, no id in the claim), the MEMBER arm of `crates/server/src/auth.rs`'s identity seam (auth_subject → Member → ScopeMembership → exactly one restaurant scope, fails closed otherwise), the `sign_in`/`sign_in_return`/`not_linked` screens on `restaurant_backoffice.yaml` (round 2 landed the hand-written `sign_in_return` return leg — the mail-client link now goes somewhere), the `member-sign-in` observability contract, behind `RUN_MEMBER_SIGN_IN_DOOR` (default false, `MEMBER-SIGN-IN-DOOR-PRECONDITIONS` open, now also naming the return route/`?next=`/Art. 13/Art. 30 items as preconditions of the FLIP). **The public-graph-limits Concern DISCHARGES in round 2** — see the Concerns list above for the mechanism and its tests; round 1's deferral is superseded. **6-iv — roster and invitation — MERGED via [PR #901](https://github.com/TheCaptainCompany/captain-food/pull/901) (squash `95542ada`, 2026-09-05) at the three-round ceiling; the invite door stays DARK behind `RUN_RESTAURANT_INVITATION`, its flip gated by `RESTAURANT-INVITATION-PRECONDITIONS`**: the `RestaurantInvitation` aggregate (`InviteRestaurantMember`/`RevokeRestaurantInvitation`/`AcceptRestaurantInvitation`, PENDING→{ACCEPTED,REVOKED,EXPIRED} lifecycle, TTL reminder declared behind `RESTAURANT_INVITATION_TTL_SECONDS`, `UNVERIFIED input` 604800s). **Round 2 (ADR-20260905-101349 §2 amendment, a presentation pass)** split the accept's second leg by PRIVILEGE rather than field: `GrantRestaurantAccess` reverted ADMIN-only with every field required; a new PUBLIC `GrantRestaurantAccessByInvitation { invitationId, token }` proves the caller IS the accepting subject (the same token proof, no new legal actor) and DERIVES `membershipId = UUIDv5(invitationId)` (structural one-membership-per-invitation, `UNVERIFIED input` namespace) — closing round 1's gap where the widened single command let an anonymous caller mint a membership for any subject on any restaurant. The `RestaurantRoster`/`RestaurantInvitationList` read models are now real (own projector groups, own migration, `restaurantRoster`/`restaurantInvitations` GraphQL queries, executable rebuild-recipe tests), and the `/team`/`/invitation` screens (C/D) are now built: a live roster/invite/revoke screen and a hand-written `/invitation` acceptance landing page sequencing the two-lane accept client-side. The MANAGER-authority guard is now GENUINELY enforced server-side (`AuthorityGuard` + `MemberAuthorityRepository` reading `domain_events` joined to the `member` bridge, never the roster projection), superseding round 1's "GraphQL-layer only, effectively unbuilt" state. `RUN_RESTAURANT_INVITATION` stays default false (`RESTAURANT-INVITATION-PRECONDITIONS` open, now also naming [#902](https://github.com/TheCaptainCompany/captain-food/issues/902)'s expiry-delivery gap and the Art. 14 correction). **Still NOT landed, declared and tracked**: a revoke confirmation step (`open_bottom_sheet` carries no per-row context); `invitationPreview` (needs a resolver mixing live token verification with a read-model lookup, no precedent in this codebase — reported rather than invented); the TTL reminder's DELIVERY (not its scheduling) needs a `RecordLeg` match arm in the fenced `mailbox/handler.rs` (#902); the roster's `RestaurantAccessRevoked` arm (a revoked colleague stays listed). **6-v — the platform grant and the ADMIN seam binding — MERGED via [PR #907](https://github.com/TheCaptainCompany/captain-food/pull/907) (squash `2c3bf3f1`, 2026-09-06) at the three-round ceiling** (round 3 narrowed the deploy secret gate's optionality to an EXPLICITLY declared `required: []` after round 2 had widened it by omission; follow-ups [#908](https://github.com/TheCaptainCompany/captain-food/issues/908)) (issue [#905](https://github.com/TheCaptainCompany/captain-food/issues/905)) ([ADR-20260905-223957](../adr/ADR-20260905-223957-the-admin-door-in-two-slices-a-platform-membership-binds-the-admin-seam-then-the-system-host-is-routed.md): 6-iii's own precondition — "only once an admin can sign in" — had no slice): a new `PlatformMembership` aggregate (`GrantPlatformAccess`, ADMIN-only, `basis: CAPTAIN_ONBOARDING` only, no revoke command); the `PlatformMember` bridge (own checkpoint from 0, standalone, no `ScopeMembership` prefix); `crates/server/src/auth.rs`'s ADMIN seam makes `Identity::Admin` unspellable without a live grant, mirroring the RIDER/MEMBER seams; a one-shot `bootstrap-platform-admin` subcommand dispatches the FIRST grant through the ordinary mailbox door (deterministic id, never a row/SQL/migration); the `admin-sign-in` observability contract; behind `RUN_PLATFORM_ACCESS_GRANT` (default false, `ADMIN-DOOR-PRECONDITIONS` open). **6-iii — System host routing behind auth + the ADMIN sign-in door** (issue [#906](https://github.com/TheCaptainCompany/captain-food/issues/906)): briefed 2026-09-06 by eight lenses, all carrying the member-door shape (a PUBLIC `requestAdminSignInLink`/`confirmAdminSignIn` pair, identify-only against the `PlatformMember` bridge, its own width-5 actor, `AdminAccessNotGranted`, routing ungated / door behind `RUN_ADMIN_SIGN_IN_DOOR`) — the record ([ADR-20260906-023825](../adr/ADR-20260906-023825-the-admin-sign-in-door-is-the-member-doors-shape-identify-only-against-the-platform-grant.md)) landed as the first commit of its PR. **IN PROGRESS via [PR #909](https://github.com/TheCaptainCompany/captain-food/pull/909) (DRAFT)**: the door specs/handlers, the `Surface::System`/`HostRoute::System` routing, the `sign_in`/`sign_in_return`/`no_access` screens, the `admin-sign-in-door` observability contract, and red-first tests with mutants m1-m4/m6/m7 all landed and green. **STOPPED before hand-off** (farley's own Q2 CATCH, the card's unconditional `deploy/generated/**` STOP condition): `make generate` added a `/public/graphql` ingress path on `system.captain.food` (`deploy/generated/manifests/ingress.yaml`) — mechanically the SAME rule that already put that path on `restos.captain.food` (6-ii) and `riders.captain.food` (2c-i), triggered here because the door's own screens carry `graphql_role: PUBLIC`, but a diff farley's Q2 said would not occur. Gates NOT fully run (m5's gauge-hardcoded-1 mutant, `make rust`, DB-gated tests, `make wasm`, clippy) — see the hand-back for the full state. | 1, 2c-ii for R1; 6-i (6-ii, verification-only per the ADR) | HOLD: human (identity, Tours-facing) |
...
2. **`MemberAuthority`'s values** (§2.1): **settled 2026-09-05 — `MANAGER | OPERATOR`**, two values, additive-only,
   no `*AuthorityChanged`; `ADMINISTRATOR` refused (one stem for two populations beside the platform ADMIN), [ADR-20260905-101349](../adr/ADR-20260905-101349-step-6-lands-in-four-slices-the-bridge-and-the-grant-first-the-door-second-and-the-accept-is-two-commands-in-two-lanes.md) §3.
   `AccessEvidence` is renamed **`AccessBasis`** there (declared four, 6-i accepts only `CAPTAIN_ONBOARDING`).
