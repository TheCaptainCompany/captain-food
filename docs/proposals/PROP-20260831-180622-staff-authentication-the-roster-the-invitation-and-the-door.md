...
| 6 | **Staff authentication, in FOUR slices** — ruled 2026-09-05 by the team, [ADR-20260905-101349](../adr/ADR-20260905-101349-step-6-lands-in-four-slices-the-bridge-and-the-grant-first-the-door-second-and-the-accept-is-two-commands-in-two-lanes.md) (thirteen lenses): **6-i — MERGED via [PR #897](https://github.com/TheCaptainCompany/captain-food/pull/897)** (squash `e7582283`, two review rounds, follow-ups [#898](https://github.com/TheCaptainCompany/captain-food/issues/898)) — the bridge and the grant: the `RestaurantMembership` aggregate (`GrantRestaurantAccess` / `RevokeRestaurantAccess`, ADMIN-only, human-acted; accepts only `basis: CAPTAIN_ONBOARDING`, the other three `AccessBasis` values declared and refused), the `(MEMBER, authSubject)` reservation in 2a's `auth_subject_reservations` (no new table, no release), the `Member` bridge table (own checkpoint from 0), `ScopeMembership`'s targeted MEMBER grant/revoke pair (never the broad role-wide delete), the `RUN_MEMBER_ACCESS_GRANT` gate (default false, `MEMBER-ACCESS-GRANT-PRECONDITIONS` open) and the spec-only `restaurants_with_a_bound_member` fold. Ships DARK: no query, no UI, no door — inventory until 6-ii. **6-ii — the door — landed on [PR #899](https://github.com/TheCaptainCompany/captain-food/pull/899), DRAFT, not yet reviewed/merged**: PUBLIC `requestMemberSignInLink`/`confirmMemberSignIn` (byte-identical accept response for a member and a stranger; the `Member` bridge never consulted on the request leg), the third role-only stamper `identity.stamp_member_claim` (`{role: MEMBER}`, no id in the claim), the MEMBER arm of `crates/server/src/auth.rs`'s identity seam (auth_subject → Member → ScopeMembership → exactly one restaurant scope, fails closed otherwise), the `sign_in`/`not_linked` screens on `restaurant_backoffice.yaml`, the `member-sign-in` observability contract, behind `RUN_MEMBER_SIGN_IN_DOOR` (default false, `MEMBER-SIGN-IN-DOOR-PRECONDITIONS` open). **The public-graph-limits Concern does NOT discharge**: per-role GraphQL depth/complexity limits were not built this slice — the server serves ONE master schema for every role (role-as-path is a runtime ACL, not seven compiled schemas), so async-graphql's built-in schema-build-time limiter cannot differ per role; a genuinely per-role ceiling needs a custom `parse_query` extension plus a codegen-derived per-role ceiling table, neither built yet (reason and remaining scope recorded on PR #899's own comment, per the Concern's own stated alternative). Also deferred on 6-ii: the hand-written `sign_in_return` confirmation-landing page (declared as a `gaps:` line on `sign_in` instead of building a `HandWrittenScreen`). **6-iv — roster and invitation**, **6-iii — System host routing** (last): not started. | 1, 2c-ii for R1, and the **public-graph-limits** Concern (still open after 6-ii — see above); 6-i (6-ii, verification-only per the ADR) | HOLD: human (identity, Tours-facing) |
