...
Every one of the 57 bin manifests carries a header asserting that *"linking a domain crate is the
ONLY way that scope's vocabulary exists in this deployable, so the wrong coupling is **unspellable**
rather than merely unrouted"* (`tools/codegen-rs/src/emit/bins.rs:550-552`). For 45 of them the
opposite is true, and the declared dependency is not merely insufficient — it is **unused**:
...
| # | Defect | Evidence |
|---|---|---|
| 1 | 45 of 57 bins link all 8 domain scopes + `application` + `infrastructure` | `cargo tree -p projector-catalog`; `crates/bin_runtime/Cargo.toml:2-4` |
...
identical code, gated by configuration. The 12 clean bins (5 surfaces `bo-*`/`fo-*`, 7 `gateway-*`)
prove the target shape is reachable: they link neither `bin_runtime` nor `server` and their closure
is honest today.
...
| **(C) Codegen test over the transitive graph** — assert each bin's closure ⊆ its declared scopes ∪ a platform allowlist | Fails the build on the 46th violation; cheap; **measures** every decomposition slice (rows deleted). Landable today with the 45 current violations enumerated | Level 4 — a check an agent can edit. Alone it either fails the build immediately (unlandable) or ships with a 45-row excuse list that becomes permanent |
...
| (d) All 45 at once | One landing | Not reviewable; touches every family's runtime simultaneously |
...
1. **Slice 0 — the ratchet (C).** Codegen test `bin_scope_closure_matches_declaration`: for each
   ...
   its family and the slice that will remove it. Lands green with 45 rows. **No bin may join the
...
4. **Slice 3 — actors + PMs**, i.e. [#307](https://github.com/TheCaptainCompany/captain-food/issues/307)
...
| Per-actor implementation crates | D2 **(a)**, decided 2026-08-02 | Tracked, unbuilt, costing owed — [#307](https://github.com/TheCaptainCompany/captain-food/issues/307) |
...
- The program is large and its value is invisible to a customer. Under any ordering but the current
  directive it would keep losing to features — which is how it reached 45 bins.
