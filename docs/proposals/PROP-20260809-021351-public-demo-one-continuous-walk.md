...
- **Concerns**:
  ...
  - [ ] `comparison-not-computed` **— HALF RESOLVED, corrected in place 2026-08-12; stays unchecked because the half that matters commercially is live.** The **total now computes**: the cart projection is a pure money-free fold and the read side prices lines fresh via `application::pricing::price_cart` ([ADR-20260810-112836](../adr/ADR-20260810-112836-cart-priced-live-on-read.md), [#451](https://github.com/TheCaptainCompany/captain-food/issues/451)) — the carry-forward accessors this concern described **no longer exist**. The **comparison is still always `None`**, now in the read seam rather than the projector (`crates/server/src/graphql/cart_read.rs:187`, with the Order-side `uber_*` columns still `TODO(runtime)` at `crates/application/src/projectors/order_tracking.rs:95`). So the commercial screen renders a real total beside a claim that does not compute.
  ...
  - [ ] `fee-breakdown-anchors-zero` **— UNCHANGED, re-verified 2026-08-12 at `crates/application/src/pricing.rs:105-113`** (the line range moved with the money-free-fold change; the zeroes did not): it hard-zeroes delivery, service fee, restaurant contribution, rider payout and platform net. Publishing that breakdown anchors free delivery before the delivery-fee decision exists.
...
| **G5** ✅ **FIXED** (re-verified 2026-08-12) | `/checkout` — **the walk stopped here** | **FIXED by [#420](https://github.com/TheCaptainCompany/captain-food/issues/420) + [#451](https://github.com/TheCaptainCompany/captain-food/issues/451).** The `if !screen.sdui { return; }` guard that sat above the crate's only `mount_to_body` is **gone** (`renderer.rs:645-646` records its removal by name), and the hardcoded SSR values have **zero occurrences** in `crates/web/src/router.rs` — the shell now carries the cart it is about to charge for and the publishable key, both under named tests (`router.rs::the_checkout_shell_carries_the_cart_it_is_about_to_charge_for`, `…_delivers_the_publishable_key_only_when_configured`). **As written when this proposal landed**: the page was inert — `hydrate()` returned early for every `sdui: false` screen; no Stripe element mounted; `checkout::submit()` was unreachable from a browser; and the SSR shell was data-less, hardcoding `restaurant_name: ""`, `cart_line_count: 0`, `formatted_total: ""`, `payment_failed: false`. | `crates/web/src/router.rs`, `crates/web/src/renderer.rs` |
| **G6** ✅ **FIXED** (re-verified 2026-08-12) | `/orders/:id/confirmation` — tracking | **FIXED by [#420](https://github.com/TheCaptainCompany/captain-food/issues/420).** Production SSR builds `TrackingState::from_resolved`; `TrackingState::new(order_id)` survives only in tests and in the comments recording the fix (`router.rs:400-401`, `tracking.rs:77-78`). **As written**: `router.rs:247-251` built `TrackingState::new(order_id)` with `order: None`, so SSR rendered the **UNKNOWN / not-found hero for every order, forever** — the same root cause as G5, not a second bug. | same |
| **G7** ✅ **FIXED** (re-verified 2026-08-12 — this row was still being reported as open on 2026-08-12 and is not) | The live update | **FIXED by [#420](https://github.com/TheCaptainCompany/captain-food/issues/420) / [#424](https://github.com/TheCaptainCompany/captain-food/issues/424), on both sides.** Server: the subscription now also accepts **this order's own `DeliveryJob-` stream** (`crates/server/src/graphql/generated/subscription.rs:172-184`, learned lazily so no subscriber re-reads on every platform-wide job envelope) and dedupes on the row's **`updated_at`** rather than `status` (`:147-154`), with the reason in place — a status-keyed dedupe *"SWALLOWED exactly the movement the tracking screen exists to show"*. Client: `handwritten.rs::mount_tracking` is pull-THEN-push over the graphql-transport-ws client and re-pulls on every reconnect. **What remains unproven is end-to-end in a browser against a deployment** — which is what the recorded walk in [ADR-20260812-214021](../adr/ADR-20260812-214021-the-founder-answer-sheet-of-2026-08-12.md) Decision 2 exists to establish. **As written**: the subscription filtered `stream_name == "Order-<id>"` and deduped when `order.status` was unchanged, so the delivery-status movement wired by #424 never reached the screen. | `specs/ordering/api.yaml`, transport filter |
| **G7b** ⚠️ **STILL REAL** (re-verified 2026-08-12: no `/auth/callback` route anywhere in `crates/**` — the only OAuth callback is HubRise's — and no sign-in screen in `specs/screens/**`) | Restaurant / rider sign-in | **No browser sign-in exists for staff.** No login/magic-link screen in any `specs/screens/**`, no `/auth/callback` route. A RESTAURANT or RIDER JWT is obtainable only through the Supabase admin API. **Nobody can open the back office in a browser today.** | `specs/screens/*.yaml`, `crates/server/src/auth_routes.rs` |
| **G8** ⚠️ **STILL REAL, and now the LARGEST live row here** (re-verified 2026-08-12: `crates/application/src/ports.rs` is 140 lines declaring exactly four traits — `EventStore`, `GoogleOwnershipVerifier`, `GbpOrderLinkProbe`, `RestaurantRepository` — and **zero notification anything**; the design was settled meanwhile by [DECISIONS §31 BND-5](DECISIONS.md): policy → `order`, recipient contract → `restaurant`, transport → `platform`) | The restaurant is *told* | **Nobody is told.** No notification port of any kind in `crates/application/src/ports.rs`. `orderStatusChanged` is keyed per `orderId`, so a queue cannot subscribe to arrivals; `orders_queue` refreshes only on page load. **A paid order sits silently until someone reloads.** This is the domain lens's named worst failure mode, live on `main`. | `specs/screens/restaurant_backoffice.yaml:116-136`, `specs/ordering/api.yaml:289-297` |
...
| **C1** ⚠️ **HALF FIXED — the total is; the comparison is not, and it MOVED HOUSE** (re-verified 2026-08-12) | The cart's total and the competitor comparison | **The TOTAL is FIXED** by [ADR-20260810-112836](../adr/ADR-20260810-112836-cart-priced-live-on-read.md) / [PROP-20260810-231500](PROP-20260810-231500-cart-current-priced.md) Option B ([#451](https://github.com/TheCaptainCompany/captain-food/issues/451)): the cart projector is now a **pure, money-free fold** and the read side prices lines fresh on every read via `application::pricing::price_cart`. The `TODO(runtime)` cart accessors this row cited **no longer exist**. **The COMPARISON is still never computed**, and it is no longer a projector defect — it is `uber_comparison: None` in the read seam (`crates/server/src/graphql/cart_read.rs:187`: *"a policy read this seam does not perform yet"*), with the Order-side `uber_*` columns still `TODO(runtime)` (`crates/application/src/projectors/order_tracking.rs:95`). **So do not read this row as closed**: the number that anchors the commercial screen renders, the claim beside it does not. | `crates/server/src/graphql/cart_read.rs:187`, `crates/application/src/projectors/order_tracking.rs:95` (was `projectors/cart.rs:27-44`) |
| **C2** ⚠️ **STILL REAL** (re-verified 2026-08-12 at `crates/application/src/pricing.rs:105-113`) | The fee breakdown | Delivery, service fee, restaurant contribution, rider payout and platform net are **hard-zeroed** — the honest render today is *articles = total, livraison 0,00 €, frais de service 0,00 €*. | `crates/application/src/pricing.rs:105-113` (verified) |
...
> ✅ **2026-08-12 — the table above is CORRECTED IN PLACE, and the "why a green `main` did not notice"
> paragraph is now history rather than a live finding.** `renderer.rs` no longer skips `!screen.sdui`
> screens in either the hydrate path or the render-every-screen test (the rename and the removed
> `continue` are recorded at `renderer.rs:716-717`), so the two screens that were excluded from the
> only test that would have caught them are inside it. **G5, G6 and G7 are fixed** (#420 + #451 +
> #424); **C1 is half fixed** — the total prices live on read, the competitor comparison still never
> computes and has moved from the projector to the read seam; **G7b, G8 and C2 are live**, and **G8 is
> the largest**, being *nobody is told about a paid order*. Corrected under the
> living-document rule ([ADR-20260801-020000](../adr/ADR-20260801-020000-proposals-are-living-documents.md)):
> every cell says what is fixed, by which change, and what survives — because each of these rows was
> TRUE when written, the cited lines still exist in some form, and only re-reading the code separates a
> fixed defect from a live one. Record:
> [ADR-20260812-214021](../adr/ADR-20260812-214021-the-founder-answer-sheet-of-2026-08-12.md) §5.2.
