...
5. **Divergence policy and the disclosure UI (slice 4).** `CheckoutRepriced`, the min rule, the band, UC2/UC3/UC5/UC6 — ux's three no-price states (§4 UC1 note below) ride here, not 3a/3b/4. Negative test for F4: HEAD above as-of by more than the band never produces a charge above the quote. Negative test for F6: expiry with zero delta produces a placed order and **no** customer-visible reprice.
