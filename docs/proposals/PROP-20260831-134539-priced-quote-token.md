...
- **Register rows**: `QUOTE-STALENESS` (open — **priced** in §7, not closed here) · `CAPTAINNET-ZERO` (open, founder-owned — §6 D6's *alternative* resolves into it) · `ERASURE-LAUNCH-GATE` (decided 2026-08-29 — §3 F5 exists to keep it intact)
- **Related**: [#817 "Checkout button states a price, not the obligation to pay (L221-14 / CRD Art. 8(2)) — sanction is the consumer is not bound"](https://github.com/TheCaptainCompany/captain-food/issues/817) — same screen, re-armed by this work, **referenced not absorbed** (§4 UC4) · [PROP-20260810-231500](PROP-20260810-231500-cart-current-priced.md) (the option space ADR-20260810-112836 decided) · [PROP-20260815-142349](PROP-20260815-142349-actor-answers-block-and-the-ask-step.md) §18/D2 (the envelope-vs-payload tension, named in §6 D1) · [PROP-20260829-150752](PROP-20260829-150752-customer-erasure.md) §3.4 (crypto-shred, the split F5 relies on) · `docs/legal/BRIEF-20260818-counsel-packet-and-self-answer-triage.md` §2 and §5
- **History**: this file is LIVING (ADR-20260801-020000) — `git log -p` on it, never appended superseded blocks.

- **Concerns**:
  - [ ] `legal`-lens return not attached: the blockers **B1–B5** and counsel questions **QT-1…QT-10** were named in the dispatch that commissioned this proposal, but their text was not handed to it and resolves nowhere in the repo (`grep -rn "QT-1\|QT-4\|QT-10" docs/ specs/` returns **0 hits**, verified 2026-08-31). §8 reconstructs the legal map from **primary repo records only**, under this proposal's own `L1–L7` labels. The lens's own return must be attached, and its numbering reconciled with §8, before `Approved`.
  - [ ] `QUOTE-STALENESS` N is not decided here: §7 recommends **30 minutes as a backstop only** with its derivation, but the instrument that would let N be re-derived from evidence rather than judgement does not exist yet (§9 C1). The row stays **open**.
  - [ ] `PlaceOrder` input change is **non-additive**: §6 D4 makes `quote` required and retires `expectedTotal`. That is a breaking GraphQL input change on a shipped money mutation — posture **`HOLD: human`**, and the client-rollout order in §11 must be reviewed before it lands.
  - [ ] The as-of catalog fold has **no implementation anywhere today** (`grep -rn "as_of\|asOf" crates/ --include=*.rs` returns 0 hits on any catalog path, verified 2026-08-31). §6 D2's capability split is the design; its cost at peak (§12) is a projection, not a measurement.
...
## 7. The staleness policy — a recommendation, priced, not decided

Register row **`QUOTE-STALENESS`** stays **open**. What follows is the team's pricing of it, per that row's own instruction that it be *"priced rather than re-asked"*.

**Recommendation: N = 30 minutes, as a backstop only. M (a version count) is dropped.**
...
**A gap this proposal must declare rather than paper over.** The `legal` lens's blockers **B1–B5** and counsel questions **QT-1…QT-10** were named in the dispatch that commissioned this proposal. Their text was **not handed to this proposal** and resolves nowhere in the repository: `grep -rn "QT-1\|QT-4\|QT-10" docs/ specs/` returns **0 hits** (2026-08-31). Reproducing ten numbered questions from memory would be inventing a lens return, so §8 is reconstructed from **primary repo records only**, under this proposal's own `L1–L7` labels. Attaching the lens's return and reconciling the numbering is an unchecked **Concern** in the header, which mechanically blocks `Approved`.
...
1. **N** — the backstop, 30 minutes recommended (§7). Row `QUOTE-STALENESS`, **open**, owner `team`. Recommendation: adopt 30 as a backstop and re-derive from C1 after the first peak.
...
3. **`legal`'s B1–B5 / QT-1…QT-10** — not handed to this proposal and absent from the repo (§8). A blocking Concern.
...
- `docs/decisions/QUOTE-TOKEN.yaml` (decided) · `docs/decisions/QUOTE-STALENESS.yaml` (open) · `docs/decisions/CAPTAINNET-ZERO.yaml` (open) · `docs/decisions/ERASURE-LAUNCH-GATE.yaml` (decided)
