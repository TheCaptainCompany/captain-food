...
- **Realized by**: slice 1 (HEAD orderability at checkout) — [PR #824](https://github.com/TheCaptainCompany/captain-food/pull/824) (`require_orderable_line` on the `PlaceOrder` path, rule `CheckoutPricesCartCreatesPaymentIntent` extended, the three behaviour tests); slices 2–6 _(filled at completion)_
