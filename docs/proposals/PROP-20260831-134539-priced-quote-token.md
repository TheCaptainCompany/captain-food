...
| The event log already carries the coordinate this design needs | `specs/database/tables/eventstore.yaml:17` `version` — the spec note there still says 0-based and is STALE: the writer is 1-based (`event_store.rs` `expected_version + index + 1`), the correction was decided by ADR-20260808-171056 and has not landed ([#921](https://github.com/TheCaptainCompany/captain-food/issues/921) item 1); the coordinate follows the code + `:28` `unique: [stream_name, version]`; the catalog's stream is `Catalog-{catalogId}` (`specs/catalog/actors.yaml:8-10`) |
