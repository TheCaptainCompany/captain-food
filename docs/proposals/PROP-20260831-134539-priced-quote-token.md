...
- **Realized by**: slice 1 (HEAD orderability at checkout) — [PR #824](https://github.com/TheCaptainCompany/captain-food/pull/824) (`require_orderable_line` on the `PlaceOrder` path, rule `CheckoutPricesCartCreatesPaymentIntent` extended, the three behaviour tests); slice 2 (the as-of capability) — [PR #920](https://github.com/TheCaptainCompany/captain-food/pull/920) (`AsOfCatalog`/`AsOfPriceAuthority`, dark — no caller, no checkout wiring); slices 3–6 _(filled at completion)_
...
- **Glossary** (added at slice 2, [PR #920](https://github.com/TheCaptainCompany/captain-food/pull/920) — the word enters the codebase here, `specs/**` is deliberately NOT where it lands; see §11 slice 2): **as-of** — a read reconstructed at a FIXED past coordinate on an event stream, never the live head (the camelCase `asOf` spelling is dropped from the glossary at round 2; the sequence diagrams' `asOfTotal` and the token table's `asOf` are diagram and payload identifiers, not the term); **`AsOfCatalog`** — the Catalog aggregate's price-only as-of capability (`crates/domain/src/catalog_as_of.rs`), built by re-folding the same `catalog::apply` the write path uses and truncating at the coordinate; **`CatalogVersion`** — the coordinate itself (round 2), the 1-based `domain_events.version` verbatim (ADR-20260808-171056), never a port-invented 0-based convention and never a slice index.
...
- **Deliberate**: the as-of fold must be **price-only**. `price_cart` resolves price *and existence* in one lookup (`crates/application/src/pricing.rs:57-59`). **Slice 1 ([PR #824](https://github.com/TheCaptainCompany/captain-food/pull/824)) closed the gap this bullet originally warned about**: `require_orderable_line` now runs a third time at checkout too (`crates/application/src/commands.rs:2851`, rule `CheckoutPricesCartCreatesPaymentIntent`), so the pricer's lookup is no longer the *only* reality check on the path. The fold still must be price-only regardless — an as-of fold that could ALSO answer availability would let a future caller skip the HEAD re-check that now exists, silently re-opening the same oversell hole from the other side.
...
| **STALE as of [PR #824](https://github.com/TheCaptainCompany/captain-food/pull/824), 2026-09-06.** The oversell guard now runs at checkout | `require_orderable_line` defined `crates/application/src/commands.rs:810`, called at `:938`/`:967` (`AddCartLine`/`ChangeCartLineQuantity`) **and now also at `:2851`** on the `PlaceOrder` path (rule `CheckoutPricesCartCreatesPaymentIntent`) |
| The checkout-time reality check is therefore TWO lookups, not one: the pricer's `offer_by_id` (price/existence) and `require_orderable_line` (availability/stock/options) | `crates/application/src/pricing.rs:57-59`; `crates/application/src/commands.rs:2851` |
...
| The event log already carries the coordinate this design needs | `specs/database/tables/eventstore.yaml:17` `version` — the note now says 1-based, correcting the note this row previously flagged as stale ([#921](https://github.com/TheCaptainCompany/captain-food/issues/921) item 1, ADR-20260808-171056); the writer is 1-based (`event_store.rs` `expected_version + index + 1`), the note matches it; the coordinate follows the code + `:28` `unique: [stream_name, version]`; the catalog's stream is `Catalog-{catalogId}` (`specs/catalog/actors.yaml:8-10`) |
...
| **STALE as of [PR #920](https://github.com/TheCaptainCompany/captain-food/pull/920), 2026-09-06.** An as-of fold now exists — dark, no caller | `crates/domain/src/catalog_as_of.rs` (`AsOfCatalog::from_stream`/`price_of`), `crates/application/src/ports.rs` (`AsOfPriceAuthority`), `crates/infrastructure/src/persistence/catalog_as_of.rs` (`PgAsOfCatalogRepository`) |
...
**Slice 3a status (ux, D1/D3):** as of [ADR-20260906-154419](../adr/ADR-20260906-154419-the-priced-read-is-served-from-the-fold-and-carries-its-coordinate-behind-a-door.md), the query above is one release AHEAD of the DSL — there is no `quote` field yet (that is slice 3b's). What exists today, behind a door: `cart.current`/`cart`/`carts` MAY price from the event-stream fold instead of the projection, and the coordinate that fold was bounded at is server-carried and DROPPED before the reply is built — no field, no render, no timer, exactly this mockup's "not rendered" note, just for one more release. `cart.current` is a `data_requirement` of exactly this screen and `/checkout` (`specs/screens/restaurant_frontoffice.yaml:406,467`) — never the menu screen, never the ETA slot — so the mint (when the door is open) is one coordinate per priced read, on both screens — the /cart→/checkout seam (two reads, two mints, two moments) is recorded OPEN for 3b/4. ux's three no-price states — binding per register row item (9) (rendered before the flip) and detailed at item 5 (slice 4) in §11 (copied verbatim from the ADR's D7, not invented here): no number on the total row/CTA; the pay button not pressable; a timeout with explicit no-charge reassurance — never a stale total, never a zero, never price-change wording for a defect. This owes the screens spec no new component; the ONE prose line is at the binding table (`restaurant_frontoffice.yaml:72`, where `cart.current` is annotated "priced LIVE at read"), recording that the coordinate is server-carried and deliberately unrendered — never a `gaps:` entry, since nothing is missing from the customer's point of view.
...
**Three cheap-now gaps, named by `legal` at the slice 2 briefing (2026-09-06), none blocking, none resolved here:**
1. **`TaxRate` is a per-service-mode OBJECT** (`specs/common/entities.yaml:74` — `delivery` required, `collection`/`eatIn` nullable), never a scalar: a scalar `tax_rate` would silently pick one mode and collapse the French 10% / 5.5% / 20% split a single order can legitimately span. `AsOfCatalog::price_of` therefore returns the whole folded object. Which mode applies to a given order, and the null-fallback to the account's `defaultTaxRate` (`specs/network/entities.yaml:103` — a DIFFERENT stream this coordinate does not pin), stay open questions for the slice that actually selects a mode.
2. **Only `Product` carries `taxRate`; `ProductItemOption` does not.** An alcohol option therefore inherits its parent product's food rate under this design. "One rate per priced line" must not be typed as an invariant anywhere downstream — option-level rates are not foreclosed, just not built.
3. **The delivery fee's own VAT is outside this coordinate entirely.** The quote prices catalog lines; whatever tax treatment the delivery fee itself needs is a separate subject with its own rate source, not something `AsOfCatalog` can or should answer.

### D4 — Optionality on the command

| Option | Pros | Cons |
|---|---|---|
| **`quote` required, `expectedTotal` removed, in one change** (original recommendation) | Omission becomes a structural GraphQL rejection before any handler runs — no new error code needed · exact precedent on this command: `customerId` under #144 (`specs/ordering/commands.yaml:116-123`) · leaves no dead optional field for the next reader to mistake for enforcement | **Non-additive GraphQL input change** on a shipped money mutation → `HOLD: human` — and an env-gated door **cannot un-remove an already-shipped SDL field**: five lenses (farley, graphql, reviewer, beck, business) independently found this un-gateable at the 3b+4 briefing (2026-09-06) |
| **`quote` nullable now, `expectedTotal` kept nullable + `deprecated:` + ignored, requiredness enforced by the WRITE DOOR at the handler — expand/contract** ✅ **decided** | `ADR-0043`'s expand/contract discipline, extended to a shipped GraphQL input for the first time — a rollback is a clean flag flip, never a redeploy; the checkpoint metric `place_order_quote_present_total{present}` makes the non-null flip evidence-gated rather than a guess; `expectedTotal`'s own deletion becomes a separate, later, cheaply-reviewed change | Two nullable fields live in the SDL for a while, one of them dead weight; needs the small `deprecated:` emitter key (in scope) |
| `quote` required, `expectedTotal` kept nullable, no door | Marginally gentler rollout than the original | Preserves the #816 fossil: a field the spec describes as a guarantee and nothing populates. That is the defect, not a mitigation |
| `quote` nullable, enforced by a domain rule with no door | Additive, no client coordination | This is precisely how #816 happened. A guarantee carried by an optional field is carried by convention |

**DECIDED by [ADR-20260906-192007](../adr/ADR-20260906-192007-slice-3b-and-the-command-change-land-as-expand-contract-behind-an-interlocked-write-door-with-the-refusal-set-enumerated.md) (2026-09-06, team consent, fourteen-lens briefing, every lens CONCERN):** the original recommendation is **superseded** by the expand/contract row above. `quote` is nullable in the SDL now; the **write door** `RUN_QUOTE_REQUIRED_ON_PLACE_ORDER` decides whether an absent `quote` is refused — CLOSED (default) = `quote` structurally accepted and never read, every order priced and charged at HEAD (today's behaviour, verbatim); OPEN = absent `quote` refused at the handler (typed, never a GraphQL non-null violation), present `quote` verified, one refusal on mismatch (D-D of that record enumerates the refusal set — it is not one undifferentiated error). The write door is **interlocked** with 3a's read door `RUN_FOLD_PRICED_CART_READ` (that record's D-B): it may only be `true` when the read door is also `true`, refused at startup otherwise. `expectedTotal` stays in the SDL, nullable, `deprecated:` (a new emitter key, D-C), ignored by the server — its **deletion** is deferred to a separate, later recorded change (the contract half), never bundled with `quote`'s requiredness flip. The non-null flip on `quote` is itself a third, later recorded step, gated on `place_order_quote_present_total{present}` reaching 100% and a cache-drain condition. **"Required" throughout this proposal, wherever a shipped instance is described, therefore means required BY THE DOOR, never by the SDL** — the phrase in item 4 of §11 below is read under this correction.
...
Items are numbered 1–6; "slice 3a"/"slice 3b" are design names of items 3 and 4; "slice 4" in older
records (ADR-20260831-165146, journal) means item 5.
...
2. **The as-of capability.** `AsOfCatalog` + `OfferPrice` (named `OfferPrice`, not `PricedOffer` — `Offer` is a catalog entity, this is a line price at a coordinate) with a required `tax_rate`. Test: folding `Catalog-{id}` to version V reproduces the price at V after arbitrary later events, including `OfferStockUpdated`. Negative: `OfferPrice` exposes no availability accessor — enforced by the type (a `compile_fail` doctest, paired with an identical passing doctest so the negative itself is proven to compile under the RIGHT conditions).
   **Done-when, added at approval (the re-expressed third `Concerns` entry): DISCHARGED.** **REALIZED by [PR #920](https://github.com/TheCaptainCompany/captain-food/pull/920), 2026-09-06, round 2** — `crates/domain/src/catalog_as_of.rs`, `crates/application/src/ports.rs#/AsOfPriceAuthority`, `crates/infrastructure/src/persistence/catalog_as_of.rs`. DARK: no production caller, no cache, no checkout wiring (mob consent — the one-pricer property is protected by an equivalence test instead of a caller). **Round 1's presentation pass found the coordinate itself unsound** (vernon B1/B2, young B1, evans B1): a bare `i64` port coordinate with a `+1` "0-based convention" at the SQL boundary, and a domain-level truncation that filtered by SLICE INDEX rather than by each event's own stream version — inert at the only production call site. **Round 2 fixes it**: `CatalogVersion` is now the ONE spelling of the coordinate everywhere on this path — the 1-based `domain_events.version` verbatim (ADR-20260808-171056), the same number `EventStore::append` returns — `AsOfCatalog::from_stream` filters by each event's OWN version (never a slice position), and the adapter fails CLOSED when the highest row version returned does not equal the requested coordinate (a coordinate beyond head is an `Err`, never a HEAD price). The fold's cost is **measured, not projected** — §12's first bullet is rewritten below to the round-2 measurement, now covering the READ PRODUCTION ACTUALLY PERFORMS (V = L, not only the V = 200 mutant-detector arm).
   **RESOLVED by slice 3a (below): option (b).** The coordinate is minted from the write-side-shaped fold on the READ side too — `AsOfPriceAuthority::at_head`, ONE unbounded range read to head returning prices and the coordinate as one value, never a projected-version column on `Catalog`. [ADR-20260906-154419](../adr/ADR-20260906-154419-the-priced-read-is-served-from-the-fold-and-carries-its-coordinate-behind-a-door.md) is the deciding record; **(a)** stays on the record as the rejected alternative (the rebuild-window failure); **(c)**'s `processmanager.yaml:68` repair landed in slice 3a's own commit (prose only, the note now states the guarantee narrows while the door is open — the FULL repair, making the WRITE side also fold, is still slice 4's, per `rules.yaml:60-65`).
3. **Slice 3a — mint on the priced read, behind a door, no verify, no signing (REALIZED by [PR #922](https://github.com/TheCaptainCompany/captain-food/pull/922), 2026-09-06, [ADR-20260906-154419](../adr/ADR-20260906-154419-the-priced-read-is-served-from-the-fold-and-carries-its-coordinate-behind-a-door.md)).** `cart.current`/`cart`/`carts` price from `AsOfPriceAuthority::at_head` when `RUN_FOLD_PRICED_CART_READ` is ON (default OFF everywhere); the coordinate is server-carried and dropped at the reply boundary — no field, no input. Tests: repricing at the returned coordinate reproduces the returned price; the mint is exactly one catalog read; the closed arm never touches the fold; a fold failure is `technical_error`, never a HEAD/projection fallback. **3a alone is INVENTORY** (holub): a coordinate minted and dropped commits nobody to anything — it closes NOTHING on #816 by itself.
4. **Slice 3b + the command change — the customer is charged the price they were shown, or told why it changed, in words (next, same lane, nothing else opens between — holub's WIP condition). DECIDED as ONE deliverable by [ADR-20260906-192007](../adr/ADR-20260906-192007-slice-3b-and-the-command-change-land-as-expand-contract-behind-an-interlocked-write-door-with-the-refusal-set-enumerated.md) (2026-09-06, team consent, fourteen-lens briefing).** 3b and the command change ship as **ONE deliverable, in ONE PR** (mint, sign, the command requiring the quote behind its own write door, one enumerated refusal set): a signed quote nobody is held to is a second dark release by the same argument this item uses to call 3a inventory. `QuoteMinter` / `QuoteVerifier` ports + the signed envelope; the ONE published word `quote`, opaque and nullable, `api.yaml` `types:` only (D3), with the codegen test that `CatalogVersion` appears in zero `input` blocks **and zero field arguments** of the generated SDL, and ux's binding-table prose line at `restaurant_frontoffice.yaml:72` (the /cart→/checkout binding: the quote submitted by `place_order` is the quote of the read that painted the recap total on `/checkout`, never a quote stashed at `/cart` — the seam itself, two reads/two mints/two moments, stays recorded OPEN, [#930](https://github.com/TheCaptainCompany/captain-food/issues/930) item 17). Tests: a tampered token rejects; a token minted for cart A rejects on cart B; a token signed with a retired key id rejects (two arms: unknown keyId, and a retired key whose material still produces a valid MAC); round-trip is stable across a process restart; the write door refuses at startup when open without the read door also open (D-B); a `required: false` `quote.verify` span with an unchanged success rule is refused by its own contract test (D-I). **The walk runs inside this PR**, home = the DB-gated `crates/server/tests/quote_walk.rs` beside `rider_standing_walk.rs`/`platform_admin_walk.rs` (`tools/walk/` is NOT on `main` — corrected here and on `QUOTE-MINT-PRECONDITIONS` item (3)), door forced ON in the lab: one cart, one signed quote, one tampered token, one refusal screen, a transcript in the PR body. **The client renders the refusal**: `order_tracking` gains `operationStatus.byMessage` + one refusal `conditional_section`, a terminal path on `REJECTED` in `crates/web/src/tracking.rs` — without it a refused order shows false reassurance forever. Inherits [#921 "Priced quote token slice 2 follow-ups"](https://github.com/TheCaptainCompany/captain-food/issues/921) item 5 (an option deleted at HEAD still has an as-of price line, owing a render rule; `price_of` returns one `None` for two causes, so refusal copy must not inherit the conflation).
   **The command change.** `quote` structurally accepted and never read while the write door is closed (today's behaviour, priced and charged at HEAD); `expectedTotal` kept, nullable, `deprecated:` and ignored (D4, corrected). Client ships the field **before** the server requires it — that ordering is now the door's own checkpoint (`place_order_quote_present_total{present}`), never a same-change field swap. Story step + behaviour test per ADR-0032.
   **The signing key** is a declared `configuration.yaml` secret (`secret: true`, `required: [staging, production]`, no default, `EMAIL_QUOTA_KEY_HMAC_SECRET`-shaped) with a module-private `SigningKey` newtype; `QuoteVerifier` holds a key SET, `QuoteMinter` holds one current key; a **two-key overlap of 60 minutes** (30-minute `QUOTE-STALENESS` backstop + skew) is a precondition of THIS deliverable, not of a later rotation.
   **The bounded-read deadline arm** — a `statement_timeout` on a dedicated fold connection pool, never pool-wide, never a transaction wrapping the fold — **lands in this deliverable** (register item (8), amended), because 3b doubles the number of unbounded folds per checkout (mint + verify) with no closed arm left once `expectedTotal`'s enforcement is retired.
   **Gate, added at approval (the re-expressed second `Concerns` entry):** this slice is **`HOLD: human`** and **blocks on it** — a legal surface and a write-side door on a shipped money mutation. It stops at ready-for-review for the TEAM's independent reviewer pass ([ADR-20260815-134655](../adr/ADR-20260815-134655-the-team-merges-its-own-work-no-pr-waits-on-founder-review.md): no PR waits on founder review), and the **client-before-server rollout order stated above must be reviewed in that pass before this lands**. Approval of this proposal does not discharge it.
   **Status: REALIZED by [PR #933](https://github.com/TheCaptainCompany/captain-food/pull/933), 2026-09-07 — DARK.** The write door opens once, after item 5 below lands and the `QUOTE-MINT-PRECONDITIONS` row discharges; this deliverable ships with `RUN_QUOTE_REQUIRED_ON_PLACE_ORDER` `false` everywhere, exactly today's charge-at-HEAD behaviour. As landed, and stated here as **CURRENT design**: verify runs inside `application::commands::place_order`'s pre-payment guard block, strictly before the Stripe `PaymentIntent` call, folding `as_of(catalogId, V)` at the quote's own coordinate — never `at_head` (D-F); on success it returns the fold's **whole** `PricedCart` — items, breakdown and total, one coordinate — which `place_order` freezes as `OrderPlaced`'s lines, so the stored event's lines always sum to the charge; the fold's recompute is then compared to the token's own `total_cents` and a mismatch refuses **structurally** (`TotalMismatch`), closing the gap D-E above left open when this proposal was first written. The client sends the quote: checkout's `place_order` action carries `quote: '{{ cart.quote }}'` from the **same** `cart.current` read that painted the `/checkout` recap total (the binding sentence at `restaurant_frontoffice.yaml:72`, never a quote stashed at `/cart`), and drops `expectedTotal` from the wire entirely. Two corrections against the text above: the signing key's manifest promotion to `required: [staging, production]` was landed then **reverted** to `required: []` (checkpoint 2 — the door's own boot refusal already covers the door-OPEN case, so the manifest requirement adds nothing while the door is closed everywhere and only wedges deploy; the promotion now lands WITH the flip, `QUOTE-MINT-PRECONDITIONS` item (28)); `errors.yaml#/QuoteRequired`, named nowhere as a catalogued error, never existed — the absent-quote cause folds into `QuoteVerificationFailed`'s structural set. `QuoteNoLongerHonoured` stays DECLARED and Rust-pinned but DSL-unwired (no `throws:`) pending the closure option space recorded in the [ADR-20260906-192007](../adr/ADR-20260906-192007-slice-3b-and-the-command-change-land-as-expand-contract-behind-an-interlocked-write-door-with-the-refusal-set-enumerated.md) D-D amendment. §6 D4 above is unchanged and reflects this landed shape without correction needed.
5. **Divergence policy and the disclosure UI (slice 4).** `CheckoutRepriced`, the min rule, the band, UC2/UC3/UC5/UC6 — ux's three no-price states (§4 UC1 note below; binding per register row item (9)) — they ride in item 5 (slice 4), not in items 3 or 4. Negative test for F4: HEAD above as-of by more than the band never produces a charge above the quote. Negative test for F6: expiry with zero delta produces a placed order and **no** customer-visible reprice.
6. **The four contracts** (§9), with the metrics proved firing by a spy, in the `checkout_degraded_metric.rs` style `ADR-20260810-112836` §Follow-up already established. `catalog.as_of.fold`/`catalog_as_of_fold_ms`/`catalog_as_of_stream_length`/`catalog_as_of_payload_bytes`/`catalog_as_of_reads_total{outcome}` landed EARLY, in slice 3a (D5) — they gate the door's flip preconditions (`QUOTE-MINT-PRECONDITIONS`), not item 5's checkout-side contracts, which remain that item's own four.

Alongside, in the same deliverable as item 4 (3b + the command change): **`specs/ordering/rules.yaml:61-65` stops claiming the `expectedTotal` check is the enforcement.** Leaving that sentence in place after the check is deleted would be worse than the original defect.

---

## 12. Drawbacks — why we might regret the whole thing

- **Two catalog resolutions per checkout.** As-of plus HEAD. The as-of fold is a stream fold, not a projection read, so its cost grows with the catalog's stream length and there are no snapshots (a consciously accepted V0 trade-off). At Friday 20:30 this sits on the money path. Mitigations exist and are unproven: the shared `SNAP-1` primitive, and folding only the offers the cart references. **RE-MEASURED 2026-09-06 (slice 2 round 2, [PR #920](https://github.com/TheCaptainCompany/captain-food/pull/920)), not projected — round 1's number is SUPERSEDED, not merely refined.** Round 1 measured only V=200 of L=2,000 identical stock rows and called it "the cost"; `business`/`dba` caught at the round-1 presentation pass that a checkout actually reads ~L rows dominated by whole-menu content, not 200 — V=200 alone was a mutant detector wearing a cost-number costume. Round 2 measures BOTH arms, lab-measured on this container, DB-gated, L=2,000 events (`UNVERIFIED input` — "the largest realistic HubRise import (~500 products) plus a full resync plus a Friday's worth of stock syncs", a judgement call, not a measurement):
  - **Arm (a), V=200 (the mutant detector; rows 1–200 are `CatalogCreated` + `ProductAdded`, not stock rows — dba, round 2):** 10 iterations — `sql median=4.4ms decode median=4.3ms fold median=0.6ms end_to_end median=9.5-10.1ms max_of_10=10-12ms`, 102,296 payload bytes.
  - **Arm (b), V=L=head (the READ PRODUCTION ACTUALLY PERFORMS, import-shaped mix: 500 distinct `ProductAdded` + one `CatalogImported` of another 500 + the remainder cheap `OfferStockUpdated`):** 10 iterations — `sql median=33-36ms decode median=34-36ms fold median=17ms end_to_end median=85-90ms max_of_10=88-140ms`, 759,558 payload bytes.

  Peak-unverified both arms: no production Tours catalog stream length has been observed. Against `business`'s stop numbers (>150ms end-to-end max escalate before slice 3, >50ms ship dark with a named mitigation): arm (b)'s end-to-end max, observed 88-140ms across repeated runs at authoring, sits in the **SHIP-DARK band** — never the escalate band, but never negligible either. **Named mitigation, per this same drawback's own text: the capability ships DARK this slice (no caller, no checkout wiring)**, so the ~85-90ms median costs nothing in production today; slice 3+ must re-measure against the SNAP-1 primitive or the narrow-fold mitigation named above BEFORE wiring a live caller, not after — a dark capability crossing into live traffic at this cost, unmitigated, would sit on the Friday-peak money path at a cost order of magnitude above arm (a)'s. **Nothing here blocks slice 3 starting** (the mint/verify port work is orthogonal to this cost), but slice 3/4's checkout wiring inherits this drawback's mitigation obligation, not a clean bill.

  **Slice 3a's D6: the first LIVE caller (behind a door) gets a phase-0 BUDGET, not a verdict.** Now that `AsOfPriceAuthority::at_head` has a real caller (`cart.current` behind `RUN_FOLD_PRICED_CART_READ`, default OFF), the as-of leg is recorded as a HEADROOM CARVE inside `cart-price`'s existing p95 300 / p99 600 (`specs/observability.yaml`, INITIAL) — leg ≤ 150 ms p95, ≤ 250 ms p99 (`docs/decisions/QUOTE-MINT-PRECONDITIONS.yaml`; 150 ms = this section's own arm-(b)/(c) escalate line, ALL `UNVERIFIED input`, lab, one container, peak-unverified). Arm (c)'s NATIVE `at_head` measurement (3a, same L=2,000 fixture): 10 iterations — `sql median=33ms decode median=34ms fold median=16ms`, native median **113.8 ms** (max of 10 = 116.6 ms) BEFORE the lever, **89.6 ms** AFTER it (executor hand-back, round 2, same L=2,000 fixture, 10 iterations, lab, one container, `UNVERIFIED input`, not re-run in CI), 759,558 payload bytes — inside the ship-dark band, same order of magnitude as arm (b) (`at_head` reads every row of the same stream `as_of(V=L)` does). The lever that closed that gap — `payload_bytes` read from Postgres (`sum(octet_length(payload::text))` in the same SELECT) rather than re-serialized per row via `serde_json::to_vec` inside `load_to_head` (`crates/infrastructure/src/persistence/catalog_as_of.rs`) — is now LANDED, in slice 3a's own round-2 commit, moving the next lever, **sampled**, to the head of the list (business: `payload::text` still renders jsonb per row INSIDE Postgres, on the pool-of-5 tier, so the ~28 ms this lever removed may have MOVED rather than gone — sampled is the preferred end state, and a re-measure is owed at 3b with the contract rows live) > content-hash import suppression (#921 item 2) > narrow fold > refuse; SNAP-1 last. Composing the MEDIAN with the p95 TARGET, re-derived against 89.6 ms rather than the superseded 113.8 ms: 89.6 ms is +5% over arm (b)'s 85.6 ms end-to-end median (not the old +33%), the max-drift-to-140-ms antecedent (#920) stands unchanged, and the crossing claim is now about the p95 OF A DISTRIBUTION THAT DOES NOT EXIST YET — production suspended, ADR-20260817-105844 — never about the median, which sits comfortably under the 150 ms escalate line. The benchmark's SQL-leg arm still times `fetch_all_rows`, which has had no production caller since this lever landed — the arm measuring the query production actually performs is owed at 3b. Holub's cut, recorded as the design rule: "observed production L" is NOT a blocking precondition while production is suspended — the CONTRACT ROW is (D5's `catalog_as_of_stream_length`/`catalog_as_of_payload_bytes`); the max-L NUMBER stays deferred, the BEHAVIOUR above it is decided now (refuse, `technical_error`, never HEAD). The register row also names dba's preconditions (content-hash import suppression landed before the door opens on `/cart`, the buffer-cache instrument) as flip gates, separate from this budget. **The budget above is a PER-READ number**: `carts` renders up to 50 of these reads serially per request on a pool of 5, so the door may not open on `carts` until that fan-out is bounded or `carts` is excluded from the open arm (register row item (7)) — the phase-0 budget has no per-request multiplier baked in.
- **Catalog stream history is now load-bearing for money** (young): once a caller reads `Catalog-<id>` as-of a past version (slice 3 onward), no compaction, truncation or archival of `Catalog-*` streams may ship without a recorded decision — doing so would make some already-quoted, in-flight coordinate unresolvable mid-checkout.
...
5. ~~**Key management** for D1 option 1 — where the signing key lives, how it rotates, and what a rotation does to tokens in flight (the `keyId` field is the hook; the policy is not designed here).~~ **RESOLVED 2026-09-06**: [ADR-20260906-192007](../adr/ADR-20260906-192007-slice-3b-and-the-command-change-land-as-expand-contract-behind-an-interlocked-write-door-with-the-refusal-set-enumerated.md) D-H — the key is a declared `configuration.yaml` secret (`secret: true`, `required: [staging, production]`, no default, `EMAIL_QUOTA_KEY_HMAC_SECRET`-shaped, a module-private `SigningKey` newtype); `QuoteVerifier` holds a key SET, `QuoteMinter` holds one current key; rotation requires a two-key overlap of 60 minutes (30-minute `QUOTE-STALENESS` backstop + skew), a precondition of the 3b+4 deliverable itself, not of a later rotation event; custody is admin-gated, the founder's external list.
