...
4. **Slice 3b — the customer is charged the price they were shown, or told why it changed, in words (next, same lane, nothing else opens between — holub's WIP condition).** 3b and the command change below ship as **ONE deliverable, in ONE PR** (mint, sign, the command requiring the quote, one refusal message): a signed quote nobody is held to is a second dark release by the same argument this item uses to call 3a inventory. `QuoteMinter` / `QuoteVerifier` ports + the signed envelope; the ONE published word `quote`, opaque and nullable, `api.yaml` `types:` only (D3), with the codegen test that `CatalogVersion` appears in zero `input` blocks and ux's binding-table prose line. Tests: a tampered token rejects; a token minted for cart A rejects on cart B; a token signed with a retired key id rejects; round-trip is stable across a process restart. Inherits [#921 "Priced quote token slice 2 follow-ups"](https://github.com/TheCaptainCompany/captain-food/issues/921) item 5 (an option deleted at HEAD still has an as-of price line, owing a render rule; `price_of` returns one `None` for two causes, so refusal copy must not inherit the conflation).
   **The command change.** `quote` required, `expectedTotal` removed (D4). Client ships the field **before** the server requires it — that ordering is the whole rollout risk. Story step + behaviour test per ADR-0032.
