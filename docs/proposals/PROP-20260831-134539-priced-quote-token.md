...
3. **Slice 3a — mint on the priced read, behind a door, no verify, no signing (REALIZED by [PR #922](https://github.com/TheCaptainCompany/captain-food/pull/922), 2026-09-06, [ADR-20260906-154419](../adr/ADR-20260906-154419-the-priced-read-is-served-from-the-fold-and-carries-its-coordinate-behind-a-door.md)).** `cart.current`/`cart`/`carts` price from `AsOfPriceAuthority::at_head` when `RUN_FOLD_PRICED_CART_READ` is ON (default OFF everywhere); the coordinate is server-carried and dropped at the reply boundary — no field, no input. Tests: repricing at the returned coordinate reproduces the returned price; the mint is exactly one catalog read; the closed arm never touches the fold; a fold failure is `technical_error`, never a HEAD/projection fallback. **3a alone is INVENTORY** (holub): a coordinate minted and dropped commits nobody to anything — it closes NOTHING on #816 by itself.
   **Slice 3b — the opaque quote, signed and verified (next, same lane, nothing else opens between — holub's WIP condition).** `QuoteMinter` / `QuoteVerifier` ports + the signed envelope; the ONE published word `quote`, opaque and nullable, `api.yaml` `types:` only (D3), with the codegen test that `CatalogVersion` appears in zero `input` blocks and ux's binding-table prose line. Tests: a tampered token rejects; a token minted for cart A rejects on cart B; a token signed with a retired key id rejects; round-trip is stable across a process restart.
...
5. **Divergence policy and the disclosure UI (slice 4).** `CheckoutRepriced`, the min rule, the band, UC2/UC3/UC5/UC6 — ux's three no-price states (§4 UC1 note below) ride here, not 3a/3b. Negative test for F4: HEAD above as-of by more than the band never produces a charge above the quote. Negative test for F6: expiry with zero delta produces a placed order and **no** customer-visible reprice.
6. **The four contracts** (§9), with the metrics proved firing by a spy, in the `checkout_degraded_metric.rs` style `ADR-20260810-112836` §Follow-up already established. `catalog.as_of.fold`/`catalog_as_of_fold_ms`/`catalog_as_of_stream_length`/`catalog_as_of_payload_bytes`/`catalog_as_of_reads_total{outcome}` landed EARLY, in slice 3a (D5) — they gate the door's flip preconditions (`QUOTE-MINT-PRECONDITIONS`), not slice 4's contracts, which remain the checkout-side four this item names.
