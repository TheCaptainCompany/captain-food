...
- **Glossary** (added at slice 2, [PR #920](https://github.com/TheCaptainCompany/captain-food/pull/920) — the word enters the codebase here, `specs/**` is deliberately NOT where it lands; see §11 slice 2): **as-of** — a read reconstructed at a FIXED past coordinate on an event stream, never the live head (the camelCase `asOf` spelling is dropped at round 2 — evans NAF; there is exactly one spelling of "as-of" in this file); **`AsOfCatalog`** — the Catalog aggregate's price-only as-of capability (`crates/domain/src/catalog_as_of.rs`), built by re-folding the same `catalog::apply` the write path uses and truncating at the coordinate; **`CatalogVersion`** — the coordinate itself (round 2), the 1-based `domain_events.version` verbatim (ADR-20260808-171056), never a port-invented 0-based convention and never a slice index.
...
- **Two catalog resolutions per checkout.** As-of plus HEAD. The as-of fold is a stream fold, not a projection read, so its cost grows with the catalog's stream length and there are no snapshots (a consciously accepted V0 trade-off). At Friday 20:30 this sits on the money path. Mitigations exist and are unproven: the shared `SNAP-1` primitive, and folding only the offers the cart references. **RE-MEASURED 2026-09-06 (slice 2 round 2, [PR #920](https://github.com/TheCaptainCompany/captain-food/pull/920)), not projected — round 1's number is SUPERSEDED, not merely refined.** Round 1 measured only V=200 of L=2,000 identical stock rows and called it "the cost"; `business`/`dba` caught at the round-1 presentation pass that a checkout actually reads ~L rows dominated by whole-menu content, not 200 — V=200 alone was a mutant detector wearing a cost-number costume. Round 2 measures BOTH arms, lab-measured on this container, DB-gated, L=2,000 events (`UNVERIFIED input` — "the largest realistic HubRise import (~500 products) plus a full resync plus a Friday's worth of stock syncs", a judgement call, not a measurement):
  - **Arm (a), V=200 (the mutant detector, stock-sync-shaped):** 10 iterations — `sql median=4.4ms decode median=4.3ms fold median=0.6ms end_to_end median=9.5-10.1ms max_of_10=10-12ms`, 102,296 payload bytes.
