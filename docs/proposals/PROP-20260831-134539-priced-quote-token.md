...
2. **The as-of capability.** `AsOfCatalog` + `OfferPrice` (named `OfferPrice`, not `PricedOffer` — `Offer` is a catalog entity, this is a line price at a coordinate) with a required `tax_rate`. Test: folding `Catalog-{id}` to version V reproduces the price at V after arbitrary later events, including `OfferStockUpdated`. Negative: `OfferPrice` exposes no availability accessor — enforced by the type (a `compile_fail` doctest, paired with an identical passing doctest so the negative itself is proven to compile under the RIGHT conditions).
   ...
   **Opens for slice 3, not resolved here (graphql-architect CATCH; sharpened at round 2, holub NB8/graphql NB5):** the HEAD `Catalog` projection table carries no projected-version column (`specs/database/tables/projection_tables.yaml#/Catalog`), so slice 3 cannot mint a `catalogVersion` coordinate that is guaranteed to match the prices the SAME read produced without one of: (a) a spec change adding a projected-version column to `Catalog`, or (b) minting the coordinate from the write-side fold rather than the read-side projection. This is slice 3's first question, not slice 2's — nothing in slice 2 depends on the answer. Three additions from the round-2 presentation pass, recorded here rather than decided:
   - **(a) The customer consequence of getting this wrong.** A coordinate that does not match the priced rows a customer was shown is a SELF-INFLICTED price-mismatch refusal at the pay button — the exact #816 shape this whole proposal exists to close, reopened from the minting side rather than the display side (ux NB5).
   - **(b) The decidable answer, named as the LEADING option, not decided here.** The coordinate is the last applied stream version the SAME range read returns — `AsOfCatalog::from_stream`'s own fold already knows the highest version it applied (round 2's fail-closed check computes exactly this number as a side effect), so slice 3 can mint the coordinate from the write-side fold directly, with NO projected-version column needed on `Catalog`. This is option (b) above, sharpened: it costs nothing extra to compute, because round 2's refusal check already computes it.
   - **(c) The write-side repair-list join.** `specs/ordering/processmanager.yaml:68` records that pricing from the catalog projection is DELIBERATE — folding the stream instead would make that comment and the process manager's actual behaviour disagree once slice 3+ lands. It joins `specs/ordering/rules.yaml:61-65` (the `expectedTotal` enforcement clause slice 4 already must rewrite, §11 alongside note) on slice 4's repair list: both need rewriting in the SAME change once the as-of fold becomes the write side's actual price authority (evans NB3).
