...
- **Glossary** (added at slice 2, [PR #920](https://github.com/TheCaptainCompany/captain-food/pull/920) — the word enters the codebase here, `specs/**` is deliberately NOT where it lands; see §11 slice 2): **as-of** / **asOf** — a read reconstructed at a FIXED past coordinate on an event stream, never the live head; **`AsOfCatalog`** — the Catalog aggregate's price-only as-of capability (`crates/domain/src/catalog_as_of.rs`), built by re-folding the same `catalog::apply` the write path uses and truncating at the coordinate.
...
2. **The as-of capability.** `AsOfCatalog` + `OfferPrice` (named `OfferPrice`, not `PricedOffer` — `Offer` is a catalog entity, this is a line price at a coordinate) with a required `tax_rate`. Test: folding `Catalog-{id}` to version V reproduces the price at V after arbitrary later events, including `OfferStockUpdated`. Negative: `OfferPrice` exposes no availability accessor — enforced by the type (a `compile_fail` doctest, paired with an identical passing doctest so the negative itself is proven to compile under the RIGHT conditions).
   **Done-when, added at approval (the re-expressed third `Concerns` entry): DISCHARGED.** **REALIZED by [PR #920](https://github.com/TheCaptainCompany/captain-food/pull/920), 2026-09-06** — `crates/domain/src/catalog_as_of.rs`, `crates/application/src/ports.rs#/AsOfPriceAuthority`, `crates/infrastructure/src/persistence/catalog_as_of.rs`. DARK: no production caller, no cache, no checkout wiring (mob consent — the one-pricer property is protected by an equivalence test instead of a caller). The fold's cost is **measured, not projected** — §12's first bullet is rewritten below to the measurement.
   **Opens for slice 3, not resolved here (graphql-architect CATCH):** the HEAD `Catalog` projection table carries no projected-version column (`specs/database/tables/projection_tables.yaml#/Catalog`), so slice 3 cannot mint a `catalogVersion` coordinate that is guaranteed to match the prices the SAME read produced without one of: (a) a spec change adding a projected-version column to `Catalog`, or (b) minting the coordinate from the write-side fold rather than the read-side projection. This is slice 3's first question, not slice 2's — nothing in slice 2 depends on the answer.
...
- **Two catalog resolutions per checkout.** As-of plus HEAD. The as-of fold is a stream fold, not a projection read, so its cost grows with the catalog's stream length and there are no snapshots (a consciously accepted V0 trade-off). At Friday 20:30 this sits on the money path. Mitigations exist and are unproven: the shared `SNAP-1` primitive, and folding only the offers the cart references. **MEASURED 2026-09-06 (slice 2, [PR #920](https://github.com/TheCaptainCompany/captain-food/pull/920)), not projected**: lab-measured on this container, DB-gated, L=2,000 events (`UNVERIFIED input` — "the largest realistic HubRise import x a Friday's stock syncs", a judgement call, not a measurement), reading up to V=200 (10 iterations) — `sql_p50=2.38ms decode_p50=1.63ms total_p50=5.58ms total_p95=total_p99=total_max=6.41ms`, 40412 payload bytes. Peak-unverified: no production Tours catalog stream length has been observed. Against `business`'s stop numbers (§ dispatch record): p99 6.41ms is **far** under both the 50ms ship-dark threshold and the 150ms escalate threshold — nothing here blocks slice 3.
