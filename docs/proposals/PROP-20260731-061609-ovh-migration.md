...
- **Status**: **Superseded in its destination by
  [ADR-20260807-002705](../adr/ADR-20260807-002705-hosting-ovh-mks-cnpg-gitops.md) / PROP-20260806-223656
  (Approved)**: the destination is **OVH Managed Kubernetes + in-cluster CNPG**, after an interim
  Clever Cloud decision (ADR-20260806-151122, 2026-08-06, itself now superseded). This file remains
  the record of the instance-vs-PaaS-vs-VPS option space (§2 D1/D2 — the VPS/vRack findings, the
  Clever Cloud pricing, the managed-PG trade-offs) and of the reasons for leaving Render/Supabase
  (§1, unchanged). **The start-clean decision (ADR-20260807-002705 §6) deletes this file's
  dump→restore cutover steps**; the deploy-pipeline shape (§3 D4) survives, retargeted at the GitOps
  path.
- **Date**: 2026-07-31 (destination revised 2026-08-06, final 2026-08-07)
...
| **Not OVH at all — Clever Cloud (French PaaS, Paris region)** — raised by the product owner 2026-08-05, and **choosing it REOPENS ADR-20260731-061609**, whose destination is otherwise settled | **Deletes the host layer entirely**, and with it most of [PROP-20260805-181926](PROP-20260805-181926-host-provisioning-and-configuration-ownership.md): no OS to patch, no cloud-init, no OpenTofu, no WireGuard, no block volumes, no WAL archiving to build — only its D7 survives. Deploys via Docker, so the GHCR + digest-pinned pipeline can be preserved — though **not for free**: Clever Cloud builds a Dockerfile from the repo rather than pulling a pre-built image, so it takes a one-line `FROM …@sha256:<digest>` Dockerfile written by CI (ADR-20260806-151122 follow-up), and the **Docker** runtime rather than the `Rust` one. **Managed PostgreSQL with daily backups (7-day retention) included on paid plans, and PITR via pgBackRest on request** — which is precisely the work self-hosting makes ours. Strongest **sovereignty** posture on this page: data in France, European jurisdiction, explicitly outside the Cloud Act, against a Supabase that is US-owned. For a team of one product owner plus agents, not operating a Postgres server is the whole argument | **We just left a PaaS, and the Render failure modes were egress exhaustion, build caps and disk** — whether Clever Cloud meters egress the same way is THE question to settle before anything else, and it was not established here. Cost is above 2 x d2-2: an app instance plus a paid database plan, and the **free DEV database plan carries NO backups** (since 2025-10-01), so it is a trap for the same reason the Supabase free tier is. Exact plan prices were NOT verified — use the vendor's price estimator, not a secondary source (this proposal has already been bitten once by third-party spec tables). Less control than an instance, which is the point and also the risk |
| Managed Kubernetes — **OVH MKS** or **Clever Kubernetes Engine (CKE)** | The playbook's someday-K8s posture; rolling deploys. Both control planes are cheap-to-free: OVH's is **free for life**, and CKE (public beta since 2026-04-27) integrates with Clever Cloud's managed databases and S3 storage | **"Free" is the CONTROL PLANE only** — re-checked 2026-08-06 after the product owner read it as a free cluster. Worker nodes, block volumes, public IPs and a **Public Cloud Load Balancer** are all billed separately, so OVH MKS starts around **EUR 18/month**: above the EUR 9.75 Clever Cloud selection and above 2 x d2-2. The control plane was never the expensive part, so its price does not touch the original objection — **a control plane to operate for ONE pod**, with [#193](https://github.com/TheCaptainCompany/captain-food/issues/193) capping us at one instance until [#242](https://github.com/TheCaptainCompany/captain-food/issues/242)'s leases land. It also **undoes the reason we chose a PaaS at all** (ADR-20260806-151122: a team of one product owner plus agents should not operate infrastructure) — k8s is strictly MORE operational surface than the instance plan already rejected. And it does not solve the database: either Postgres runs in-cluster, which is the hardest thing to do well in k8s and holds our event log, or we buy managed Postgres anyway and have added a cluster for nothing. **CKE is additionally in public BETA** — the wrong risk for the system holding paid orders. Revisit when #242's leases make multi-instance real AND there is more than one deployable service. **On Kubernetes Jobs specifically** (product owner, 2026-08-06 — *"more flexibility/tools"*, the strongest pro-k8s argument raised): the NEED is real and growing — SIRENE sync, the projector's eventual move out of the web process (ADR-0040/0043), the GDPR erasure technical worker (ADR-20260731-160000), order expiry, reminders. But a cluster is not how we get it, and **three cheaper tiers already exist**: (1) **GitHub Actions scheduled workflows**, already running FIVE of these (`architecture-review`, `prod-smoke`, `render-status`, `sirene-sync`, `stale-claim-reaper`) and free on this public repo — `sirene_ingest` is deliberately a thin CI crate for exactly this reason; (2) **Clever Tasks**, an app that runs a command to completion and shuts down — *literally* a Kubernetes Job without a cluster; (3) **Clever Cloud Workers / a second app** for the projector when it leaves the web process. **And the most important category must NOT be a Job at all**: a domain deadline — the 5-minute acceptance timeout ([PROP-20260726-164500](PROP-20260726-164500-order-operational-safety.md) D1/D2), order expiry, reminders ([PROP-20260731-195500](PROP-20260731-195500-runtime-d-pm-mailboxes-and-reminders.md)) — is per-aggregate business logic that belongs in the actor runtime and the event log, where it is testable and rule-linked. As a cron sweep it becomes an invariant living in infrastructure, which the dependency rule forbids, and it loses precision on the money path: a minute-granularity sweep can leave a PAID order un-accepted for up to 60 extra seconds. **On "eight apps from the beginning" and being scale-ready** (product owner, 2026-08-06): the count is right and the deployment conclusion does not follow — `c4-l2.yaml` declares eight apps, but **seven of them are CLIENTS, not server workloads**. `web-client`/`web-restaurant`/`web-admin` compile to **WASM served as static assets by `api`** (the Dockerfile builds the hydrate bundle into the image); `desktop-restaurant` is a **Tauri app** shipped to machines; the three `mobile-*` are **native shells shipped to app stores**. None of those is a pod. Server-side, `c4-l2.yaml` leaves **`api` + `bam` + `otel-collector`** — `sync-worker` is specified as *"GitHub Actions cron + Rust binary"*, i.e. deliberately not a pod, and `event-store`/`read-models` are managed PostgreSQL. **Three server workloads, not eight.** The other two arguments: *deploy without considering the host* is **already true at the image layer** — the portable unit is the digest-pinned OCI image, which runs unchanged on a PaaS, a cluster or an instance, and the Clever Cloud lock-in is a Dockerfile plus env vars plus a connection string. And *prepared for scale from the beginning* is the one that cannot be bought from a platform: [#193](https://github.com/TheCaptainCompany/captain-food/issues/193) caps us at ONE instance until [#242](https://github.com/TheCaptainCompany/captain-food/issues/242)'s mailbox **leases and fencing** land — the blocker is in the APPLICATION, so a cluster would orchestrate a system that cannot yet run two copies of itself. **The honest trigger to revisit**: if the three web clients become separate SSR deployments rather than assets of `api`, server-side workloads reach ~6 and per-app PaaS pricing starts losing to pod packing — that, plus #242 done, is when CKE or MKS earns it |
...
**Note — the fixed-price / parallel-build case for a cluster (product owner, 2026-08-06), checked against
this repo.** Three claims, one of which is actionable but not via Kubernetes:

- *"Fixed price for all the pods, each using a fraction of the cluster CPU."* True of Kubernetes, and
  real above a crossover point — but it prices **compute only**, and compute is not our dominant cost.
  The **managed PostgreSQL with adequate disk is bought separately either way**, a cluster adds a
  **Load Balancer** as its own paid component (OVH's MKS-specific LB reaches EoL July 2026, so the
  Public Cloud LB is now mandatory), and "fixed" is fixed until the next node — after which it is a
  step function plus capacity planning, i.e. operational work. At **three** server workloads
  (`api`, `bam`, `otel-collector`) per-app PaaS pricing is comparable or cheaper.
- *"Compile apps in parallel to cut deployment duration."* There is **one** `server` binary and **one**
  `web` WASM bundle from a shared Cargo workspace — not eight independently compilable apps. Cargo
  already parallelises across cores inside a build, and splitting across machines makes each one
  rebuild the shared dependency graph unless a cache is shared: usually **worse** wall-clock and
  strictly more CPU-hours. Deployment today is `docker pull <digest>` plus start; Kubernetes would add
  a node image pull, a rolling update and readiness probes to that, not remove anything.
- *"Cut PR-check time."* **PR checks do not touch the hosting platform at all** — they run on
  GitHub-hosted runners, which ADR-20260721-175411 chose precisely because they are free and unlimited
  on this PUBLIC repo. A cluster we pay for cannot beat free-and-unlimited, and no hosting choice
  changes `ci.yml`'s duration by one second.
  **The real lever, and it is free**: `ci.yml` is currently a **single sequential job** doing
  workspace build → wasm check → unit tests → DB tests → validator → drift. The validator is seconds
  and sits behind a full Rust build. Splitting it into parallel jobs (spec/validator · build+tests ·
  DB suites) is the actual PR-check win, costs nothing, and is available today. `Swatinem/rust-cache`,
  `cargo-chef` and the buildx GHA layer cache are already in place — the missing piece is
