...
| OVH VPS | Cheapest — the 2026 VPS-2 is 6 vCPU / 12 GB / 100 GB NVMe at ~EUR 8.49/month, resources that comfortably cover V0 | Weaker isolation/perf guarantees; Public Cloud instances are barely more and sit in the same ecosystem as the managed DB. **Decisive addition (2026-08-05, checked while the product owner was pricing a 2-VPS split): VPS appears to have NO private network** — OVH's VPS product page lists only public IPv4/IPv6 and bandwidth, and every vRack document scopes the feature to Public Cloud, Hosted Private Cloud and Bare Metal, with VPS absent. So an app-VPS + database-VPS pair would carry Postgres traffic **over public IPs**, forcing an internet-exposed database holding paid orders and customer PII behind nothing but TLS and an IP allowlist. Confirm with OVH support before buying, but this is why the split that looks cheapest is the weakest posture on the one component that must not be got wrong |
