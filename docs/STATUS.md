...
| **Aggregates own the facts — foreign-stream-append isolation, FIRST** | 🚧 plan decided | Founder directive 2026-08-29 (*"We need to resolve this isolation subject first to avoid any new bad development"*), [ADR-20260829-230418](adr/ADR-20260829-230418-aggregates-own-the-facts-isolation-first.md) / register row [AGGREGATES-OWN-THE-FACTS](decisions/AGGREGATES-OWN-THE-FACTS.yaml). The property is the spec's own sentence (`specs/common/processmanager.yaml:7-9`): only the owning aggregate's lane worker appends to its stream; a PM decides, never appends. Plan: **C1** flip-run of the built Order-birth route (C1a produces the pre-flip evidence; the DEFAULT flip goes to the **founder** with the evidence — dispatch card 598 §7/§9) → **C2** [#595](https://github.com/TheCaptainCompany/captain-food/issues/595) → **C3** the twelve remaining `deliver:` steps in target-actor groups (Payment ×7, DeliveryJob ×4, Cart ×1; per-route GENERATED gates; legacy arm deleted separately under golden payload-equality) → **C4** the compiler owns it (EventStore `save` unspellable from a PM). Erasure BUILD waits on C1 + founder approval; the erasure PROPOSAL flow proceeds |
