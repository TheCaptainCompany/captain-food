...
| **Read-surface scope binding — `pendingRefunds` (#618 chunk 1)** | 🟡 **shipped OBSERVING, unflipped** | `READ_SCOPE_BINDING_MODE` defaults to **`observe`**; the bound-caller arm is **unreachable and unflipped, since 2026-08-18** (nothing mints a restaurant claim, so `observe` and `enforce` are observationally identical today — which is what makes the eventual flip a reading rather than a guess). The refund queue is now bound by INTERSECTION: no filter ⇒ the caller's own queue, a conflicting filter ⇒ **empty** plus `read_authorization_scope_mismatch_total{operation,role,mode}`, never the caller's own rows (substitution would make the schema lie). An **unbound** RESTAURANT credential — the only one that can exist today — reads **nothing**, and that arm is NOT gated between `observe` and `enforce`. ADMIN unchanged — **and unbounded** (the view folds every `RefundOpened` ever recorded, no `LIMIT`; follow-up owed). ⚠️ **The `off` rung is Render-env-only** — `emit/deploy.rs` renders no non-secret key into any bin manifest and there is no ConfigMap, so after the #358 cutover the only override is a drifting hand-edit; that cutover chunk or the `enforce`-flip chunk owns rendering it. ⚠️ **`off` DIES at the credential-minting chunk (#639)**: from the moment a real restaurateur holds a login it stops being a rollback and becomes a cross-member disclosure switch an operator can throw at 20:00 on a Friday with no second signature. **The WRITE half is still open** — `approveRefund`/`denyRefund` consult no identity anywhere (#635/#178), so a restaurant now sees only its own refunds and can still approve anybody's. If this row is still here when a restaurant credential exists, the chunk failed and it is legible. |
