...
Further amendment (2026-07-20, product-owner directive): §1's `size/*` labels are **renamed
`impact/*`** — same T-shirt, same meaning (**Impact = the size of the change on the code**),
clearer name; the project board's Impact field (renamed from "Size") carries the same value, and
the org **Effort** field is its coarse projection. The **value** an issue brings if completed is
a separate signal: the org **Value Size** field (T-shirt XS–XL). See
