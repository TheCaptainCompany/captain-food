...
| [0040](0040-materialized-read-model-tables-projectors.md) | Materialized read-model tables — physical form = file (`projection_views.yaml` VIEWs `View_*` vs `tables/projection_tables.yaml` TABLEs, no prefix); `projector: trigger` (generated AFTER-INSERT on domain_events + rebuild fn, mechanical folds) vs `app` (deferred Rust, computed columns — business logic stays out of the DB, write path uncoupled) |
