...
> reviewer pass, never a founder wait.
...
> **2026-09-06 — [#926 "#924 follow-ups (the Red-first gate, round 2)"](https://github.com/TheCaptainCompany/captain-food/issues/926) items 1-5 landed, draft [PR #927](https://github.com/TheCaptainCompany/captain-food/pull/927), Lane B (session_01H3AFBVzhSiGXJcFuwKjiMQ, `926-redfirst-none-form-and-token-mirror`).** Item 1: the `Red-first: none` explicit negative was a PREFIX GLOB (`[Nn]one*`) reading anything starting with "none" as the negative — found LIVE: the coordinator's own first draft of this card was refused by Lane D itself (`dispatch-redfirst-false-negative`, 17:26Z) because a prose sentence carried the marker followed by the word "nonesuch" on a genuine 52-hit trail. Now the DECLARED text only (`none — <record> names no test`); `<record>` must resolve AND be one the trail cites (membership by resolved path, never id string); anything else beginning with `none` falls to the shape refusal (RF10/RF11a/RF11b/RF15, each shown red against the old glob and the two narrower mutants, then green). Item 2 (consent decision, option (a) — reviewer, beck and farley unanimous): `rf_entry_ok` stays one boolean for the whole section; workflow.md now separates the CARD's obligation (one entry per hit, read by the mob and the reviewer, not checkable — the ADR's own honesty limit already concedes it) from what THE HOOK checks (at least one well-shaped entry), with the residual (one valid entry excuses the other hits) named beside the declaration. Item 3: `tools/codegen-rs/src/tests.rs` no longer hand-copies Rule 1's token set — it reads `REDFIRST_TOKENS` from the hook file itself at test time, fails closed on an empty extraction, and asserts no alternative carries a regex metacharacter. Item 4: a new non-empty 0-hit fixture (RF12 pins "line exists, no token" distinctly from past-EOF), RF13 pins the existing-path-with-no-`::` guard, RF14 pins "the negative speaks for the whole trail" over two cited records where only one hits, and `expect_d` gained an optional stderr-fragment assertion (RF10, RF12). **Checkpoint (reviewer, beck, farley — all three had declared a CONCERN at briefing): all three CONTINUE; beck and the reviewer independently re-ran every planted mutant in scratch copies and every red reproduced.** Item 5 reshaped by farley at the checkpoint, replacing the card's own proposed per-step `timeout-minutes`/self-hang-bound design: a post-hoc self-timer CANNOT be a hang bound (a hung script never reaches the line that would read it; CI's `gate-scripts` job already carries `timeout-minutes: 10` at the job level) — so `register-check-selftest.sh` now prints its elapsed wall seconds on every PASSING run and, past `REGISTER_CHECK_SELFTEST_CEILING_SECONDS` (default 120s, antecedent 4s on five green main runs / 4.7s local), prints exactly one WARN line naming itself a TRIPWIRE (an unverified ceiling, never a hang bound, never the meter — [#923](https://github.com/TheCaptainCompany/captain-food/issues/923) §4's `gate_minutes_per_round`, read from CI check-run times, owns that); the exit code never changes, so the every-turn local path from `stop-gate.sh` cannot become a re-run trainer. Pinned by ST1, a small in-process function shown red (`command not found`) before it existed — the card's own Red-first entry named ST1 as an env-knob mutant (override the ceiling to 0 through the env knob); the shipped ST1 pins the function in-process instead, never the knob or the call site itself (follow-up). `ci.yml:82`'s antecedent corrected to the `changes` job's own measured ~9-14s; the word "meter" does not occur in `ci.yml` (grepped) — no edit made for it. Two must-fixes found IN THE CHECKPOINT DIFF: beck found the token-mirror test was green with `REDFIRST_TOKENS=''` (`split('|')` on empty yields `[""]`, and `str::contains("")` matches every line — both the emptiness check and the regex-metacharacter check were vacuously true over an empty string) and its count/crafted-line assertions were tautological self-comparisons that could never red regardless of what the tokens said; fixed with a non-empty-after-trim assertion and a BEHAVIOURAL test (a crafted three-line fixture, matcher must select exactly the one line naming a real token), shown red by pointing the extraction at a scratch hook copy with `REDFIRST_TOKENS=''` and restored; beck also added RF14b (the same two-record trail, citation order REVERSED), closing a gap RF14 alone left open — RF14 never catches "count hits over the LAST cited record only" because its own citation order happens to make that mutant sum correctly by accident. The reviewer found a **card defect**: workflow.md claimed ADR-20260906-024838 "carries 17 hit lines" (a coordinator brief's uncounted figure) where the gate's own oracle (`grep -ciE "$REDFIRST_TOKENS"` on the resolved file) says 28 — corrected, naming the oracle; the same pass updated two places still describing the none-form arm as `[Nn]one*`, now stale after item 1. Gates: `register-check-selftest.sh` exit 0 ("all cases pass"), `make validate` 0 errors, `make rust` green (454 Rust tests, including the new/updated ones, no drift); `make test-crates`/`cargo clippy` not applicable (no `crates/**`, no Postgres). PR stays DRAFT (GREEN merge condition; the coordinator flips ready + auto-merge). Item 6 stays deferred with [#914](https://github.com/TheCaptainCompany/captain-food/issues/914) item 10.

> **2026-09-06 — PROP-20260831-134539 slice 3a MERGED-READY (draft [PR #922](https://github.com/TheCaptainCompany/captain-food/pull/922), continuation run, deliverables 2–6 landed on top of the first run's 1+half-of-5): the priced cart read can now be served from the fold, behind a door that stays CLOSED everywhere.** New key `RUN_FOLD_PRICED_CART_READ` (`runKind: door`, `decisionRow: QUOTE-MINT-PRECONDITIONS`, default/deploy `false`, declared at both composition roots under [ADR-20260904-081527](../adr/ADR-20260904-081527-rider-standing-is-a-grant-on-the-identity-row-the-doors-are-human-only-and-step-4-lands-in-three-slices.md) §8's fifth carve-out — the standalone declaration is fleet-parity evidence only — and, from round 2, the sixth carve-out for the `:68` prose rewrite (reviewer/vernon/farley)). `crates/server/src/graphql/cart_read.rs::priced` gains the open arm: CLOSED prices from the `catalog` projection exactly as before; OPEN resolves `catalog_id` from the SAME snapshot then makes ONE `AsOfPriceAuthority::at_head` call, pricing via `price_cart_at` — the coordinate is used and dropped before the reply is built, reaching no field, input or command. A fold failure is `technical_error` through the existing unresolvable-at-read path, never a HEAD/projection fallback — the STOP every lens named at the briefing. `specs/ordering/processmanager.yaml:68`'s "display and charge cannot disagree" note is rewritten to say the guarantee narrows while the door is open, closed for real only by slice 3b's signed quote. `catalog.as_of.fold` now joins the `cart-price` observability contract (required only when the door opens it) with four new metrics, including the `catalog_as_of_reads_total{outcome}` dead-man; `business.correlation_id` is recorded from the priced read's own request id — the first time this span has ever carried a real one. Benchmark honesty: a new `sql+decode+fold median <= e2e median` sanity assertion, a third arm measuring the NATIVE `at_head` call itself (median 113–117 ms, same ship-dark band as the existing `as_of` arm), and ceilings restated as a stated multiple (6×) of the phase-0 budget row's 150 ms target, both `UNVERIFIED input`. Record: [ADR-20260906-154419](../adr/ADR-20260906-154419-the-priced-read-is-served-from-the-fold-and-carries-its-coordinate-behind-a-door.md) (team decision by consent, twelve-lens Consulted block, D1–D9 of the briefing, amends [ADR-20260810-112836](../adr/ADR-20260810-112836-cart-priced-live-on-read.md) §1/§3 in part); new register row `QUOTE-MINT-PRECONDITIONS` (open, team); PROP-20260831-134539 §4/§11/§12 rewritten (slice 3 split into 3a REALIZED / 3b next / 4, the D6 budget target recorded, UC1's slice-status note); counsel packet gains CQ-5/CQ-6 (never advice). Four red-first server tests, one telemetry value-capturing test and the fleet-parity codegen test all planted red then reverted green (quotes in the hand-back). Gates: `make validate` 0 errors, `make rust` green, `cargo clippy --workspace --all-targets -D clippy::disallowed-methods` clean, `python3 tools/link-check.py` 0 broken across 470 files. This slice ALONE is inventory (holub): it closes nothing on [#816](https://github.com/TheCaptainCompany/captain-food/issues/816) until 3b's signed quote lands, in the same lane, next. PR stays DRAFT — HOLD: human, the coordinator flips ready after the team's reviewer pass. Round 2 facts: mutant C (mixed-authority pricing) killed by a test, `payload_bytes` from Postgres (113.8 → 89.6 ms local, `UNVERIFIED input`), 3b+4 confirmed as ONE deliverable, register row items (7)-(13) named.
...
> **2026-09-06 — PROP-20260831-134539 slice 3a: structural deliverables 1 and a partial 5 landed DARK (no caller); deliverables 2–6 deferred on a FENCE FINDING, draft [PR #922](https://github.com/TheCaptainCompany/captain-food/pull/922) on [#816](https://github.com/TheCaptainCompany/captain-food/issues/816), HOLD: human (money path).** `domain::catalog_as_of::AsOfCatalog` now carries the coordinate it was folded at (the CEILING `from_stream` was bounded with, never "the highest applied business version" — a stream whose last business event is version 2, folded to `up_to=3`, still carries coordinate 3; `narrow` is the sole constructor and always takes it). `application::ports::AsOfPriceAuthority` gains `at_head(catalog_id) -> (AsOfCatalog, CatalogVersion)` — ONE unbounded range read to head, never `latest_version()`, never `Option<CatalogVersion>`, never a relaxed `load_range` check; the Postgres adapter reads the whole stream and takes the ceiling as the max over the RAW rows (the same technique the fail-closed check already used), refusing a stream with zero rows (a catalog never created). `application::pricing::price_cart_at` is the fold-priced counterpart to `price_cart`, equal on the same lines — names/product ids stay HEAD's business (the fold carries no labels, by slice 2's own design), prices/coordinate come from the SAME `AsOfCatalog` the caller already resolved. The one-pricer equivalence test is extended to the coordinate AND dba's two-folds-agree property (`price_cart` and `price_cart_at` must produce the IDENTICAL `PricedCart` at HEAD). Deliverable 5's mechanical half: `the_benchmark_fixture_is_actually_l_events_long` pins the slice-2 benchmark's L via `SELECT max(version)`, never the seeding loop's own arithmetic. **The fence finding**: deliverable 2 (the door key `RUN_FOLD_PRICED_CART_READ` + `declare_flag` at both composition roots + the fleet-parity gate) requires a touch inside `crates/infrastructure/src/mailbox/standalone.rs` — the codegen `run_flag_parity` test hardcodes this file as THE standalone composition root for every `runKind: door` key, unconditionally, regardless of subject matter. That path sits inside this card's own Fence list (ADR-20260904-081527 §8), which names exactly ONE carve-out (the `processmanager.yaml:68` prose rewrite); the card's own binding language — *"If anything else needs the fence, STOP and hand back"*, and the STOP-conditions list separately naming *"the fence beyond the one carve-out"* — leaves no room to grant a new carve-out solo, however narrow (a precedented, declare-only, no-`CommandDeps`-field shape exists, `RUN_RIDER_RESTRICTION_SOCKET_CLOSE`'s class, `standalone.rs:321-323`, but every prior carve-out to this ADR was recorded as its own numbered amendment after mob/reviewer sign-off, not a solo executor's call). Deliverables 2–6 (the door, the open arm in `cart_read.rs`, the observability contract, the rest of deliverable 5, the ADR/register/PROP records) are deferred until the architect either grants the carve-out (recording it in ADR-20260904-081527) or directs an alternative shape. Gates: `make validate` 0 errors, `make rust` green (empty `git status --short`), DB-gated `make test-crates` green (230 suites, 1968 passed, 0 failed), `cargo clippy --workspace --all-targets -- -D clippy::disallowed-methods` clean, `make check-drift` clean, `python3 tools/link-check.py` 0 broken. PR stays DRAFT.
