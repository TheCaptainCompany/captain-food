...
| ~~Projection worker never drains `DeliveryJob-%` streams — `OrderTracking.delivery_status` mirror columns spec'd but unfed~~ ✅ landed 2026-08-09 ([#424](https://github.com/TheCaptainCompany/captain-food/issues/424)): the Order projector group slices the full `DeliveryJob-%` family under its own checkpoint, and every delivery fact in the fedBy carries `orderId` (D-QW1 option b, [ADR-20260808-234907](adr/ADR-20260808-234907-answer-sheet-confirmations-dqw1-option-b.md)) so keying is mechanical. Runtime proof: `crates/infrastructure/tests/order_projection.rs::delivery_facts_move_the_customers_delivery_mirror` | — |
