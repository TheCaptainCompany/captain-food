...
### Erasure-zone and retention additions (G-series, 2026-08-11 — context: [BRIEF-20260811-erasure-zone-and-retention.md](BRIEF-20260811-erasure-zone-and-retention.md))

Each question's full context — the spec/code evidence an avocat needs to answer it cold — is in the correspondingly numbered section of that brief.

- **G1 — Empty-log reliance and the trigger moment**: with no data subject in `domain_events` (production starts clean by decision), is the absent Art. 17 path an **unmet precondition** rather than an infringement — and is the trigger the **first real customer order**, the same deadline as Art. 35 ("prior to the processing") and the médiation de la consommation registration, or does the DPIA fall earlier at first real *prospect* processing?
- **G2 — Whole-stream deletion as the Art. 17 mechanism**: does deleting the entire event stream satisfy Art. 17(1) for an event-sourced store, and is the pseudonymous completion receipt a proportionate Art. 5(2) artifact rather than itself excessive retention (and does it need its own window)?
- **G3 — Accounting retention vs Art. 17 — BLOCKING**: Code de commerce L123-22 (10 yr) / LPF L102 B (6 yr) against Art. 17 and Art. 5(1)(e). Which closure is more defensible — (A) keep the `Order` window at the 10-year horizon and tombstone personal fields through projections, or (B) export a financial skeleton before deleting the stream on a short personal-data window? Blocking because the one built erasure path deletes the whole stream on a **single configurable window with no per-category split**, as its own spec states.
- **G4 — The orphaned `sub`**: after the Supabase identity is deleted as an Art. 28 processor instruction, is the `sub` left in the append-only `domain_events.user_id` **anonymous** under Recital 26 or **still personal data**? If still personal, we ask counsel to say directly that **crypto-shredding of the identifier columns is the only mechanism compatible with an append-only log**.
- **G5 — Art. 21 register vs Art. 17 on `Restaurant-*`**: confirm we may (and must) refuse erasure of the suppression entry while erasing the rest of the stream, and state the **minimum field set** for an entry keyed on SIREN/SIRET — and whether keying on the state identifier is required or merely prudent.
- **G6 — Per-category retention schedule** validated against the CNIL référentiel gestion commerciale (délib. 2021-044): abandoned guest carts, identified customer carts, dormant accounts (~3 yr notify-then-delete?), and prospect restaurant records **not obtained from the subject** (plus Art. 14 notice timing when first communication *is* first contact).
- **G7 — `dietaryTags` and Art. 9**: an unconstrained `array<Tag>` (`maxLength: 80`, no enum) makes `halal`, `kosher` and `allergy:peanut` spellable **today**. Does the Art. 9 exposure attach to the field's capability under Art. 25; is the remedy an **excluding enum** or a full **Art. 9(2)(a) explicit-consent** flow; and confirm the **DPIA cannot be finalised while the field is unconstrained**.
- **G8 — Art. 18 restriction of processing** (distinct from erasure, and unbuilt): is a **marker event that every projection and fold honours** an adequate Art. 18 mechanism for an event-sourced store — may rows stay and be filtered at read time, must derived/analytics processing also stop, and does an event carrying the Art. 18(3) notification reference suffice as the pre-lift record?
