...
§25's `{{ root.path }}` binding walk (`screen-binding-unknown-field`, `tools/codegen-rs/src/validate/screen_bindings.rs`) originally checked only a screen's OWN component subtree — a sheet a screen opens (`open_bottom_sheet`) was invisible to it, so `{{ rider.riderld }}` inside a bottom sheet passed `make validate` at 0 errors and would have dispatched a mutation with an empty id at runtime. `check_screen_bindings` now takes the whole screens FILE `doc` and, after checking the screen body, walks every sheet id `screen_roles::reachable_sheets(doc, screen)` returns (the SAME transitive reachability §26's own role walk already derives — one derivation, not two) against the SAME root map the screen's own `data_requirements` computed: a sheet opened from a detail route reads that route's resolver root (`restrict_rider_sheet` reads `rider.*`, exactly like `rider_detail`, which opens it). A finding inside a sheet reports as `screen-sheet-binding-unknown` — a DISTINCT rule code from the screen-body `screen-binding-unknown-field` (same underlying walk, different location, so a screen-level typo and a sheet-level typo are never conflated in a triage list). Wiring this full-strength on the real corpus found and required fixing two genuinely dead bindings already committed in `restaurant_frontoffice.yaml`'s `rating_sheet` (`order.tipRecipient`, `order.currency` — neither is a declared `Order` property; the currency lives at `order.totalAmount.currency`). **Round 2 correction**: `recipient` was ALSO changed to a fixed `RIDER` literal in that same commit, but that was an unrelated, unauthorized decision reversal against ADR-20260722-181500 ("RIDER, or RESTAURANT for self-dispatch"), flagged by the round-2 review — it stays landed only because reverting to the dead `order.tipRecipient` binding trips this very rule again (`Order` has no such field), a genuine unresolved gap recorded in `docs/SPEC-LOG.md` (2026-09-05) and the screen's own `gaps:`, not this rule's to decide. The SAME "corpus first, wiring last, every commit green" discipline §25's own history states.

## `screen-condition-on-form-field` — a `condition:`/`visible_when:` may never read a form field (#639 part C step 4-iii-A round 2 addendum, reviewer)

The recurring #870-class defect (journal 2026-W36 ~443-451): `RenderContext::lookup` (`crates/web/src/renderer.rs`) serves RESOLVER data only, and the client (`interact.rs`) evaluates every `condition:`/`visible_when:` exactly once, at SSR/paint time, before the user has touched anything — a chip pick stashes its value in a hidden input for the NEXT mutation dispatch, it never triggers a re-paint. A node gated on a FORM FIELD declared in the SAME screen/sheet (a `chip_multi_select`, `text_input`, `text_area`, `otp_input`, `phone_field`, `phone_input`, `email_input`, `star_rating`, `tip_amount_selector`, `search_input` — `screen_bindings.rs::FORM_FIELD_KINDS`) can therefore only ever see that field's INITIAL (normally empty/null) state: the node can never actually appear, silently. `check_condition_on_form_field` (`tools/codegen-rs/src/validate/screen_bindings.rs`) collects every such component's own `id:` in a screen's component tree or in ONE `bottom_sheets` entry (checked as two independent closed subtrees, never merged, mirroring exactly where the bug lives: a chip and the condition reading it are always SIBLINGS inside the same unit), then flags any `condition:`/`visible_when:` whose leading dotted-path identifier names one of those ids. Wiring this on the real corpus found it alive in three places unrelated to the round that added the rule (`restaurant_backoffice.yaml`'s `claim_resolve` partial-refund amount picker — a MONEY-PATH control that could never render regardless of which resolution an admin picked — and its `issue_resolution_sheet`'s note field; `restaurant_frontoffice.yaml`'s `rating_sheet` late-delivery reason field), all fixed by rendering the content unconditionally. **`specs/screens/rider.yaml` is excluded from this rule's wiring** (`validate/core.rs`, both call sites): the round that added the rule was forbidden from touching that file (a concurrent-session merge fence), and it carries one instance of the same class (`issue_kind.value` in `rider_report_sheet`) already documented, in a PRIOR round's own comment, as a deliberately-deferred gap — the rule is scoped OFF that file rather than either breaking the fence or leaving a known-red finding in the corpus.
