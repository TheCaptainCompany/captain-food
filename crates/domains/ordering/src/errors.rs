...
/// DECLARED ahead of its wiring (ADR-20260906-192007 D-D, PROP-20260831-134539 slice 3b): the designed REPLACEMENT for `errors.yaml#/PriceMismatch`, which stays live and unchanged until the quote is verified server-side, the phase that lands the write-side verify guard together with this error's real `thrown:` coverage (`make validate`'s `test-uncovered-error` refuses a `throws:` reference to an error no code can yet produce, confirmed empirically — the guard-replace and this error's wiring are one indivisible change, not two). Repurposing `PriceMismatch` instead of deleting it was refused (evans): its copy ("Prices have changed…") is false for the structural refusal causes the new refusal set enumerates. This is the ONE business error in that set, for exactly two causes, both collapsed to the SAME structural fact the signed quote's own binding makes checkable: the cart was edited after the quote was minted (the token binds cartId AND the cart's own stream version, so an edit is a version mismatch the moment the two differ), and expiry (QUOTE-STALENESS, 30 min). Quiet in telemetry (`DomainError::Rejected`, never loud like the structural/forged-signature refusals, which reject on the ADR-20260810-112836 §6 technical path instead and never reach this error at all). `context: { cartId }` ONLY — no amount, no delta, no currency, no cause: the customer sees ONE cause-neutral screen for every refusal in the set (forged, foreign-cart, retired-key, V-beyond-head, cart-edited, expired, fold-failure), because naming the cause on a consumer surface risks implying a price change occurred when it may not have (a forged token, a stale cart). MUST NOT: an amount, an old→new pair, a delta, a currency figure, a countdown, refund/bank-date language, or the word "price" in a way that implies one changed for causes that never were a price change. The CTA reuses `checkout.payment_failed.back_to_cart` — no new CTA, no retry control. Messages below are ux's DRAFT for counsel review (CQ-7, never clearance) — not yet legally cleared copy.
/// Context: `cartId`.
...
/// Reachable only when a client sends `expectedTotal`, which no shipped client does (`crates/web/src/checkout.rs:610` sets it in a test fixture only; `application::commands`'s `if let Some(expected) = &cmd.expected_total` at :2859 is the live, currently-dead check) — the check is LIVE IN CODE, DEAD ON THE SHIPPED CLIENT (ADR-20260906-192007 D-D): the client-submitted confirmation total (PlaceOrder.expectedTotal) differs from the total the server recomputed from the live catalog. The server is the only price authority: the checkout is rejected so the customer is never charged an amount other than the one they were shown. Superseded by `QuoteNoLongerHonoured` (declared above) once the quote is verified server-side — deletion and the guard-replace land together, in the same change as that guard's real test coverage (see `QuoteNoLongerHonoured`'s own description for why the two cannot split across phases).
