...
impl CoopCycleWebhookIngestor {
    ...
    pub async fn ingest(
        ...
        // `DeliveryJobClient` (#284 slice 3; ADR-20260731-122500 — co-op vocabulary stops here).
        ...
        let corr = coopcycle_correlation_id(instance_id, &event.id);
        let recorded = match domain_event {
            ...
            other => {
                self.raw.mark_processed(&key).await?;
                return Ok(CoopCycleIngestOutcome::Unmappable {
                    reason: format!(
        ...
        let outcome = match recorded {
            ...
            EnqueueOutcome::Deduplicated(_) => CoopCycleIngestOutcome::Duplicate,
