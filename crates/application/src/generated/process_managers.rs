...
pub mod delivery_dispatch_process {
    ...
    pub trait OrderMarkedReadyHooks: Send + Sync {
        /// Execute `read order` over `OrderTracking` (where order_id = message.orderId). SELECTed from the `ordertracking` projection (delivery_dispatch.rs read_order -> OrderReadRepository::by_id -> SELECT FROM ordertracking). An order not yet projected SKIPS the leg rather than dispatching blind. Return `Skip` to end the leg as a benign no-op.
        ...
        /// Execute `read restaurant` over `Restaurant` (where restaurant_id = message.restaurantId). The pickup address -- FOLDED from the Restaurant's own stream (delivery_dispatch.rs read_restaurant -> EventStore::load(restaurant_stream)), never SELECTed from the `restaurant` projection, even though the surrounding leg IS generated: the hook chose the stream so a courier is never sent to a stale address. SOURCE is per-STEP, not per-leg -- this step is the proof. Return `Skip` to end the leg as a benign no-op.
    ...
    pub async fn on_order_marked_ready(
        ...
        // read `order` ← OrderTracking where order_id = message.orderId. SELECTed from the `ordertracking` projection (delivery_dispatch.rs read_order -> OrderReadRepository::by_id -> SELECT FROM ordertracking). An order not yet projected SKIPS the leg rather than dispatching blind.
...
pub mod cart_binding_process {
    ...
    pub trait CustomerIdentifiedHooks: Send + Sync {
        /// Execute `read open_carts` over `Cart` (where session_id = message.sessionId, status = OPEN). SELECTed from the `cart` projection (cart_binding.rs read_open_carts -> CartReadRepository::open_by_session -> SELECT FROM cart): the leg needs to ENUMERATE a session's carts, which no single aggregate stream can answer. Projector lag is benign here -- a cart missed on this pass binds on the next identification, and the Cart's one-time bind absorbs a duplicate. Absence = empty (no skip).
    ...
    pub async fn on_customer_identified(
        ...
        use crate::process_managers::Outcome;
        let actor = crate::process_managers::saga_actor(env);
        // read `open_carts` ← Cart where session_id = message.sessionId, status = OPEN. SELECTed from the `cart` projection (cart_binding.rs read_open_carts -> CartReadRepository::open_by_session -> SELECT FROM cart): the leg needs to ENUMERATE a session's carts, which no single aggregate stream can answer. Projector lag is benign here -- a cart missed on this pass binds on the next identification, and the Cart's one-time bind absorbs a duplicate.
...
pub mod payment_settlement_process {
    ...
    pub trait OrderDeliveredHooks: Send + Sync {
        /// Execute `read order` over `OrderTracking` (where order_id = message.orderId). The fulfilled order, with its folded payment state. SELECTed from the `ordertracking` projection (payment_settlement.rs load_order -> OrderReadRepository::by_id), like all four legs of this saga -- the projected row is where `payment_status` and `payment_intent_id` are folded together, which is the pair the capture/release decision needs. Return `Skip` to end the leg as a benign no-op.
        async fn read_order(&self, order_id: domain::generated::scalars::OrderId) -> Result<super::HookOutcome<OrderRead>, domain::shared::errors::DomainError>;
    ...
    pub async fn on_order_delivered(
        ...
        use crate::process_managers::Outcome;
        // read `order` ← OrderTracking where order_id = message.orderId. The fulfilled order, with its folded payment state. SELECTed from the `ordertracking` projection (payment_settlement.rs load_order -> OrderReadRepository::by_id), like all four legs of this saga -- the projected row is where `payment_status` and `payment_intent_id` are folded together, which is the pair the capture/release decision needs.
        let order = match hooks.read_order(event.order_id).await? {
            super::HookOutcome::Ready(v) => v,
            super::HookOutcome::Skip(reason) => return Ok(Outcome::Skipped(reason)),
        };
...
pub mod refund_process {
    ...
    #[async_trait::async_trait]
    pub trait OrderRejectedByRestaurantHooks: Send + Sync {
        /// Execute `read order` over `OrderTracking` (where order_id = message.orderId). SELECTed from the `ordertracking` projection (refund.rs load_order -> OrderReadRepository::by_id), like all four opening legs of this saga -- the captured total and the payment intent the RefundOpened fact carries come from the projected row. Return `Skip` to end the leg as a benign no-op.
        async fn read_order(&self, order_id: domain::generated::scalars::OrderId) -> Result<super::HookOutcome<OrderRead>, domain::shared::errors::DomainError>;
    ...
    pub async fn on_order_rejected_by_restaurant(
        ...
        use crate::process_managers::Outcome;
        let actor = crate::process_managers::saga_actor(env);
        // read `order` ← OrderTracking where order_id = message.orderId. SELECTed from the `ordertracking` projection (refund.rs load_order -> OrderReadRepository::by_id), like all four opening legs of this saga -- the captured total and the payment intent the RefundOpened fact carries come from the projected row.
        let order = match hooks.read_order(event.order_id).await? {
            super::HookOutcome::Ready(v) => v,
            super::HookOutcome::Skip(reason) => return Ok(Outcome::Skipped(reason)),
        };
