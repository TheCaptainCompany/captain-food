...
// permission to address `PlatformMembership`, and nothing else.
//
...
// permission): `send` + `PlatformMembershipCommand` exist iff the actor receives >=1 COMMAND, `record` +
// `PlatformMembershipFact` iff it receives >=1 inbound FACT, `schedule`/`cancel_scheduling` iff it declares
...
impl sealed::Sealed for domain::generated::commands::GrantPlatformAccess {}

/// GENERATED from actors.yaml `PlatformMembership.receives`: marker for every COMMAND the `PlatformMembership` actor
...
pub trait PlatformMembershipCommand: sealed::Sealed + serde::Serialize + Send {
    /// The mailbox `message_type` (the commands.yaml key).
    const MESSAGE_TYPE: &'static str;
}

impl PlatformMembershipCommand for domain::generated::commands::GrantPlatformAccess {
    const MESSAGE_TYPE: &'static str = "GrantPlatformAccess";
}

/// GENERATED from actors.yaml: the strongly-typed client for ONE `PlatformMembership` mailbox lane --
...
pub struct PlatformMembershipClient {
    door: ActorDoor,
    actor_id: uuid::Uuid,
}

impl PlatformMembershipClient {
    /// A client addressing the `PlatformMembership` instance `actor_id`.
    ...
    pub async fn send<M: PlatformMembershipCommand>(
        &self,
        msg: M,
        env: Envelope,
        ...
        self.door.assert_addressed("PlatformMembershipClient", self.actor_id, M::MESSAGE_TYPE, &payload)?;
        self.door
            .send_command("PlatformMembership", self.actor_id, M::MESSAGE_TYPE, payload, env)
            .await
    }
}
