...
// permission to address `Conversation`, and nothing else.
//
...
// permission): `send` + `ConversationCommand` exist iff the actor receives >=1 COMMAND, `record` +
// `ConversationFact` iff it receives >=1 inbound FACT, `schedule`/`cancel_scheduling` iff it declares
...
impl ConversationClient {
    ...
    pub async fn send<M: ConversationCommand>(
        ...
        self.door.assert_addressed("ConversationClient", self.actor_id, M::MESSAGE_TYPE, &payload)?;
        self.door
            .send_command("Conversation", 5, self.actor_id, M::MESSAGE_TYPE, payload, env)
            .await
    }
}
