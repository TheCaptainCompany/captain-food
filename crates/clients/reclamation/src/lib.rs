...
// permission to address `Reclamation`, and nothing else.
//
...
// permission): `send` + `ReclamationCommand` exist iff the actor receives >=1 COMMAND, `record` +
// `ReclamationFact` iff it receives >=1 inbound FACT, `schedule`/`cancel_scheduling` iff it declares
