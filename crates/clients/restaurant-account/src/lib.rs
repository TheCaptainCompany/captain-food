...
// permission to address `RestaurantAccount`, and nothing else.
//
...
// permission): `send` + `RestaurantAccountCommand` exist iff the actor receives >=1 COMMAND, `record` +
// `RestaurantAccountFact` iff it receives >=1 inbound FACT, `schedule`/`cancel_scheduling` iff it declares
