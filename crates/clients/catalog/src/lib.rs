...
// permission to address `Catalog`, and nothing else.
//
...
// permission): `send` + `CatalogCommand` exist iff the actor receives >=1 COMMAND, `record` +
// `CatalogFact` iff it receives >=1 inbound FACT, `schedule`/`cancel_scheduling` iff it declares
