...
// permission to address `MailboxSupervision`, and nothing else.
//
...
// permission): `send` + `MailboxSupervisionCommand` exist iff the actor receives >=1 COMMAND, `record` +
// `MailboxSupervisionFact` iff it receives >=1 inbound FACT, `schedule`/`cancel_scheduling` iff it declares
...
/// width 1 (`mailbox.partitions`). The only door to this actor.
...
impl MailboxSupervisionClient {
    ...
    pub async fn send<M: MailboxSupervisionCommand>(
        ...
        self.door.assert_addressed("MailboxSupervisionClient", self.actor_id, M::MESSAGE_TYPE, &payload)?;
        self.door
            .send_command("MailboxSupervision", 1, self.actor_id, M::MESSAGE_TYPE, payload, env)
            .await
    }
}
