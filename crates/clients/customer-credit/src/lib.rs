...
// permission to address `CustomerCredit`, and nothing else.
//
...
// permission): `send` + `CustomerCreditCommand` exist iff the actor receives >=1 COMMAND, `record` +
// `CustomerCreditFact` iff it receives >=1 inbound FACT, `schedule`/`cancel_scheduling` iff it declares
...
impl CustomerCreditClient {
    ...
    pub async fn send<M: CustomerCreditCommand>(
        ...
        self.door.assert_addressed("CustomerCreditClient", self.actor_id, M::MESSAGE_TYPE, &payload)?;
        self.door
            .send_command("CustomerCredit", 5, self.actor_id, M::MESSAGE_TYPE, payload, env)
            .await
    }
}
