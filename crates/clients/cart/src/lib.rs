...
// permission to address `Cart`, and nothing else.
//
...
// permission): `send` + `CartCommand` exist iff the actor receives >=1 COMMAND, `record` +
// `CartFact` iff it receives >=1 inbound FACT, `schedule`/`cancel_scheduling` iff it declares
...
impl CartClient {
    ...
    pub async fn send<M: CartCommand>(
        ...
        self.door.assert_addressed("CartClient", self.actor_id, M::MESSAGE_TYPE, &payload)?;
        self.door
            .send_command("Cart", 5, self.actor_id, M::MESSAGE_TYPE, payload, env)
            .await
    }
