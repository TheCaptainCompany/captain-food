...
// permission to address `Prospect`, and nothing else.
//
...
// permission): `send` + `ProspectCommand` exist iff the actor receives >=1 COMMAND, `record` +
// `ProspectFact` iff it receives >=1 inbound FACT, `schedule`/`cancel_scheduling` iff it declares
...
impl ProspectClient {
    ...
    pub async fn send<M: ProspectCommand>(
        ...
        self.door.assert_addressed("ProspectClient", self.actor_id, M::MESSAGE_TYPE, &payload)?;
        self.door
            .send_command("Prospect", 5, self.actor_id, M::MESSAGE_TYPE, payload, env)
            .await
    }
}
