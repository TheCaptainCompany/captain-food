...
async fn grant(
    ...
    grant_restaurant_access(
        ...
        GrantRestaurantAccess {
            ...
            scope_type: Some(ScopeType::RESTAURANT),
            scope_id: Some(RestaurantId(scope_id)),
            member_id: Some(MemberId(member_id)),
            auth_subject: Some(AuthSubject(auth_subject.to_string())),
            authority: Some(MemberAuthority::MANAGER),
...
async fn door_closed_never_touches_the_auth_subject_reservation() {
    ...
    let result = grant_restaurant_access(
        ...
        GrantRestaurantAccess {
            ...
            scope_type: Some(ScopeType::RESTAURANT),
            scope_id: Some(RestaurantId(uuid::Uuid::new_v4())),
            member_id: Some(MemberId(uuid::Uuid::new_v4())),
            auth_subject: Some(AuthSubject(subject.clone())),
            authority: Some(MemberAuthority::MANAGER),
            basis: AccessBasis::CAPTAIN_ONBOARDING,
            invitation_id: None,
        },
