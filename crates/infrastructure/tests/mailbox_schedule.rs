...
use infrastructure::mailbox::{
    cancel_reminder, reminder_message_id, schedule_reminder, ScheduleOutcome,
};
...
fn gated() -> Option<String> {
    match std::env::var("DATABASE_URL") {
        ...
        Err(_) => {
            ...
            eprintln!("SKIP mailbox_schedule test: DATABASE_URL not set");
            None
        }
    }
}
...
/// message_type = the payload FACT, position NULL and the sequence untouched — and re-declaring
/// with a later time moves the SAME row in place.
#[tokio::test]
async fn schedule_then_redeclare_reschedules_the_same_row_in_place() {
    ...
    let r = row(&pool, mid).await;
    assert_eq!(r.get::<String, _>("status"), "SCHEDULED", "a reschedule never transitions status");
...
/// §2: once the promotion pass has stamped a position the occurrence is SPENT — a re-declaration
/// is a Duplicate (same payload) or a PayloadConflict (different payload), and the row is
...
async fn cancelled_reminder_is_never_promoted() {
    ...
    assert!(!cancel_reminder(&mailbox, order, "expire").await.expect("re-cancel"), "already CANCELLED");
