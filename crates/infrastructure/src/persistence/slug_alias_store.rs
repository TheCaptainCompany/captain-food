...
pub async fn load(exec: impl sqlx::PgExecutor<'_>, previous_slug: Slug) -> Result<Option<SlugAliasRow>, DomainError> {
    let sql = format!("SELECT {COLUMNS} FROM slugalias WHERE previous_slug = $1");
    let row = sqlx::query(&sql).bind(previous_slug.0).fetch_optional(exec).await.map_err(db_err)?;
    row.as_ref().map(decode).transpose()
}

/// Write the folded row. Idempotent on re-projection: replaying the same rename rewrites identical
/// values.
pub async fn upsert(exec: impl sqlx::PgExecutor<'_>, row: &SlugAliasRow) -> Result<(), DomainError> {
    let sql = format!(
