...
//! `delivery_status` are TEXT values (see [`crate::persistence::enum_sql`]); `items`/
...
pub(crate) fn decode(row: &PgRow) -> Result<OrderTrackingRow, DomainError> {
    Ok(OrderTrackingRow {
        ...
        service_type: EnumText::from_text(&row.try_get::<String, _>("service_type").map_err(db_err)?)?,
        ...
        uber_basis: opt_from_text(row.try_get("uber_basis").map_err(db_err)?)?,
        ...
        rating_comment: row
            ...
            .map_err(db_err)?
            .map(RatingComment),
        rider_thumb: opt_from_text(row.try_get("rider_thumb").map_err(db_err)?)?,
        delivery_timeliness: opt_from_text(row.try_get("delivery_timeliness").map_err(db_err)?)?,
...
pub async fn upsert(pool: &PgPool, row: &OrderTrackingRow) -> Result<(), DomainError> {
    ...
    sqlx::query(&sql)
        ...
        .bind(opt_to_text(&row.uber_basis))
        ...
        .bind(opt_to_text(&row.rider_thumb))
        .bind(opt_to_text(&row.delivery_timeliness))
