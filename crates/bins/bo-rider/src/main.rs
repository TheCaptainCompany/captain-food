...
//! `bo-rider` — surface bin -- assets/SSR; speaks to its role gateway, so no database and no views access (the `domain` facade still rides in behind surface_runtime -> web, #475).
//!
//! BUSINESS RUNTIME (#385 API-tier wiring): serves the rider back office (riders.captain.food): the wasm hydrate bundle under `/assets` and the audience's generated screens SSR'd through the PUBLIC role gateway (surface_runtime). Serves the `/health` + `/ping` probes its
//! generated Deployment (deploy/generated/manifests/bins/bo-rider.yaml) declares (`wired:true`),
//! draining on SIGTERM.
//!
...
async fn main() {
    ...
    let settings = surface_runtime::SurfaceSettings {
        audience: surface_runtime::Audience::Rider,
