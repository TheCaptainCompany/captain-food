...
//! `pm-reclamation` — mailbox worker realizing the `ReclamationProcess` process manager -- a DECLARED cross-scope bridge; its domain-crate links are its spec refs.
//!
//! BUSINESS RUNTIME (#385): hosts the `ReclamationProcess` saga runner, restricted to this PM over its own `pm:` checkpoint. Serves the `/health` + `/ping` probes its generated
//! Deployment (deploy/generated/manifests/bins/pm-reclamation.yaml) declares (`wired:true`, readiness
