...
//! `projector-comms` — projection worker for the `comms` scope -- consumes the single log filtered to its scope's events, maintains only its schema's View_*, own checkpoint (D4).
//!
//! BUSINESS RUNTIME (#385): hosts the `comms`-scope projection worker: the shared registry filtered to this scope's groups, on their shared per-group checkpoints (monolith handover needs no re-projection). Serves the `/health` + `/ping` probes its generated
//! Deployment (deploy/generated/manifests/bins/projector-comms.yaml) declares (`wired:true`, readiness
