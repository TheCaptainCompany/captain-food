...
//! `adapter-coopcycle` — partner webhook ingestion bin for `coopcycle` -- verify, mirror, ACL, ENQUEUE on the shared mailbox; holds ONLY this partner's secrets (one bin per adapter, ADR-20260808-062432).
//!
//! BUSINESS RUNTIME (#385 API-tier wiring): hosts the `coopcycle` partner webhook ingestor: verify, mirror into the external_* journals, translate through the ACL, ENQUEUE on the shared mailbox — the owning actor bins deliver. This pod holds ONLY this partner's secrets (ADR-20260808-062432). Serves the `/health` + `/ping` probes its
//! generated Deployment (deploy/generated/manifests/bins/adapter-coopcycle.yaml) declares (`wired:true`),
//! draining on SIGTERM.
//!
...
const BIN: &str = "adapter-coopcycle";
