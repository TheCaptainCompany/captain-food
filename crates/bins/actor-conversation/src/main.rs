...
//!
//! BUSINESS RUNTIME (#385): hosts the `Conversation` mailbox lane's supervised worker fleet (the SAME `infrastructure::mailbox::standalone` runtime the adapter binaries use: leases, fencing, head-of-line, posture-gated money lanes, graceful drain). Serves the `/health` + `/ping` probes its generated
