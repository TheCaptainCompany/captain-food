...
//!
//! BUSINESS RUNTIME (#385 API-tier wiring): hosts the business-activity projection worker: the shared registry filtered to the `bam` scope label (no group carries it yet — the worker idles caught-up, honestly, until a bam view is specified). Serves the `/health` + `/ping` probes its
//! generated Deployment (deploy/generated/manifests/bins/bam.yaml) declares (`wired:true`),
//! draining on SIGTERM.
//!
