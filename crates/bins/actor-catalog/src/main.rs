...
//! `actor-catalog` — mailbox worker realizing the `Catalog` aggregate -- drains ONLY its own lanes (the LANES const it hands the shared fleet, not a link fact).
//!
//! BUSINESS RUNTIME (#385): hosts the `Catalog` mailbox lane's supervised worker fleet (the SAME `infrastructure::mailbox::standalone` runtime the adapter binaries use: leases, fencing, head-of-line, the money lanes' startup Stripe-fact backfill, graceful drain). Serves the `/health` + `/ping` probes its generated
//! Deployment (deploy/generated/manifests/bins/actor-catalog.yaml) declares (`wired:true`, readiness
