...
//! `graphql-delivery` — GraphQL subgraph for the `delivery` scope -- one domain, one graph, one GRANT (D8).
//!
//! BUSINESS RUNTIME (#385 API-tier wiring): serves `/{role}/graphql` for every role, restricted to the `delivery` scope's slice of the master schema (server::bin_support — same composition as the monolith, scope wall from the generated composition table). Serves the `/health` + `/ping` probes its
//! generated Deployment (deploy/generated/manifests/bins/graphql-delivery.yaml) declares (`wired:true`),
//! draining on SIGTERM.
//!
...
const BIN: &str = "graphql-delivery";
