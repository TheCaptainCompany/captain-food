...
//! `actor-prospect` — mailbox worker realizing the `Prospect` aggregate -- drains ONLY its own lanes (the scoping is the linker).
//!
//! PROBE-SERVING SHELL (gate-then-stabilize): binds `$PORT` and serves the `/health` + `/ping`
//! probes its generated Deployment (deploy/generated/manifests/bins/actor-prospect.yaml) declares,
