...
//! `pm-payment-settlement` — mailbox worker realizing the `PaymentSettlementProcess` process manager -- a DECLARED cross-scope bridge; its domain-crate links are its spec refs.
//!
//! BUSINESS RUNTIME (#385): hosts the `PaymentSettlementProcess` saga runner, restricted to this PM over its own `pm:` checkpoint. Serves the `/health` + `/ping` probes its generated
//! Deployment (deploy/generated/manifests/bins/pm-payment-settlement.yaml) declares (`wired:true`, readiness
