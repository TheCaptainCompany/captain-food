...
//! `graphql-network` — GraphQL subgraph for the `network` scope -- one domain, one graph, one GRANT (D8).
//!
//! BUSINESS RUNTIME (#385 API-tier wiring): serves `/{role}/graphql` for every role, restricted to the `network` scope's slice of the master schema (server::bin_support — same composition as the monolith, scope wall from the generated composition table). Serves the `/health` + `/ping` probes its
//! generated Deployment (deploy/generated/manifests/bins/graphql-network.yaml) declares (`wired:true`),
//! draining on SIGTERM.
//!
