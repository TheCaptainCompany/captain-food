...
//! `pm-cart-binding` — mailbox worker realizing the `CartBindingProcess` process manager -- a DECLARED cross-scope bridge; its domain-crate links are its spec refs.
//!
//! BUSINESS RUNTIME (#385): hosts the `CartBindingProcess` saga runner, restricted to this PM over its own `pm:` checkpoint. Serves the `/health` + `/ping` probes its generated
//! Deployment (deploy/generated/manifests/bins/pm-cart-binding.yaml) declares (`wired:true`, readiness
