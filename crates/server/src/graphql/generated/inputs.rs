...
pub struct PlaceOrderInput {
    ...
    /// DEPRECATED (ADR-20260906-192007 D-A), superseded by `quote` below. The equality check against the server-recomputed total (divergence rejects with `errors.yaml#/PriceMismatch`) is LIVE IN CODE and DEAD ON THE SHIPPED CLIENT: no shipped client sets this field (`crates/web/src/checkout.rs:610` is a test fixture, not a call site), so the check never actually fires in production today. It stays checked, unchanged, for whatever DOES send it, until the quote is verified server-side and this field's own check retires together with the write-side verify guard it hands off to (the two land in one change, `rules.yaml#/ServerPriceAuthority`). Kept nullable for the expand/contract discipline in the meantime. Its OWN deletion is a separate, later recorded change (the contract half of D-A), gated on a client cache-drain condition.
    #[graphql(name = "expectedTotal")]
    #[graphql(deprecation = "Retired by the quote (PROP-20260831-134539 slice 3b); today no shipped client sends it and the check is dead in production.")]
    pub expected_total: Option<MoneyInput>,
    /// The signed quote (ADR-20260906-192007 D-A/D-J) from the read that painted the checkout recap total the customer confirmed — never one stashed from an earlier read. Accepted structurally and NEVER READ by any code yet: the write door `configuration.yaml#/RUN_QUOTE_REQUIRED_ON_PLACE_ORDER` and the verify guard that would read this field both land only once the quote is verified server-side (D-F/D-G) — until then this field is pure expand, and every order still prices and charges at HEAD via the unchanged `expectedTotal` equality check above. Once that lands: CLOSED (default, everywhere) keeps this exact behaviour; OPEN, an ABSENT quote refuses at the HANDLER (typed) — never as a GraphQL non-null violation, so requiredness can flip without a same-change SDL edit; a PRESENT quote is verified (rules.yaml#/ServerPriceAuthority, D-F) and a mismatch produces exactly one of the enumerated refusals (D-D). The requiredness flip and `expectedTotal`'s deletion are each their own separate, later recorded steps — not this deliverable's.
    #[graphql(name = "quote")]
