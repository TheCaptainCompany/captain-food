...
impl MutationRoot {
    ...
    #[graphql(name = "grantPlatformAccess", guard = "RoleGuard::new(ALLOW_ADMIN).and(StandingGuard::new(&[], \"grantPlatformAccess\"))", visible = "visible_admin")]
    async fn grant_platform_access(&self, ctx: &async_graphql::Context<'_>, input: GrantPlatformAccessInput, metadata: Option<MetadataInput>) -> async_graphql::Result<MutationAcceptance> {
        // command.receive (SERVER). Opened before any fallible work so an input that fails to
        // deserialize still leaves a span naming the command that was attempted.
        let __receive = telemetry::spans::command_receive(
            "GrantPlatformAccess",
        ...
        // SYNC input validation (fail fast as a GraphQL error) AND the typed value the actor client
        // sends (#284 slice 2) -- the mailbox payload is the domain command's own serde form.
        let cmd: domain::generated::commands::GrantPlatformAccess = serde_json::from_value(payload_json.clone())
            .map_err(|e| async_graphql::Error::new(e.to_string()))?;
        let env = request_envelope(ctx, &metadata);
        // run_identity: both ids are mandatory in every contract and both may be server-generated.
        telemetry::spans::record_envelope(&__rx, &env.message_id.to_string(), &env.correlation_id.to_string());
        let actor_id = payload_json.get("platformMembershipId").and_then(|v| v.as_str()).and_then(|s| uuid::Uuid::parse_str(s).ok()).ok_or_else(|| async_graphql::Error::new("GrantPlatformAccess: identity property 'platformMembershipId' missing or not a uuid -- unaddressable"))?;
        // The TYPED DOOR (#284 slice 2, PROP-20260728-152752 §2.1): the generated PlatformMembershipClient
        // assembles the mailbox row through the SAME shared constructors the worker-channel
        // enqueue uses (lane, partition, kind, payload hash), so the GraphQL door can never
        // drift from any other door and no resolver builds a mailbox entry inline.
        let __client = client_platform_membership::PlatformMembershipClient::new(mailbox, actor_id);
        let __envelope = actor_client::mailbox::Envelope {
            message_id: env.message_id,
            correlation_id: env.correlation_id,
            cause_id: env.cause_id,
        ...
        match __outcome {
            actor_client::EnqueueOutcome::PayloadConflict(_) => {
                ...
                // acceptance outcome the contract does NOT count as success.
                telemetry::spans::record_journal_status(&__journal, telemetry::journal_status::CONFLICT);
                telemetry::meters::acceptance::sync_conflict("GrantPlatformAccess");
                return Err(conflict_error(env.message_id));
            }
            actor_client::EnqueueOutcome::Deduplicated(status) => {
                telemetry::spans::record_journal_status(&__journal, telemetry::journal_status::DUPLICATE);
    ...
    #[graphql(name = "requestAdminSignInLink", guard = "RoleGuard::new(ALLOW_PUBLIC).and(StandingGuard::new(&[], \"requestAdminSignInLink\"))", visible = "visible_public")]
    async fn request_admin_sign_in_link(&self, ctx: &async_graphql::Context<'_>, input: RequestAdminSignInLinkInput, metadata: Option<MetadataInput>) -> async_graphql::Result<MutationAcceptance> {
        // command.receive (SERVER). Opened before any fallible work so an input that fails to
        // deserialize still leaves a span naming the command that was attempted.
        let __receive = telemetry::spans::command_receive(
        ...
        // SYNC input validation (fail fast as a GraphQL error) AND the typed value the actor client
        // sends (#284 slice 2) -- the mailbox payload is the domain command's own serde form.
        let cmd: domain::generated::commands::RequestAdminSignInLink = serde_json::from_value(payload_json.clone())
            .map_err(|e| async_graphql::Error::new(e.to_string()))?;
        let env = request_envelope(ctx, &metadata);
        // run_identity: both ids are mandatory in every contract and both may be server-generated.
        telemetry::spans::record_envelope(&__rx, &env.message_id.to_string(), &env.correlation_id.to_string());
        ...
        // The TYPED DOOR (#284 slice 2, PROP-20260728-152752 §2.1): the generated AdminSignInClient
        // assembles the mailbox row through the SAME shared constructors the worker-channel
        // enqueue uses (lane, partition, kind, payload hash), so the GraphQL door can never
        // drift from any other door and no resolver builds a mailbox entry inline.
        let __client = client_admin_sign_in::AdminSignInClient::new(mailbox, actor_id);
        let __envelope = actor_client::mailbox::Envelope {
            message_id: env.message_id,
            correlation_id: env.correlation_id,
            cause_id: env.cause_id,
        ...
        match __outcome {
            actor_client::EnqueueOutcome::PayloadConflict(_) => {
                ...
                // acceptance outcome the contract does NOT count as success.
                telemetry::spans::record_journal_status(&__journal, telemetry::journal_status::CONFLICT);
                telemetry::meters::acceptance::sync_conflict("RequestAdminSignInLink");
                return Err(conflict_error(env.message_id));
            }
            actor_client::EnqueueOutcome::Deduplicated(status) => {
                telemetry::spans::record_journal_status(&__journal, telemetry::journal_status::DUPLICATE);
    ...
    #[graphql(name = "confirmAdminSignIn", guard = "RoleGuard::new(ALLOW_PUBLIC).and(StandingGuard::new(&[], \"confirmAdminSignIn\"))", visible = "visible_public")]
    async fn confirm_admin_sign_in(&self, ctx: &async_graphql::Context<'_>, input: ConfirmAdminSignInInput, metadata: Option<MetadataInput>) -> async_graphql::Result<MutationAcceptance> {
        // command.receive (SERVER). Opened before any fallible work so an input that fails to
        // deserialize still leaves a span naming the command that was attempted.
        let __receive = telemetry::spans::command_receive(
            "ConfirmAdminSignIn",
        ...
        // SYNC input validation (fail fast as a GraphQL error) AND the typed value the actor client
        // sends (#284 slice 2) -- the mailbox payload is the domain command's own serde form.
        let cmd: domain::generated::commands::ConfirmAdminSignIn = serde_json::from_value(payload_json.clone())
            .map_err(|e| async_graphql::Error::new(e.to_string()))?;
        let env = request_envelope(ctx, &metadata);
        // run_identity: both ids are mandatory in every contract and both may be server-generated.
        telemetry::spans::record_envelope(&__rx, &env.message_id.to_string(), &env.correlation_id.to_string());
        ...
        match __outcome {
            actor_client::EnqueueOutcome::PayloadConflict(_) => {
                ...
                // acceptance outcome the contract does NOT count as success.
                telemetry::spans::record_journal_status(&__journal, telemetry::journal_status::CONFLICT);
                telemetry::meters::acceptance::sync_conflict("ConfirmAdminSignIn");
                return Err(conflict_error(env.message_id));
            }
            actor_client::EnqueueOutcome::Deduplicated(status) => {
                telemetry::spans::record_journal_status(&__journal, telemetry::journal_status::DUPLICATE);
    ...
    async fn report_delivery_issue(&self, ctx: &async_graphql::Context<'_>, input: ReportDeliveryIssueInput, metadata: Option<MetadataInput>) -> async_graphql::Result<MutationAcceptance> {
        ...
        // run_identity: both ids are mandatory in every contract and both may be server-generated.
        telemetry::spans::record_envelope(&__rx, &env.message_id.to_string(), &env.correlation_id.to_string());
        let actor_id = payload_json.get("deliveryJobId").and_then(|v| v.as_str()).and_then(|s| uuid::Uuid::parse_str(s).ok()).ok_or_else(|| async_graphql::Error::new("ReportDeliveryIssue: identity property 'deliveryJobId' missing or not a uuid -- unaddressable"))?;
        // The TYPED DOOR (#284 slice 2, PROP-20260728-152752 §2.1): the generated DeliveryJobClient
        // assembles the mailbox row through the SAME shared constructors the worker-channel
        // enqueue uses (lane, partition, kind, payload hash), so the GraphQL door can never
        // drift from any other door and no resolver builds a mailbox entry inline.
        ...
        match __outcome {
            actor_client::EnqueueOutcome::PayloadConflict(_) => {
                ...
                // acceptance outcome the contract does NOT count as success.
                telemetry::spans::record_journal_status(&__journal, telemetry::journal_status::CONFLICT);
                telemetry::meters::acceptance::sync_conflict("ReportDeliveryIssue");
                return Err(conflict_error(env.message_id));
            }
            actor_client::EnqueueOutcome::Deduplicated(status) => {
                telemetry::spans::record_journal_status(&__journal, telemetry::journal_status::DUPLICATE);
    ...
    async fn resolve_delivery_issue(&self, ctx: &async_graphql::Context<'_>, input: ResolveDeliveryIssueInput, metadata: Option<MetadataInput>) -> async_graphql::Result<MutationAcceptance> {
        ...
        // run_identity: both ids are mandatory in every contract and both may be server-generated.
        telemetry::spans::record_envelope(&__rx, &env.message_id.to_string(), &env.correlation_id.to_string());
        let actor_id = payload_json.get("deliveryJobId").and_then(|v| v.as_str()).and_then(|s| uuid::Uuid::parse_str(s).ok()).ok_or_else(|| async_graphql::Error::new("ResolveDeliveryIssue: identity property 'deliveryJobId' missing or not a uuid -- unaddressable"))?;
        // The TYPED DOOR (#284 slice 2, PROP-20260728-152752 §2.1): the generated DeliveryJobClient
        // assembles the mailbox row through the SAME shared constructors the worker-channel
        // enqueue uses (lane, partition, kind, payload hash), so the GraphQL door can never
        // drift from any other door and no resolver builds a mailbox entry inline.
        ...
        match __outcome {
            actor_client::EnqueueOutcome::PayloadConflict(_) => {
                ...
                // acceptance outcome the contract does NOT count as success.
                telemetry::spans::record_journal_status(&__journal, telemetry::journal_status::CONFLICT);
                telemetry::meters::acceptance::sync_conflict("ResolveDeliveryIssue");
                return Err(conflict_error(env.message_id));
            }
            actor_client::EnqueueOutcome::Deduplicated(status) => {
                telemetry::spans::record_journal_status(&__journal, telemetry::journal_status::DUPLICATE);
    ...
    async fn decline_delivery(&self, ctx: &async_graphql::Context<'_>, input: DeclineDeliveryInput, metadata: Option<MetadataInput>) -> async_graphql::Result<MutationAcceptance> {
        ...
        // run_identity: both ids are mandatory in every contract and both may be server-generated.
        telemetry::spans::record_envelope(&__rx, &env.message_id.to_string(), &env.correlation_id.to_string());
        let actor_id = payload_json.get("deliveryJobId").and_then(|v| v.as_str()).and_then(|s| uuid::Uuid::parse_str(s).ok()).ok_or_else(|| async_graphql::Error::new("DeclineDelivery: identity property 'deliveryJobId' missing or not a uuid -- unaddressable"))?;
        // The TYPED DOOR (#284 slice 2, PROP-20260728-152752 §2.1): the generated DeliveryJobClient
        // assembles the mailbox row through the SAME shared constructors the worker-channel
        // enqueue uses (lane, partition, kind, payload hash), so the GraphQL door can never
        // drift from any other door and no resolver builds a mailbox entry inline.
        ...
        match __outcome {
            actor_client::EnqueueOutcome::PayloadConflict(_) => {
                ...
                // acceptance outcome the contract does NOT count as success.
                telemetry::spans::record_journal_status(&__journal, telemetry::journal_status::CONFLICT);
                telemetry::meters::acceptance::sync_conflict("DeclineDelivery");
                return Err(conflict_error(env.message_id));
            }
            actor_client::EnqueueOutcome::Deduplicated(status) => {
                telemetry::spans::record_journal_status(&__journal, telemetry::journal_status::DUPLICATE);
    ...
    async fn hand_back_delivery(&self, ctx: &async_graphql::Context<'_>, input: HandBackDeliveryInput, metadata: Option<MetadataInput>) -> async_graphql::Result<MutationAcceptance> {
        ...
        // SYNC input validation (fail fast as a GraphQL error) AND the typed value the actor client
        // sends (#284 slice 2) -- the mailbox payload is the domain command's own serde form.
        let cmd: domain::generated::commands::HandBackDelivery = serde_json::from_value(payload_json.clone())
            .map_err(|e| async_graphql::Error::new(e.to_string()))?;
        let env = request_envelope(ctx, &metadata);
        // run_identity: both ids are mandatory in every contract and both may be server-generated.
        telemetry::spans::record_envelope(&__rx, &env.message_id.to_string(), &env.correlation_id.to_string());
        let actor_id = payload_json.get("deliveryJobId").and_then(|v| v.as_str()).and_then(|s| uuid::Uuid::parse_str(s).ok()).ok_or_else(|| async_graphql::Error::new("HandBackDelivery: identity property 'deliveryJobId' missing or not a uuid -- unaddressable"))?;
        // The TYPED DOOR (#284 slice 2, PROP-20260728-152752 §2.1): the generated DeliveryJobClient
        // assembles the mailbox row through the SAME shared constructors the worker-channel
        // enqueue uses (lane, partition, kind, payload hash), so the GraphQL door can never
        // drift from any other door and no resolver builds a mailbox entry inline.
        ...
        match __outcome {
            actor_client::EnqueueOutcome::PayloadConflict(_) => {
                ...
                // acceptance outcome the contract does NOT count as success.
                telemetry::spans::record_journal_status(&__journal, telemetry::journal_status::CONFLICT);
                telemetry::meters::acceptance::sync_conflict("HandBackDelivery");
                return Err(conflict_error(env.message_id));
            }
            actor_client::EnqueueOutcome::Deduplicated(status) => {
                telemetry::spans::record_journal_status(&__journal, telemetry::journal_status::DUPLICATE);
    ...
    async fn grant_restaurant_access(&self, ctx: &async_graphql::Context<'_>, input: GrantRestaurantAccessInput, metadata: Option<MetadataInput>) -> async_graphql::Result<MutationAcceptance> {
        ...
        // SYNC input validation (fail fast as a GraphQL error) AND the typed value the actor client
        // sends (#284 slice 2) -- the mailbox payload is the domain command's own serde form.
        let cmd: domain::generated::commands::GrantRestaurantAccess = serde_json::from_value(payload_json.clone())
            .map_err(|e| async_graphql::Error::new(e.to_string()))?;
        let env = request_envelope(ctx, &metadata);
        // run_identity: both ids are mandatory in every contract and both may be server-generated.
        telemetry::spans::record_envelope(&__rx, &env.message_id.to_string(), &env.correlation_id.to_string());
        let actor_id = payload_json.get("membershipId").and_then(|v| v.as_str()).and_then(|s| uuid::Uuid::parse_str(s).ok()).ok_or_else(|| async_graphql::Error::new("GrantRestaurantAccess: identity property 'membershipId' missing or not a uuid -- unaddressable"))?;
        // The TYPED DOOR (#284 slice 2, PROP-20260728-152752 §2.1): the generated RestaurantMembershipClient
        // assembles the mailbox row through the SAME shared constructors the worker-channel
        // enqueue uses (lane, partition, kind, payload hash), so the GraphQL door can never
        // drift from any other door and no resolver builds a mailbox entry inline.
        let __client = client_restaurant_membership::RestaurantMembershipClient::new(mailbox, actor_id);
        let __envelope = actor_client::mailbox::Envelope {
            message_id: env.message_id,
            correlation_id: env.correlation_id,
            cause_id: env.cause_id,
        ...
        match __outcome {
            actor_client::EnqueueOutcome::PayloadConflict(_) => {
                ...
                // acceptance outcome the contract does NOT count as success.
                telemetry::spans::record_journal_status(&__journal, telemetry::journal_status::CONFLICT);
                telemetry::meters::acceptance::sync_conflict("GrantRestaurantAccess");
                return Err(conflict_error(env.message_id));
            }
            actor_client::EnqueueOutcome::Deduplicated(status) => {
                telemetry::spans::record_journal_status(&__journal, telemetry::journal_status::DUPLICATE);
    ...
    #[graphql(name = "grantRestaurantAccessByInvitation", guard = "RoleGuard::new(ALLOW_PUBLIC).and(StandingGuard::new(&[], \"grantRestaurantAccessByInvitation\"))", visible = "visible_public")]
    async fn grant_restaurant_access_by_invitation(&self, ctx: &async_graphql::Context<'_>, input: GrantRestaurantAccessByInvitationInput, metadata: Option<MetadataInput>) -> async_graphql::Result<MutationAcceptance> {
        // command.receive (SERVER). Opened before any fallible work so an input that fails to
        // deserialize still leaves a span naming the command that was attempted.
        let __receive = telemetry::spans::command_receive(
        ...
        // SYNC input validation (fail fast as a GraphQL error) AND the typed value the actor client
        // sends (#284 slice 2) -- the mailbox payload is the domain command's own serde form.
        let cmd: domain::generated::commands::GrantRestaurantAccessByInvitation = serde_json::from_value(payload_json.clone())
            .map_err(|e| async_graphql::Error::new(e.to_string()))?;
        let env = request_envelope(ctx, &metadata);
        // run_identity: both ids are mandatory in every contract and both may be server-generated.
        telemetry::spans::record_envelope(&__rx, &env.message_id.to_string(), &env.correlation_id.to_string());
        ...
        match __outcome {
            actor_client::EnqueueOutcome::PayloadConflict(_) => {
                ...
                // acceptance outcome the contract does NOT count as success.
                telemetry::spans::record_journal_status(&__journal, telemetry::journal_status::CONFLICT);
                telemetry::meters::acceptance::sync_conflict("GrantRestaurantAccessByInvitation");
                return Err(conflict_error(env.message_id));
            }
            actor_client::EnqueueOutcome::Deduplicated(status) => {
                telemetry::spans::record_journal_status(&__journal, telemetry::journal_status::DUPLICATE);
    ...
    #[graphql(name = "revokeRestaurantAccess", guard = "RoleGuard::new(ALLOW_ADMIN).and(StandingGuard::new(&[], \"revokeRestaurantAccess\"))", visible = "visible_admin")]
    async fn revoke_restaurant_access(&self, ctx: &async_graphql::Context<'_>, input: RevokeRestaurantAccessInput, metadata: Option<MetadataInput>) -> async_graphql::Result<MutationAcceptance> {
        // command.receive (SERVER). Opened before any fallible work so an input that fails to
        // deserialize still leaves a span naming the command that was attempted.
        let __receive = telemetry::spans::command_receive(
        ...
        // SYNC input validation (fail fast as a GraphQL error) AND the typed value the actor client
        // sends (#284 slice 2) -- the mailbox payload is the domain command's own serde form.
        let cmd: domain::generated::commands::RevokeRestaurantAccess = serde_json::from_value(payload_json.clone())
            .map_err(|e| async_graphql::Error::new(e.to_string()))?;
        let env = request_envelope(ctx, &metadata);
        // run_identity: both ids are mandatory in every contract and both may be server-generated.
        telemetry::spans::record_envelope(&__rx, &env.message_id.to_string(), &env.correlation_id.to_string());
        let actor_id = payload_json.get("membershipId").and_then(|v| v.as_str()).and_then(|s| uuid::Uuid::parse_str(s).ok()).ok_or_else(|| async_graphql::Error::new("RevokeRestaurantAccess: identity property 'membershipId' missing or not a uuid -- unaddressable"))?;
        // The TYPED DOOR (#284 slice 2, PROP-20260728-152752 §2.1): the generated RestaurantMembershipClient
        // assembles the mailbox row through the SAME shared constructors the worker-channel
        // enqueue uses (lane, partition, kind, payload hash), so the GraphQL door can never
        // drift from any other door and no resolver builds a mailbox entry inline.
        ...
        match __outcome {
            actor_client::EnqueueOutcome::PayloadConflict(_) => {
                ...
                // acceptance outcome the contract does NOT count as success.
                telemetry::spans::record_journal_status(&__journal, telemetry::journal_status::CONFLICT);
                telemetry::meters::acceptance::sync_conflict("RevokeRestaurantAccess");
                return Err(conflict_error(env.message_id));
            }
            actor_client::EnqueueOutcome::Deduplicated(status) => {
                telemetry::spans::record_journal_status(&__journal, telemetry::journal_status::DUPLICATE);
    ...
    #[graphql(name = "requestMemberSignInLink", guard = "RoleGuard::new(ALLOW_PUBLIC).and(StandingGuard::new(&[], \"requestMemberSignInLink\"))", visible = "visible_public")]
    async fn request_member_sign_in_link(&self, ctx: &async_graphql::Context<'_>, input: RequestMemberSignInLinkInput, metadata: Option<MetadataInput>) -> async_graphql::Result<MutationAcceptance> {
        // command.receive (SERVER). Opened before any fallible work so an input that fails to
        // deserialize still leaves a span naming the command that was attempted.
        let __receive = telemetry::spans::command_receive(
        ...
        // SYNC input validation (fail fast as a GraphQL error) AND the typed value the actor client
        // sends (#284 slice 2) -- the mailbox payload is the domain command's own serde form.
        let cmd: domain::generated::commands::RequestMemberSignInLink = serde_json::from_value(payload_json.clone())
            .map_err(|e| async_graphql::Error::new(e.to_string()))?;
        let env = request_envelope(ctx, &metadata);
        // run_identity: both ids are mandatory in every contract and both may be server-generated.
        telemetry::spans::record_envelope(&__rx, &env.message_id.to_string(), &env.correlation_id.to_string());
        ...
        match __outcome {
            actor_client::EnqueueOutcome::PayloadConflict(_) => {
                ...
                // acceptance outcome the contract does NOT count as success.
                telemetry::spans::record_journal_status(&__journal, telemetry::journal_status::CONFLICT);
                telemetry::meters::acceptance::sync_conflict("RequestMemberSignInLink");
                return Err(conflict_error(env.message_id));
            }
            actor_client::EnqueueOutcome::Deduplicated(status) => {
                telemetry::spans::record_journal_status(&__journal, telemetry::journal_status::DUPLICATE);
    ...
    #[graphql(name = "confirmMemberSignIn", guard = "RoleGuard::new(ALLOW_PUBLIC).and(StandingGuard::new(&[], \"confirmMemberSignIn\"))", visible = "visible_public")]
    async fn confirm_member_sign_in(&self, ctx: &async_graphql::Context<'_>, input: ConfirmMemberSignInInput, metadata: Option<MetadataInput>) -> async_graphql::Result<MutationAcceptance> {
        // command.receive (SERVER). Opened before any fallible work so an input that fails to
        // deserialize still leaves a span naming the command that was attempted.
        let __receive = telemetry::spans::command_receive(
            "ConfirmMemberSignIn",
        ...
        // SYNC input validation (fail fast as a GraphQL error) AND the typed value the actor client
        // sends (#284 slice 2) -- the mailbox payload is the domain command's own serde form.
        let cmd: domain::generated::commands::ConfirmMemberSignIn = serde_json::from_value(payload_json.clone())
            .map_err(|e| async_graphql::Error::new(e.to_string()))?;
        let env = request_envelope(ctx, &metadata);
        // run_identity: both ids are mandatory in every contract and both may be server-generated.
        telemetry::spans::record_envelope(&__rx, &env.message_id.to_string(), &env.correlation_id.to_string());
    ...
    #[graphql(name = "inviteRestaurantMember", guard = "RoleGuard::new(ALLOW_RESTAURANT).and(StandingGuard::new(&[], \"inviteRestaurantMember\")).and(AuthorityGuard::new(domain::generated::scalars::MemberAuthority::MANAGER))", visible = "visible_restaurant")]
    ...
    #[graphql(name = "revokeRestaurantInvitation", guard = "RoleGuard::new(ALLOW_RESTAURANT).and(StandingGuard::new(&[], \"revokeRestaurantInvitation\")).and(AuthorityGuard::new(domain::generated::scalars::MemberAuthority::MANAGER))", visible = "visible_restaurant")]
