...
/// An OPAQUE, server-signed token binding a cart's PRICES — never its display labels, which the read always resolves fresh at HEAD (`application::pricing::price_cart_at`'s own split: "HEAD's business, never this capability's") — to the exact catalog coordinate that produced them (PROP-20260831-134539 slice 3b, ADR-20260906-192007 D-A/D-J) — base64url, no internal structure a client may parse or construct. Returned by `cart.current`/`cart` on the SAME read that painted the total the customer sees; `null` on that same read means one of three things, never distinguished on the wire (D-D's cause-neutral posture, restated at the read): the fold-priced read is CLOSED (`configuration.yaml#/RUN_FOLD_PRICED_CART_READ` off — no coordinate exists to sign), the fold itself failed, or this is the `carts` LIST read (D-L: the fan-out never mints, structurally). The checkout submits the quote of the read that painted `/checkout`'s own recap total, never one stashed at `/cart` (restaurant_frontoffice.yaml — the /cart→/checkout binding). WIRED on the write side (corrected checkpoint 2, evans item H — this used to say "DARK ... until the quote is verified server-side", a claim phase C made false): `application::quote::verify_quote` (`application::commands::place_order`'s pre-payment block) reads and cryptographically verifies `PlaceOrder.quote` whenever the write door (`configuration.yaml#/RUN_QUOTE_REQUIRED_ON_PLACE_ORDER`) is OPEN — CLOSED by default everywhere, the only state shipped flipped on today, in which the value is still accepted structurally and left unread, exactly the prior DARK behaviour. NOTE (reviewer NB3): `pattern`/`maxLength` above are STILL enforced by NO emitter — the generated newtype (`domains::ordering::scalars::CartQuote(pub String)`) stays a bare wrapper with no validation, and the now-landed verifier (`decode_and_check_signature`) does not perform a `pattern`/ `maxLength` check either: base64/JSON decode failure is what rejects a malformed token (`QuoteRefusal::Tampered`), never this declaration — NB3's finding survives the verifier landing.
#[derive(Debug, Clone, PartialEq, Eq, Hash, serde::Serialize, serde::Deserialize)]
