...
pub struct Config {
    ...
    /// DEFAULT `false`. The write-side door (ADR-20260906-192007 D-A/D-B/D-F, WIRED). CLOSED (default, everywhere, and the only state this deliverable SHIPS FLIPPED-ON): `quote` on `PlaceOrder` is accepted structurally and NEVER READ -- every order is priced and charged AT HEAD from the live catalog projection, exactly as before this key existed; `expectedTotal`'s equality check is deleted (`errors.yaml#/PriceMismatch` retired). OPEN: an absent `quote` refuses at the handler with `errors.yaml#/QuoteVerificationFailed` (typed, never SDL non-null); a present `quote` is verified (`application::quote::verify_quote`) against `as_of(catalogId, V)` at the coordinate it names (never `at_head` -- D-F), and a mismatch produces exactly one of the enumerated refusals (D-D) -- the charge becomes that fold's `price_cart_at` recompute, never the HEAD projection. INTERLOCKED, not a shared key (D-B): this door may only be `true` when `RUN_FOLD_PRICED_CART_READ` is ALSO `true` -- the write-door witness (`application::quote::WriteDoorOpen`) is CONSTRUCTED FROM both bools together (compiler-first, ADR-20260803-234035 level 4), so the mixed state (verify against a coordinate the read side never mints) is unrepresentable rather than merely checked; a boot refusal at BOTH composition roots (`QuoteGuard::resolve_at_boot`), the CONSTRUCTOR pinned by `application::quote::tests::the_write_door_cannot_open_while_the_read_door_is_closed` -- R5 (fix round after presentation pass 1, beck NB6 / farley NB10): that pin covers `WriteDoorOpen`'s own interlock, never the two `declare_flag` call sites themselves (`crates/server/src/lib.rs`, `crates/infrastructure/src/mailbox/standalone.rs`) -- a root-level wiring test per composition root is a flip precondition (`docs/decisions/QUOTE-MINT-PRECONDITIONS.yaml` item 33), not yet written. `decisionRow` binds this key to `docs/decisions/QUOTE-MINT-PRECONDITIONS.yaml` (open): the door opens ONCE, after every item on that row discharges. `declare_flag` at BOTH composition roots (`crates/server/src/lib.rs`, `crates/infrastructure/src/mailbox/standalone.rs`) is the fleet-parity evidence (ADR-20260905-223957 §5, ADR-20260906-113444) -- the standalone root's line is the PRE-RECORDED carve-out of ADR-20260904-081527 §8's standing clause, since this key is a `runKind: door` newly introduced.
    pub run_quote_required_on_place_order: bool,
    /// The signed quote's CURRENT signing key (ADR-20260906-192007 D-H), copying `specs/common/configuration.yaml#/EMAIL_QUOTA_KEY_HMAC_SECRET` key-for-key. REVERTED (checkpoint 2, farley + business + reviewer item J) back to `required: []`, TEMPORARILY -- a brief promotion to `required: [staging, production]` (phase C) was found to add ZERO safety with the write door OFF (the default everywhere, the only state shipped today): `QuoteGuard::resolve_at_boot`'s OWN dev-key refusal already catches an unprovisioned/dev key whenever the door is OPEN in a live profile (see below), so a MANIFEST-level `required:` only wedges every one of the 14 bin deploy manifests + `deploy.yml:68` (incl. the rollback path) on a secret nothing consumes while the door stays closed. The promotion is the FLIP CARD's, landing alongside the door actually opening (D-H's own two-step rollout, restated: the manifest requirement gates the FLIP, never the code that merely CAN read the key). NO default -- a defaulted signing key is a forgeable price (dba). Unset in development/test, a fixed `DEV_ONLY` value is used (`application::quote::DEV_ONLY_HMAC_KEY`, `application::email_guard::DEV_ONLY_HMAC_KEY`'s own precedent), and the write door REFUSES TO OPEN AT BOOT when the resolved key is that dev value (an ABSENT secret resolves to the SAME dev-only fallback, so "absent" and "the placeholder dev value" are ONE case, not two) and the door is open in a LIVE profile (staging/production) -- `application::quote::QuoteGuard::resolve_at_boot`, never a per-request check, pinned by a named Rust unit test (`noTestFixturePossible` class: no `tests.yaml` scenario can select a profile). `application::quote::SigningKey` wraps this value with a module-private constructor (the `ActingRole` shape, ADR-20260803-234035 level 4) so the dev value is uncallable from the production construction path. `QuoteMinter` holds exactly ONE current key from this value (never the plural set below) -- a minter cannot accidentally verify. Key CUSTODY is admin-gated, the founder's external list (not a crypto-shredding key -- nothing personal is encrypted by it); rotation policy is the team's, recorded as: single key + one live `keyId` is acceptable at V0 provided the overlap window below is observed on any rotation.
    pub quote_signing_key_hmac_secret: Option<String>,
    /// The signed quote's RETIRED-BUT-STILL-ACCEPTED key (ADR-20260906-192007 D-H) -- explicitly `required: []` PERMANENTLY (round 3, ADR-20260815-015422) -- unlike its `QUOTE_SIGNING_KEY_HMAC_SECRET` current-key sibling, which is TEMPORARILY `required: []` too as of checkpoint 2 item J (reverted from a brief promotion, above) but only until the flip card promotes it back alongside the write door actually opening: absent whenever no rotation is in flight. `QuoteVerifier` holds a key SET built from both values (plural -- it must accept a quote signed under a key that has since been retired but is still inside the overlap window), a DIFFERENT type from `QuoteMinter`'s single current key, so a verifier cannot accidentally mint. A two-key overlap of 60 minutes (`N` + skew, `N` = the existing 30-minute `QUOTE-STALENESS` backstop) is a PRECONDITION of this deliverable, not of a later rotation: without it, a hard key-rotation cutover rejects every quote minted in the preceding 30 minutes platform-wide. UNVERIFIED: the 60-minute figure implies a 30-minute `skew` term (`N` + skew = 60), but no source names 30 minutes of clock skew as the operative bound -- the `skew` value itself has no recorded antecedent and needs one before this figure is treated as load-bearing.
...
const BAKED: &[(&str, &str, &str)] = &[
    ...
    ("RUN_SIRENE_WORKER", "production", "false"),
    ("RUN_SIRENE_WORKER", "staging", "false"),
