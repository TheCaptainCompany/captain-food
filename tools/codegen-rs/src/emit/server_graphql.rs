...
pub(crate) fn wired_mutation_dispatch(name: &str) -> Option<String> {
    ...
    if name == "placeOrder" {
        return Some(
            ...
            // app restart (#12); the router's prelude binds `env` from the mailbox row.
            "application::commands::place_order(store.as_ref(), catalogs.as_ref(), payments.as_ref(), pm_state.as_ref(), cmd, env.session_id.map(domain::generated::scalars::SessionId), &actor, chrono::Utc::now()).await.map(|_| ())".into(),
        );
    }
...
pub(crate) fn wired_subscription_body(name: &str) -> Option<&'static str> {
    match name {
        ...
        // below — on a long-lived subscription each pushed update is its own request, and a
            ...
            'events: loop {
                ...
                for attempt in 0..12u32 {
                    ...
                    match restaurants.by_id(row.restaurant_id).await {
                        Ok(Some(restaurant)) => yield Ok(Order::from((row, Restaurant::at(restaurant, chrono::Utc::now(), service_window_horizon)))),
                        Ok(None) => {}
