...
pub(crate) fn emit_server_types(model: &Model) -> String {
    ...
    // Cart: the row is a MONEY-FREE pure fold (ADR-20260810-112836) — every priced field of the API
    // type (lines, totalAmount, breakdown, uberComparison) is computed at READ TIME by
    // application::pricing::price_cart at the resolver seam. This From impl carries only the
    // fold-backed fields; until the Phase-2 read-side pricer lands (#451), the priced fields hold the
    // DEGENERATE placeholder the pre-#451 stub projector produced (empty lines, 0 EUR, no breakdown)
    // — same rendered outcome as before, now sourced honestly from "not priced yet" instead of a
    // fake stored price.
    out.push_str(
...
pub(crate) fn render_gated_pm_resolver(
    name: &str,
    acl: &str,
    fnname: &str,
    command: &str,
    ...
    GATED_PM_RESOLVER_TEMPLATE
        ...
        .replace("{actor_crate}", &client_crate_ident(actor))
        .replace("{actor}", actor)
...
pub(crate) fn wired_mutation_dispatch(name: &str) -> Option<(String, String)> {
    ...
    let (resolve_extra, extra_arg) = match extra {
        ...
        Extra::MailboxRequeue => (
            "        let mailbox_requeue = ctx.data::<std::sync::Arc<dyn application::queries::MailboxRequeue>>()?.clone();\n".to_string(),
            ", mailbox_requeue.as_ref()",
        ),
    };
    Some((
        format!(
...
pub(crate) struct CommandAddress {
    ...
    pub(crate) process_manager: bool,
}
