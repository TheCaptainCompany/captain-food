...
pub(crate) fn emit_web_screens(model: &Model) -> String {
    ...
    for sf in &surfaces {
        ...
        for s in screens {
            ...
            // R1 (§26): the per-screen transport role, verbatim from the DSL (validated to be a
            // UserType member admitted by every bound operation).
            let graphql_role = s
                .get("graphql_role")
                .and_then(|v| v.as_str())
                .map(|r| format!("Some(\"{}\")", rust_str(r)))
                .unwrap_or_else(|| "None".to_string());
            let unauthenticated_route = s
                .get("unauthenticated")
