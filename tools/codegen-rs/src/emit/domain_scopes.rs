...
/// The scopes that get a domain crate, WITHOUT generating any code — the same condition
/// `emit_domain_scope_crates` applies (≥1 non-empty type module, or the kernel's load-bearing
/// errors module), extracted so VALIDATION paths (§15 bin topology) can ask "which crates exist"
/// on minimal fixture models whose item bodies the code emitters would reject.
pub(crate) fn crate_scopes(model: &Model) -> BTreeSet<String> {
    let any_errors =
    ...
    model
        ...
        .iter()
        .filter(|scope| {
            TYPE_KINDS.iter().any(|(kind, _)| {
                !scope_items(model, scope, kind).is_empty()
                    || (*kind == "errors.yaml" && scope.as_str() == KERNEL_SCOPE && any_errors)
            })
        })
        .cloned()
        .collect()
...
pub(crate) fn emit_crate_graph(model: &Model) -> String {
    ...
    for b in bin_topology(model) {
        let linked: Vec<serde_json::Value> = b
            .domain_scopes
            .iter()
            .map(|s| serde_json::Value::String(domain_crate_name(s)))
            .collect();
        let kind = match b.family {
            "actor" => "aggregate",
            "pm" => "process-manager",
            f => f,
        };
        let mut entry = serde_json::Map::new();
        if let Some(actor) = &b.actor {
            entry.insert("actor".to_string(), serde_json::json!(actor));
        }
        ...
        entry.insert("domain_crates".to_string(), serde_json::Value::Array(linked));
        entry.insert("path".to_string(), serde_json::json!(format!("crates/bins/{}", b.name)));
        bins.insert(b.name, serde_json::Value::Object(entry));
    }
    let doc = serde_json::json!({
        "_generated": "GENERATED by the Captain.Food codegen from specs/{scope}/ $refs and the c4-l2 container list (#373 step 2 + #382 step 3, ADR-20260807-183024) -- do not edit by hand. The derived crate + bin topology: the spec's coupling as Cargo's. `bins` covers every deployable (aggregate/process-manager/projector/subgraph/gateway/surface/worker); each entry's domain_crates is the SCOPE ASSERTION its crates/bins/ manifest links (a PM's refs are its declared dependency list), and the input contract for the #349 manifests emitter.",
