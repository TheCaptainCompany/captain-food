...
pub(crate) fn emit_config(model: &Model) -> String {
    ...
    emit_config_module(model, &keys, /* strict_windows */ true)
}
...
pub(crate) fn scoped_config_keys(model: &Model, scopes: &BTreeSet<String>) -> Vec<ConfigKey> {
    parse_config_keys(model)
        ...
        .filter(|k| {
            let origin = model
                .origins
                .get(&("configuration.yaml".to_string(), k.name.clone()))
                .map(|s| s.as_str())
...
/// The generated typed reader over an explicit key subset. `strict_windows` controls the
/// reminder-window table: the SERVER asserts every actors.yaml window key is declared (it hosts
/// every lane, so a missing key is a codegen bug); a BIN filters instead (it hosts a lane subset,
/// and a window key outside its scopes belongs to a lane it never drains — the standalone fleet's
/// spec-default fallback covers the honest gap loudly at runtime).
pub(crate) fn emit_config_module(model: &Model, keys: &[ConfigKey], strict_windows: bool) -> String {
